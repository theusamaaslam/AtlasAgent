...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Atlas Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ATLAS_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/atlas), this function
    mirrors the bridge source to a writable ATLAS_HOME location and returns that
    ...
    # Try ATLAS_HOME location first
    atlas_home = get_atlas_home()
    atlas_home_bridge = atlas_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ATLAS_HOME if needed
    if atlas_home_bridge.exists():
        return atlas_home_bridge

    # Mirror the bridge source to ATLAS_HOME
    try:
        atlas_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            atlas_home_bridge,
            dirs_exist_ok=False,
