...
There are two ways to add a platform to the Atlas gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.atlas/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Atlas code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `atlas gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `atlas_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Atlas core.
...
And add it to the `atlas-gateway` composite:

```python
"atlas-gateway": {
    "includes": [..., "atlas-your-platform"]
}
```
...
## 12. Status Display (`atlas_cli/status.py`)
...
## 13. Gateway Setup Wizard (`atlas_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ atlas_cli/ toolsets.py \
  --include="*.py" -l | sort -u
