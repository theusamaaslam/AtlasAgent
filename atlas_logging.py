"""Centralized logging setup for Atlas Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.atlas/logs/`` (profile-aware via ``get_atlas_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``atlas_cli.web_server``,
    ``atlas_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Atlas (TUI, gateway, ``hy_memory`` server, MCP
...
from atlas_constants import get_config_path, get_atlas_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_atlas_session_injector", False):
    ...
    _session_record_factory._atlas_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``atlas logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``atlas logs --component gateway``.
    "gateway": ("gateway", "atlas_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Atlas logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    atlas_home
        Override for the Atlas home directory.  Falls back to
        ``get_atlas_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_atlas_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._atlas_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the atlas group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from atlas_cli.config import is_managed
