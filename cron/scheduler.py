...
Uses a file-based lock (~/.atlas/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `atlas update` reloads
# the module) fail with ModuleNotFoundError for atlas_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from atlas_constants import get_atlas_home
from atlas_cli._subprocess_compat import windows_hide_flags
from atlas_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy atlas_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from atlas_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``atlas tools`` config for the ``cron`` platform.
    ...
    try:
        from atlas_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_atlas_home: Path | None = None


def _get_atlas_home() -> Path:
    """Resolve Atlas home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active ATLAS_HOME at call time
    ...
    """
    return _atlas_home or get_atlas_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    atlas_home = _get_atlas_home()
    lock_dir = atlas_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ATLAS_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ATLAS_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ATLAS_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Atlas-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ATLAS_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_atlas_home() / "scripts"
    ...
    # escape — scripts MUST reside within ATLAS_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _atlas_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_atlas_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_atlas_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["ATLAS_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed ATLAS_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ATLAS_SESSION_PLATFORM
    #     and ATLAS_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ATLAS_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ATLAS_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read ATLAS_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ATLAS_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ATLAS_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "ATLAS_CRON_AUTO_DELIVER_PLATFORM",
        "ATLAS_CRON_AUTO_DELIVER_CHAT_ID",
        "ATLAS_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_atlas_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this ATLAS_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_atlas_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from atlas_cli.env_loader import (
            load_atlas_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_atlas_dotenv(atlas_home=_get_atlas_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["ATLAS_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ATLAS_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ATLAS_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > ATLAS_MODEL env >
        ...
        model = job.get("model") or os.getenv("ATLAS_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_atlas_home() / "config.yaml")
        ...
        prefill_file = (
            os.getenv("ATLAS_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject ATLAS_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # ATLAS_HOME. When a workdir is configured, also inject project
        ...
        # override via ATLAS_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ATLAS_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ATLAS_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_atlas_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_atlas_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_atlas_home())
        )
        try:
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _atlas_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _atlas_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ATLAS_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ATLAS_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ATLAS_CRON_MAX_PARALLEL value; defaulting to unbounded")
