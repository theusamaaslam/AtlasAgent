#!/usr/bin/env bash
# Canonical test runner for atlas-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.atlas/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.atlas/atlas-agent/venv"; do
...
if [ -f "$HOME/.atlas/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.atlas"
...
exec env -i \
  ...
  ${ATLAS_RUN_SLOW_PET_TESTS:+ATLAS_RUN_SLOW_PET_TESTS="$ATLAS_RUN_SLOW_PET_TESTS"} \
