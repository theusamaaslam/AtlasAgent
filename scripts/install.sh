...
# Python-driven tool session (e.g. Atlas terminal tool). A pre-set PYTHONPATH
...
#   code at /usr/local/lib/atlas-agent, command at /usr/local/bin/atlas,
#   data still at /root/.atlas (ATLAS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ATLAS_HOME/.no-bundled-skills so future"
            echo "                   'atlas update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Atlas.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.atlas/atlas-agent"
            echo "                   default (root, Linux): /usr/local/lib/atlas-agent"
            echo "  --atlas-home PATH  Data directory (default: ~/.atlas, or \$ATLAS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Atlas installs the code under"
            echo "  /usr/local/lib/atlas-agent and links the command into"
            echo "  /usr/local/bin/atlas (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ATLAS_HOME"
            echo "  (default /root/.atlas).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ATLAS_HOME/atlas-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs atlas setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Atlas Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Usama Aslam.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `atlas update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Atlas-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Atlas Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install atlas command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `atlas` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ATLAS_HOME/atlas-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ATLAS_HOME/atlas-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/atlas-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ATLAS_HOME/atlas-agent — then preserve it)
#
# Always no-op when the user set --dir or $ATLAS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ATLAS_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ATLAS_HOME/atlas-agent/.git" ]; then
            INSTALL_DIR="$ATLAS_HOME/atlas-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/atlas-agent)"
            return 0
        fi
        ...
        # /usr/local/bin/atlas wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/atlas"
        log_info "  Data:    $ATLAS_HOME (unchanged)"
...
# Point a Atlas-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ATLAS_HOME/node/bin — which is NOT on
...
# Atlas's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Atlas-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ATLAS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ATLAS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ATLAS_HOME/node/etc/npmrc"
}

get_atlas_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/atlas" ]; then
        echo "$link_dir/atlas"
    else
        echo "atlas"
    fi
...
install_uv() {
    ...
    # Atlas owns its own uv at $ATLAS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (atlas_cli/managed_uv.py) looks in the same
    # place, so install.sh and `atlas update` stay in sync.
    local _managed_uv="$ATLAS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ATLAS_HOME/bin ..."
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/atlas-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/atlas-uv-installer.$$.sh")"
    ...
    # directly into $ATLAS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ATLAS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Atlas-
...
check_node() {
    ...
    # Repair pre-existing Atlas-managed installs where `npm install -g` lands
    ...
    if [ -x "$ATLAS_HOME/node/bin/node" ] && node_satisfies_build "$("$ATLAS_HOME/node/bin/node" --version)"; then
        export PATH="$ATLAS_HOME/node/bin:$PATH"
        log_success "Node.js $("$ATLAS_HOME/node/bin/node" --version) found (Atlas-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Atlas-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.atlas/node/..."
    ...
    # Place into ~/.atlas/node/ and symlink binaries into the same bin dir
    # the atlas command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ATLAS_HOME/node"
    mkdir -p "$ATLAS_HOME"
    mv "$extracted_dir" "$ATLAS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ATLAS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ATLAS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ATLAS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ATLAS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.atlas/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Atlas install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Atlas Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `atlas update` path
                # (#4735).
                ...
                local stash_name
                stash_name="atlas-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``atlas update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Atlas behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up atlas command..."

    if [ "$USE_VENV" = true ]; then
        ATLAS_BIN="$INSTALL_DIR/venv/bin/atlas"
    else
        ATLAS_BIN="$(which atlas 2>/dev/null || echo "")"
        if [ -z "$ATLAS_BIN" ]; then
            log_warn "atlas not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ATLAS_BIN" ]; then
        log_warn "atlas entry point not found at $ATLAS_BIN"
    ...
    # Create a user-facing shim for the atlas command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ATLAS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ATLAS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/atlas"
    cat > "$command_link_dir/atlas" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATLAS_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/atlas"
    log_success "Installed atlas launcher → $command_link_display_dir/atlas"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "atlas command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v atlas' \
                >/dev/null 2>&1; then
        ...
        log_info "atlas not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Atlas Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Atlas Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Atlas Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so atlas works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.atlas directory structure (config at top level, code in subdir)
    mkdir -p "$ATLAS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.atlas/.env (top level, easy to find)
    if [ ! -f "$ATLAS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ATLAS_HOME/.env"
            log_success "Created ~/.atlas/.env from template"
        else
            touch "$ATLAS_HOME/.env"
            log_success "Created ~/.atlas/.env"
        fi
    else
        log_info "~/.atlas/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.atlas/config.yaml (top level, easy to find)
    if [ ! -f "$ATLAS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ATLAS_HOME/config.yaml"
            log_success "Created ~/.atlas/config.yaml from template"
        fi
    else
        log_info "~/.atlas/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in atlas_cli/default_soul.py — the
    ...
    if [ ! -f "$ATLAS_HOME/SOUL.md" ]; then
        cat > "$ATLAS_HOME/SOUL.md" << 'SOUL_EOF'
You are Atlas Agent, an intelligent AI assistant created by Usama Aslam. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.atlas/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.atlas/"

    # Seed bundled skills into ~/.atlas/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `atlas update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'atlas update'." \
            > "$ATLAS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ATLAS_HOME/.no-bundled-skills"
        log_info "  Future 'atlas update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.atlas/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.atlas/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ATLAS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ATLAS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.atlas/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Atlas Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Atlas will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Atlas Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (atlas --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `atlas update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'atlas setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m atlas_cli.main setup < /dev/tty
    else
        python -m atlas_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Atlas to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ATLAS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'atlas whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ATLAS_CMD="$(get_atlas_command_path)"
                $ATLAS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'atlas whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'atlas gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ATLAS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ATLAS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: atlas gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: atlas gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ATLAS_CMD gateway > "$ATLAS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.atlas/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: atlas gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: atlas gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ATLAS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ATLAS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ATLAS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}atlas${NC}              Start chatting"
    echo -e "   ${GREEN}atlas setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}atlas config${NC}       View/edit configuration"
    echo -e "   ${GREEN}atlas config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}atlas gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}atlas update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'atlas' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'atlas' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'atlas' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ATLAS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ATLAS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ATLAS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ATLAS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ATLAS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Atlas for pip install"
    ...
    ATLAS_CMD="$(command -v atlas 2>/dev/null || echo "")"
    if [ -n "$ATLAS_CMD" ]; then
        log_info "Running atlas setup..."
        "$ATLAS_CMD" setup
    else
        log_warn "atlas command not found on PATH"
        log_info "Try: python -m atlas_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Atlas` rename
...
# _purge_electron_build_cache() used by `atlas desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Atlas-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `atlas desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Atlas" ]; then
            app="$desktop_dir/release/linux-unpacked/Atlas"
        elif [ -x "$desktop_dir/release/linux-unpacked/atlas" ]; then
            app="$desktop_dir/release/linux-unpacked/atlas"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Atlas.app" \
            "$desktop_dir/release/mac/Atlas.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Atlas is damaged and can't be opened").
    ...
    # checkout stays clean for the next `atlas update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Atlas-managed Node
            # provisioned during prerequisites/node-deps (at $ATLAS_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ATLAS_HOME.
    # $ATLAS_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'atlas update' on this host install.
