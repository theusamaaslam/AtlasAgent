#!/usr/bin/env python3
"""Live test harness for Atlas Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any atlas imports.
ORIGINAL_HOME = os.environ.get("ATLAS_HOME")
ORIGINAL_AUTH = Path.home() / ".atlas" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.atlas/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.atlas/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="atlas_ts_live_"))
    atlas_home = home_dir / ".atlas"
    atlas_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, atlas_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".atlas" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, atlas_home / ".env")
        ...
        load_atlas_dotenv(atlas_home=str(Path.home() / ".atlas"))
    ...
    (atlas_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return atlas_home
...
def reset_module_state():
    """Drop cached modules so the new ATLAS_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "atlas_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ATLAS_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ATLAS_HOME"] = ORIGINAL_HOME
    else:
