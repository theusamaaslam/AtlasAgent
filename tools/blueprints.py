...
`atlas skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.atlas.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.atlas.blueprint`` is a mapping containing
    ...
    atlas = meta.get("atlas") if isinstance(meta, dict) else None
    blueprint = atlas.get("blueprint") if isinstance(atlas, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.atlas.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.atlas.blueprint`` block) they can hand
    to ``atlas skills publish`` to share. ``body`` is the plain-language
