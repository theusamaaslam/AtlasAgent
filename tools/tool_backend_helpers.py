...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Atlas Tool Gateway.

    Entitlement is paid Atlas Gateway service access OR a live free tool pool
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    """Return account-aware guidance for an unavailable Atlas Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `atlas model` to refresh your "
        "Atlas Gateway login and billing status."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.atlas/.env`` (via
    ``atlas_cli.config.get_env_value`` when available) so tool-side
