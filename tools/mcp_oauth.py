...
This module provides the glue:
    - ``AtlasTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Atlas Agent"
"""
...
from atlas_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__atlas_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ATLAS_HOME so each profile gets its own OAuth tokens.
    Layout: ``ATLAS_HOME/mcp-tokens/``
    """
    try:
        from atlas_constants import get_atlas_home
        base = Path(get_atlas_home())
    except ImportError:
        base = Path(os.environ.get("ATLAS_HOME", str(Path.home() / ".atlas")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``atlas mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `atlas mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# AtlasTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AtlasTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ATLAS_HOME/mcp-tokens/<server_name>.json         -- tokens
        ATLAS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ATLAS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Atlas records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Atlas.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://atlas-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `atlas mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = AtlasTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Atlas Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`atlas mcp login {server_name}` interactively first to complete "
