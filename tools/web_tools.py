...
Backend is selected during ``atlas tools`` setup (web.backend in config.yaml).
When available, Atlas can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Atlas config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Atlas' config/.env layer (``atlas config set``, ``atlas tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.atlas/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``atlas tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `atlas tools` repaint.
        try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_atlas_dir("cache/web", "web_cache")
