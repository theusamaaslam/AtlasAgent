...
Lets the LLM write a Python script that calls Atlas tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `atlas_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `atlas_tools.py` with file-based RPC stubs
...
# available on every platform Atlas itself runs on.
...
# match a safe prefix, the operational ATLAS_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "ATLAS_" prefix was deliberately removed (#27303) — it leaked
# ATLAS_*-named config that lacks a secret substring (e.g. ATLAS_BASE_URL,
# ATLAS_KANBAN_DB, ATLAS_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ATLAS_CHILD_ALLOWED below; ATLAS_RPC_SOCKET /
# ATLAS_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ATLAS_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set atlas_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ATLAS_* vars (_ATLAS_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret ATLAS_* vars dropped by the tightened allowlist (#27303). The
    # broad "ATLAS_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ATLAS_KANBAN_DB / ATLAS_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("ATLAS_"):
            ...
            # allowlist — a deliberately-dropped ATLAS_* var.
            _dropped_atlas.append(k)
    if _dropped_atlas:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ATLAS_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Atlas-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Atlas-profile soft guard."""',
...
def generate_atlas_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the atlas_tools.py stub module.
...
"""Auto-generated Atlas tools RPC stubs."""
...
def _connect():
    ...
    ATLAS_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ATLAS_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("ATLAS_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Atlas tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ATLAS_RPC_DIR") or os.path.join(tempfile.gettempdir(), "atlas_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated atlas_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/atlas_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_atlas_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/atlas_tools.py", tools_src)
        ...
        env_prefix = (
            f"ATLAS_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"ATLAS_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ATLAS_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with atlas_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="atlas_sandbox_")
    ...
    # selector from ATLAS_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"atlas_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated atlas_tools module.
        ...
        tools_src = generate_atlas_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "atlas_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ATLAS_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ATLAS_RPC_SOCKET"] = rpc_endpoint
        child_env["ATLAS_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the atlas-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from atlas_tools import ...`` resolves even
        ...
        _pp_parts = [tmpdir, _atlas_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ATLAS_TIMEZONE is an internal Atlas setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ATLAS_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ATLAS_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.atlas/.env')).
...
def _load_config() -> dict:
    ...
    ``atlas --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (atlas-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``atlas tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and atlas-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.atlas/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Atlas tools programmatically. "
        ...
        f"Available via `from atlas_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into atlas_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from atlas_tools import {import_str}` "
