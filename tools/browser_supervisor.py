...
One ``CDPSupervisor`` runs per Atlas ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Atlas logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "atlas-dialog-bridge.invalid"
...
(() => {
  if (window.__atlasDialogBridgeInstalled) return;
  window.__atlasDialogBridgeInstalled = true;
  const ENDPOINT = "http://atlas-dialog-bridge.invalid/";
