#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Atlas Skills Hub.
...
Used by atlas_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _atlas_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            atlas_meta = metadata.get("atlas", {})
            if isinstance(atlas_meta, dict):
                tags = atlas_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Usama Aslam) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                atlas_meta = meta_block.get("atlas", {})
                if isinstance(atlas_meta, dict):
                    tags = atlas_meta.get("tags", [])
...
def _atlas_index_cache_file() -> Path:
...
def _load_atlas_index() -> Optional[dict]:
    ...
    if atlas_index_cache_file.exists():
        try:
            age = time.time() - atlas_index_cache_file.stat().st_mtime
            if age < ATLAS_INDEX_TTL:
                return json.loads(atlas_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        atlas_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        atlas_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class AtlasIndexSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "atlas-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "atlas-index"),
            identifier=entry.get("identifier", ""),
