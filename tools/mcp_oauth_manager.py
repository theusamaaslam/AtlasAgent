...
`mcp_tool.py`, and `atlas_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AtlasMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_atlas_provider_class() -> Optional[type]:
    ...
    class AtlasMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._atlas_server_name = server_name
            ...
            self._atlas_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AtlasTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AtlasTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._atlas_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AtlasTokenStorage
                        if isinstance(storage, AtlasTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._atlas_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AtlasTokenStorage
            if not isinstance(storage, AtlasTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``atlas mcp reauth``.
            ...
            back to ``atlas mcp reauth``.
            """
            try:
                if self._atlas_preregistered:
                    return
    ...
    return AtlasMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ATLAS_PROVIDER_CLS: Optional[type] = _make_atlas_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AtlasMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `atlas mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``atlas mcp remove <name>`` and (indirectly) by
        ``atlas mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
