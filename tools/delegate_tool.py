...
# Must match atlas_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``atlas-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Atlas Gateway).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. atlas-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Atlas Gateway).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.atlas/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_atlas_dir("cache/delegation", "delegation_cache")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'atlas auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (atlas_cli/config.py load_config()) so that
