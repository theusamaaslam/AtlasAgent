#!/usr/bin/env python3
"""
Atlas CLI - Interactive Terminal Interface
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    *dead*-locked one (owning pid gone — a crashed session) is unlocked first
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("atlas-"):
            continue
...
# ASCII Art - Atlas Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "Atlas - AI Assistant"
        tiny_line = "Atlas"
    else:
        agent_name = _skin.get_branding("agent_name", "Atlas") if _skin else "Atlas"
    ...
    if os.environ.get("ATLAS_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Atlas v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Usama Aslam[/]\n"
...
class AtlasCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    ...
    def show_banner(self):
        ...
        if is_nous_atlas_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Usama Aslam Atlas 3 & 4 models are NOT agentic and are not "
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors ATLAS_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            print()
            _cprint(f"  💳 {_d('Not logged into Atlas Gateway.')}")
            print("  Run `atlas portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Atlas Gateway.')}")
                print("  Run `atlas portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Usama Aslam to charge your card."
        )
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from atlas_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `atlas portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Usama Aslam to charge {card.masked} "
    ...
    def run(self):
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from atlas_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Atlas")
...
def main(
    ...
    """
    Atlas CLI - Interactive AI Assistant
