...
metadata:
  atlas:
    ...
    related_skills: [codex, claude-code, atlas-agent]
---

# Grok Build CLI — Atlas Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Atlas terminal. Grok
...
1. Prepare stable input files first with Atlas tools (`read_file`,
...
2. **Don't conflate Atlas' xAI auth with the `grok` CLI's auth.** Atlas'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Atlas' xAI auth carries
   over.
