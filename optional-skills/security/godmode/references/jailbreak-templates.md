...
## 5. GODMODE FAST — Atlas 4 405B (Zero Refusal)

**Model:** `nousresearch/atlas-4-405b`
**Strategy:** Atlas models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Atlas Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.atlas/config.yaml`:
