...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ATLAS_HOME", os.path.expanduser("~/.atlas")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ATLAS_HOME", Path.home() / ".atlas")) / "skills" / "red-teaming" / "godmode"
...
ATLAS_HOME = Path(os.getenv("ATLAS_HOME", Path.home() / ".atlas"))
CONFIG_PATH = ATLAS_HOME / "config.yaml"
PREFILL_PATH = ATLAS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Atlas models — already uncensored, just needs clean prompt
    "atlas": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "atlas" in model_lower or "nous" in model_lower:
        return "atlas"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Atlas config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.atlas/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Atlas for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Atlas for changes to take effect.")
