...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Atlas as an MCP server",
        description=(
            "Manage MCP server connections and run Atlas as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'atlas mcp add' to connect to a new server, or\n"
            "'atlas mcp serve' to expose Atlas conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Atlas as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `atlas mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Atlas-approved MCPs shipped with the repo) ────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `atlas mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Atlas-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `atlas mcp install n8n`)",
    )
