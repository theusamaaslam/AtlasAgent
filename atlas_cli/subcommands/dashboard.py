"""``atlas dashboard`` / ``atlas serve`` subcommand parsers.
...
``atlas_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `atlas dashboard` / `atlas serve` cmdlines and SIGTERM them directly —
    # the same path `atlas update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Atlas web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Atlas web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Atlas Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Atlas desktop app shells (<= 0.15.x) spawn the
    # backend as `atlas dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Atlas couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Atlas Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Atlas backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Atlas backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `atlas dashboard register` — register a self-hosted dashboard OAuth
    # client with Atlas Gateway and write the client_id into ~/.atlas/.env.
    # Nested subparser so bare `atlas dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Removed hosted dashboard OAuth registration command",
        description=(
            "Hosted dashboard OAuth registration has been removed from this "
            "Atlas build. Use username/password dashboard auth instead."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://atlas.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
