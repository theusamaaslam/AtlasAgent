...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Atlas Agent. Use 'atlas debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    atlas debug share              Upload debug report (asks for confirmation)
    atlas debug share --yes        Skip confirmation (for scripts/CI)
    atlas debug share --lines 500  Include more log lines
    atlas debug share --expire 30  Keep paste for 30 days
    atlas debug share --local      Print report locally (no upload)
    atlas debug share --no-redact  Disable upload-time secret redaction
    atlas debug share --nous       Upload to private Atlas diagnostics (private)
    atlas debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to private Atlas diagnostics storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Atlas support (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'atlas debug share'",
    )
