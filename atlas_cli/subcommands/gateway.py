"""``atlas gateway`` and ``atlas proxy`` subcommand parsers.

Extracted verbatim from ``atlas_cli/main.py:main()`` (god-file Phase 2).
...
from atlas_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ATLAS_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy atlas.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Atlas gateway unit files "
            "(e.g. atlas.service) left over from older installs. Profile "
            "units (atlas-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Atlas Gateway account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.atlas/.env. "
            "Requires being logged in (atlas setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.atlas/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="xai",
        help="Upstream provider: xai (default: xai). See `atlas proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
