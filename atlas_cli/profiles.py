"""
Profile management for multiple isolated Atlas instances.

Each profile is a fully independent ATLAS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.atlas/profiles/<name>/`` by default.

The "default" profile is ``~/.atlas`` itself — backward compatible,
zero migration needed.

Usage::

    atlas profile create coder          # fresh profile + bundled skills
    atlas profile create coder --clone  # also copy config, .env, SOUL.md, skills
    atlas profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    atlas -p coder chat                 # or via flag
    atlas profile use coder             # set as sticky default
    atlas profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See atlas_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `atlas profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `atlas
...
# `atlas skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Atlas infrastructure directories that only the default profile
         (``~/.atlas``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_atlas_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.atlas) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "atlas-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "atlas", "default", "test", "tmp", "root", "sudo",
})

# Atlas subcommands that cannot be used as profile names/aliases
_ATLAS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the atlas root, NOT to the current ATLAS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ATLAS_HOME points outside
    ``~/.atlas``, profiles live under ``ATLAS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_atlas_home() / "profiles"


def _get_default_atlas_home() -> Path:
    """Return the default (pre-profile) ATLAS_HOME path.

    In standard deployments this is ``~/.atlas``.
    In Docker/custom deployments where ATLAS_HOME is outside ``~/.atlas``
    (e.g. ``/opt/data``), returns ATLAS_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_atlas_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Atlas installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ATLAS_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_atlas_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, atlas subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ATLAS_SUBCOMMANDS:
        return f"'{canon}' conflicts with a atlas subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            ...
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(atlas_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Atlas. If we
    ...
    try:
        ...
        # not be migrated. The next `atlas doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``atlas -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``atlas profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``atlas -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``atlas profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ATLAS_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Atlas config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_atlas_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, atlas_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ATLAS_HOME.
    ...
    The returned ``atlas_home`` is the path to pass to
    ``set_atlas_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_atlas_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``atlas update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.atlas)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.atlas/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `atlas -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Atlas profile.\n"
    ...
    if not soul_path.exists():
        try:
            from atlas_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `atlas update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`atlas profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Atlas (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `atlas profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `atlas -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ATLAS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``atlas profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ATLAS_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_atlas_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Atlas *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Atlas backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ATLAS_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `atlas -p <canon> profile
    ...
    atlas_markers = ("atlas_cli.main", "atlas-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Atlas process: either an entrypoint marker in argv, or
            # a resolved executable named `atlas`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_atlas = (
                any(marker in joined for marker in atlas_markers)
                or exe_name == "atlas"
                or exe_name.startswith("atlas")
            )
            if not is_atlas:
                continue
            ...
            # ...or by ATLAS_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ATLAS_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.atlas).\n"
            "To remove everything, use: atlas uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_atlas_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ATLAS_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from atlas_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ATLAS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ATLAS_HOME")
    try:
        os.environ["ATLAS_HOME"] = str(profile_dir)
        from atlas_cli.gateway import get_service_name, get_launchd_plist_path
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.atlas/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: atlas profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ATLAS_HOME.

    Returns ``"default"`` if ATLAS_HOME is not set or points to ``~/.atlas``.
    Returns the profile name if ATLAS_HOME points into ``~/.atlas/profiles/<name>``.
    Returns ``"custom"`` if ATLAS_HOME is set to an unrecognized path.
    """
    from atlas_constants import get_atlas_home
    atlas_home = get_atlas_home()
    resolved = atlas_home.resolve()

    default_resolved = _get_default_atlas_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.atlas itself — its parent is ~/ and its
        # directory name is ".atlas", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: atlas profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.atlas itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.atlas). "
            "Specify a different name: atlas profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="atlas_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"atlas_{old_name}"
    legacy_old_host = f"atlas.{old_name}"
    new_host = f"atlas_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_atlas_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("atlas_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ATLAS_HOME path string.

    Called early in the CLI entry point, before any atlas modules
    are imported, to set the ATLAS_HOME environment variable.
    """
