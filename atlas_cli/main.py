...
Usage:
    atlas                     # Interactive chat (default)
    atlas chat                # Interactive chat
    atlas gateway             # Run gateway in foreground
    atlas gateway start       # Start gateway as service
    atlas gateway stop        # Stop gateway service
    atlas gateway status      # Show gateway status
    atlas gateway install     # Install gateway service
    atlas gateway uninstall   # Uninstall gateway service
    atlas setup               # Interactive setup wizard
    atlas logout              # Clear stored authentication
    atlas status              # Show status of all components
    atlas cron                # Manage cron jobs
    atlas cron list           # List cron jobs
    atlas cron status         # Check if cron scheduler is running
    atlas doctor              # Check configuration and dependencies
    atlas honcho setup                    # Configure Honcho AI memory integration
    atlas honcho status                   # Show Honcho config and connection status
    atlas honcho sessions                 # List directory → session name mappings
    atlas honcho map <name>               # Map current directory to a session name
    atlas honcho peer                     # Show peer names and dialectic settings
    atlas honcho peer --user NAME         # Set user peer name
    atlas honcho peer --ai NAME           # Set AI peer name
    atlas honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    atlas honcho mode                     # Show current memory mode
    atlas honcho mode [hybrid|honcho|local]  # Set memory mode
    atlas honcho tokens                   # Show token budget settings
    atlas honcho tokens --context N       # Set session.context() token cap
    atlas honcho tokens --dialectic N     # Set dialectic result char cap
    atlas honcho identity                 # Show AI peer identity representation
    atlas honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    atlas honcho migrate                  # Step-by-step migration guide: OpenClaw native → Atlas + Honcho
    atlas version             Show version
    atlas update              Update to latest version
    atlas uninstall           Uninstall Atlas Agent
    atlas acp                 Run as an ACP server for editor integration
    atlas sessions browse     Interactive session picker with search

    atlas claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: atlas_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``atlas_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``atlas update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``atlas_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, atlas crashes on import and the user can't run
# ``atlas update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'atlas' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``atlas tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``atlas.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("atlas")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"atlas", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"atlas")
        # Windows: the .exe name is already ``atlas.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* atlas_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".atlas", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ATLAS_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
...
# `entry.tsx`; this is just the earlier cousin. ``ATLAS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ATLAS_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`atlas --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from atlas_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Atlas Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``atlas --version`` before config/logging imports on Termux."""
    if os.environ.get("ATLAS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from atlas_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from atlas_cli.subcommands.cron import build_cron_parser
from atlas_cli.subcommands.gateway import build_gateway_parser
from atlas_cli.subcommands.profile import build_profile_parser
from atlas_cli.subcommands.model import build_model_parser
from atlas_cli.subcommands.setup import build_setup_parser
from atlas_cli.subcommands.postinstall import build_postinstall_parser
from atlas_cli.subcommands.whatsapp import build_whatsapp_parser
from atlas_cli.subcommands.slack import build_slack_parser
from atlas_cli.subcommands.login import build_login_parser
from atlas_cli.subcommands.logout import build_logout_parser
from atlas_cli.subcommands.auth import build_auth_parser
from atlas_cli.subcommands.status import build_status_parser
from atlas_cli.subcommands.webhook import build_webhook_parser
from atlas_cli.subcommands.hooks import build_hooks_parser
from atlas_cli.subcommands.doctor import build_doctor_parser
from atlas_cli.subcommands.security import build_security_parser
from atlas_cli.subcommands.dump import build_dump_parser
from atlas_cli.subcommands.debug import build_debug_parser
from atlas_cli.subcommands.backup import build_backup_parser
from atlas_cli.subcommands.import_cmd import build_import_cmd_parser
from atlas_cli.subcommands.config import build_config_parser
from atlas_cli.subcommands.console import build_console_parser
from atlas_cli.subcommands.version import build_version_parser
from atlas_cli.subcommands.update import build_update_parser
from atlas_cli.subcommands.uninstall import build_uninstall_parser
from atlas_cli.subcommands.dashboard import build_dashboard_parser
from atlas_cli.subcommands.gui import build_gui_parser
from atlas_cli.subcommands.logs import build_logs_parser
from atlas_cli.subcommands.prompt_size import build_prompt_size_parser
from atlas_cli.subcommands.memory import build_memory_parser
from atlas_cli.subcommands.acp import build_acp_parser
from atlas_cli.subcommands.tools import build_tools_parser
from atlas_cli.subcommands.insights import build_insights_parser
from atlas_cli.subcommands.skills import build_skills_parser
from atlas_cli.subcommands.pairing import build_pairing_parser
from atlas_cli.subcommands.plugins import build_plugins_parser
from atlas_cli.subcommands.mcp import build_mcp_parser
from atlas_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (atlas tools, atlas setup, atlas model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'atlas {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any atlas module import.
#
# Many modules cache ATLAS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ATLAS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.atlas/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ATLAS_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `atlas mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Atlas' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo atlas -p <name>` against the invoking user's home.
        ...
        candidate = home / ".atlas" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`atlas chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors atlas_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ATLAS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.atlas/profiles/coder or
    # /opt/data/profiles/coder).  If ATLAS_HOME points to the atlas root
    # instead (e.g. systemd hardcodes ATLAS_HOME=/root/.atlas), we must
    # still read active_profile — the user may have switched profiles via
    # `atlas profile use` and the gateway should honour that choice.
    # See issue #22502.
    atlas_home_env = os.environ.get("ATLAS_HOME", "")
    if profile_name is None and atlas_home_env:
        if Path(atlas_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the atlas root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ATLAS_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``atlas gateway run`` to mean
    # "the root ATLAS_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("ATLAS_S6_SUPERVISED_CHILD"):
        try:
            from atlas_constants import get_default_atlas_root

            active_path = get_default_atlas_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ATLAS_HOME
    if profile_name is not None:
        try:
            from atlas_cli.profiles import resolve_profile_env

            atlas_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            atlas_home = _resolve_sudo_user_profile_env(profile_name)
            if not atlas_home:
            ...
            # A bug in profiles.py must NEVER prevent atlas from starting
            print(
...
load_atlas_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ATLAS_REDACT_SECRETS env
# var BEFORE atlas_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_atlas_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "ATLAS_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ATLAS_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `atlas` subcommands
...
try:
    from atlas_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from atlas_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if atlas_constants not importable yet
...
from atlas_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to atlas_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (atlas_cli.main._model_flow_*) keep resolving unchanged.
from atlas_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_atlas_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ATLAS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ATLAS_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from atlas_cli.config import get_env_path, get_atlas_home, load_config
    from atlas_cli.auth import get_auth_status

    # Determine whether Atlas itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_atlas_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Atlas Gateway OAuth credentials
    auth_file = get_atlas_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Atlas has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Atlas to use their tokens.
    if _has_atlas_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, atlas_bin
        cli_args: the original CLI arguments (everything after 'atlas')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    atlas_bin = container_info["atlas_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo atlas {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo atlas {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  atlas --tui --resume {target}")
    if title:
        print(f'  atlas --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @atlas/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@atlas" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/atlas-ink/package.json",
    "packages/atlas-ink/index.js",
    "packages/atlas-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ATLAS_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ATLAS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ATLAS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ATLAS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    atlas_home = os.environ.get("ATLAS_HOME") or str(Path.home() / ".atlas")
    try:
    ...
    extras.extend([Path(atlas_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(atlas_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if atlas_cli_dir is None:
        atlas_cli_dir = Path(__file__).parent
    bundled = atlas_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``atlas update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ATLAS_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Atlas checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `atlas update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Atlas checkout, run `git restore -- ui-tui`\n"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ATLAS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ATLAS_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from atlas_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ATLAS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ATLAS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ATLAS_TUI_DIR (e.g. `unset ATLAS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @atlas/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @atlas/ink resolves through
        # packages/atlas-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "atlas-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from atlas_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from atlas_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="atlas-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ATLAS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ATLAS_PYTHON_SRC_ROOT"] = os.environ.get(
        "ATLAS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ATLAS_PYTHON", sys.executable)
    env.setdefault("ATLAS_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ATLAS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ATLAS_MODEL"] = model
        env["ATLAS_INFERENCE_MODEL"] = model
    if provider:
        env["ATLAS_TUI_PROVIDER"] = provider
        env["ATLAS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ATLAS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ATLAS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ATLAS_TUI_SKILLS"] = value
    if query:
        env["ATLAS_TUI_QUERY"] = query
    if image:
        env["ATLAS_TUI_IMAGE"] = image
    if checkpoints:
        env["ATLAS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ATLAS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ATLAS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ATLAS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ATLAS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ATLAS_ACCEPT_HOOKS"] = "1"
    ...
    # ATLAS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `atlas --tui` try to
    ...
    # still set ATLAS_TUI_RESUME themselves.
    env.pop("ATLAS_TUI_RESUME", None)
    if resume_session_id:
        env["ATLAS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `atlas update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ATLAS_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``atlas kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``atlas kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ATLAS_KANBAN_BOARD"):
        return
    try:
        from atlas_cli.kanban_db import get_current_board

        os.environ["ATLAS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.atlas/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Atlas — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Atlas still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ATLAS_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ATLAS_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'atlas sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from atlas_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from atlas_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'atlas doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Atlas isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  atlas setup")
        print()

        from atlas_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'atlas setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from atlas_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Atlas itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (ATLAS_SAFE_MODE is checked by
    # atlas_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["ATLAS_SAFE_MODE"] = "1"
        os.environ["ATLAS_IGNORE_USER_CONFIG"] = "1"
        os.environ["ATLAS_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.atlas/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["ATLAS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from atlas_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from atlas_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from atlas_constants import find_node_executable, with_atlas_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Atlas?")
        print()
    ...
    # has no creds.json.  Every subsequent `atlas gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: atlas gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `atlas gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  atlas gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Atlas Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  atlas gateway")
        ...
        print()
        print("  Or install as a service: atlas gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'atlas whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``atlas whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``atlas_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from atlas_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from atlas_cli.config import stamp_install_method
    from atlas_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Atlas post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``atlas model``) and the setup wizard
    ...
    from atlas_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ATLAS_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from atlas_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'atlas model' for "
                "available providers, or run 'atlas doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in atlas_cli/models.py). A multi-member group shows one
    ...
    if selected_provider == "openrouter":
        ...
        print("The hosted Nous gateway has been removed from this Atlas build.")
        print("Use `atlas model` to configure a direct provider or a custom endpoint.")
        return
    ...
    # a leftover OPENAI_BASE_URL in ~/.atlas/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.atlas/.env if the active provider is not 'custom'.
    ...
    """
    from atlas_cli.config import get_env_value, save_env_value, load_config
...
#
# Atlas uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `atlas model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `atlas model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`atlas_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Atlas only falls back to a lightweight backend (OpenRouter,")
        print("  Atlas Gateway) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from atlas_cli.config import load_config
    from atlas_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from atlas_cli.auth import _prompt_model_selection
    from atlas_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from atlas_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from atlas_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from atlas_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Atlas URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from atlas_cli.curses_ui import curses_radiolist
...
# downstream call sites read `atlas_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `atlas` invocation — including
# fast paths like `atlas --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from atlas_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``atlas setup`` / ``atlas model``.
    ...
    recover from a malformed paste without editing ``~/.atlas/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from atlas_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from atlas_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `atlas setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from atlas_constants import display_atlas_home as _dhh_fn

            print(
                f"    Atlas will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Atlas CLI with a provider."""
    from atlas_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from atlas_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from atlas_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from atlas_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from atlas_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from atlas_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``atlas slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: atlas slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `atlas slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from atlas_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from atlas_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from atlas_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from atlas_cli.hooks import hooks_command
...
def cmd_security(args):
    """Dispatch `atlas security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from atlas_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from atlas_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from atlas_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from atlas_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Atlas home directory to a zip file."""
    if getattr(args, "quick", False):
        from atlas_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from atlas_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Atlas backup from a zip file."""
    from atlas_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from atlas_cli.banner import format_banner_version_label
    ...
    try:
        from atlas_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Atlas Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from atlas_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from atlas_cli.uninstall import run_gui_uninstall
    ...
    from atlas_cli.uninstall import run_uninstall
...
# Critical files that every ``atlas`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``atlas update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``atlas`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="atlas-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``atlas update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``atlas_cli/web_dist/`` (per vite.config.ts
    outDir: "../atlas_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "atlas_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``atlas`` launcher present but ``atlas_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the atlas venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — atlas venv python3, no nix-shell overhead
    ...
    # a atlas venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``atlas update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``atlas web``).
    ...
    # build path usable on a stock `py -m atlas_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_atlas_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (atlas web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``atlas update`` can unconditionally call ``atlas desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``atlas desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ATLAS_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ATLAS_HOME."""
    from atlas_constants import get_atlas_home
    return get_atlas_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``atlas update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Atlas.app/Contents/MacOS/Atlas"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Atlas.exe",
            release_dir / "win-ia32-unpacked" / "Atlas.exe",
            release_dir / "win-arm64-unpacked" / "Atlas.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "atlas",
            release_dir / "linux-unpacked" / "Atlas",
            release_dir / "linux-arm64-unpacked" / "atlas",
            release_dir / "linux-arm64-unpacked" / "Atlas",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Atlas'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_atlas_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Atlas.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Atlas.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Atlas.exe: Access is
    ...
    an unrelated "Atlas" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Atlas is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Atlas.app/Contents/MacOS/Atlas  ->  app bundle = .../Atlas.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``atlas desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Atlas Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Atlas Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the ATLAS_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from atlas_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["ATLAS_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ATLAS_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "atlas_root", None):
        env["ATLAS_DESKTOP_ATLAS_ROOT"] = str(Path(args.atlas_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ATLAS_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    # `ATLAS_DESKTOP_DISABLE_GPU=... atlas desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "ATLAS_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ATLAS_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  atlas gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Atlas.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Atlas.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Atlas.exe, close any")
                    print("  running Atlas desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> atlas desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Atlas is
    ...
    if source_mode:
        print("→ Launching Atlas Desktop from source build...")
    ...
    print(f"→ Launching packaged Atlas Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``atlas dashboard`` processes other than ourselves.

    ``atlas dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``atlas update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Atlas Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``atlas serve`` as
    ...
    environment variable ``ATLAS_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "atlas dashboard",
        "atlas_cli.main dashboard",
        "atlas_cli/main.py dashboard",
        # The headless backend (`atlas serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "atlas serve",
        "atlas_cli.main serve",
        "atlas_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "atlas.*dashboard"`) keeps us consistent
            # with `atlas_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `atlas update`.
    ...
    print("  Preview now:  atlas curator run --dry-run")
    print("  Pause it:     atlas curator pause")
    print(
        "  Docs:         https://atlas.local/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``atlas update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``atlas update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: atlas curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``atlas dashboard`` processes.

    Called at the end of ``atlas update`` (default ``reason``) and also
    from ``atlas dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Atlas Desktop Electron app spawns this dashboard as a
    # backend child, it sets ATLAS_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ATLAS_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    atlas dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.atlas-update-staging"
    backup = f"{dst}.atlas-update-old"
...
def _update_via_zip(args):
    """Update Atlas Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `atlas update "
            f"--branch {branch}`, or update against main with `atlas update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="atlas-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"atlas-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for atlas-agent itself
        ...
        # GitHub ZIPs extract to atlas-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"atlas-agent-{branch}")
    ...
    from atlas_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: atlas skills list-modified  "
    ...
    try:
        from atlas_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "atlas-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Atlas behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes atlas completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Atlas couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Atlas couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Atlas behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Atlas couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Atlas couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `atlas update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_atlas_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_atlas_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``atlas update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from atlas_constants import get_default_atlas_root

    default_home = get_default_atlas_root()
...
# Install-scoped breadcrumb dropped right before ``atlas update`` mutates the
...
# survives and the next ``atlas`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ATLAS_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``atlas update`` left half-done.
    ...
    protocol stream (``atlas acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if atlas.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _atlas_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Atlas is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Atlas from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `atlas update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from atlas_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``atlas update`` is still progressing even if pip/uv itself is silent.
    """
...
def _atlas_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_atlas_instances(
    ...
    The Atlas Desktop Electron app spawns ``atlas.EXE`` as a backend child,
    so during ``atlas update`` the user-invoked process and the desktop's
    ...
    shims (``atlas.exe`` / ``atlas-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``atlas update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``atlas.exe``) is a separate process from the Python
    ...
    for shim in _atlas_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated atlas.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``atlas update``
    ...
    #      atlas.exe sitting *under* a non-Atlas parent (e.g. a Atlas
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "atlas.exe"
    lines = ["✗ Another atlas.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Atlas Desktop, exit any open `atlas` REPLs, and")
    lines.append("  stop the gateway (`atlas gateway stop`) before retrying.")
    lines.append("")
    if matches:
...
def _quarantine_running_atlas_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``atlas.exe``.
    ...
    when ``atlas update`` runs, ``atlas.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``atlas.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next atlas invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Atlas Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Atlas Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _atlas_exe_shims(scripts_dir):
        ...
        print(
            "    Close Atlas Desktop, exit other `atlas` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `atlas update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_atlas_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``atlas.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``atlas.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``atlas`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_atlas_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``atlas.exe.old.*`` left by prior updates.

    Called early on every atlas invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``atlas update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `atlas update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``atlas.exe`` / ``atlas-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_atlas_exe`` for the rationale.
    """
    ...
    # hours later inside ``atlas update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``atlas.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``atlas.exe`` shim is locked during ``atlas update``, or when uv/distlib
    skips a launcher write. The symptom is ``atlas-agent.exe`` and
    ``atlas-acp.exe`` present but ``atlas.exe`` missing, so ``atlas`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `atlas update --force` after "
            "closing other atlas processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m atlas_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``atlas update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``atlas.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``atlas`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `atlas update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `atlas update --force` after closing other atlas processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from atlas_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ATLAS_HOME/bin/uv``, but on Termux the official
    ...
    """
    from atlas_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `atlas update`, so we install root-only first
    ...
    nixos_env = with_atlas_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `atlas update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``atlas update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.atlas/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``atlas update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``atlas update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.atlas/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``atlas update --gateway``) the update is already
    ...
    try:
        ...
        # atlas_cli.config.get_atlas_home to simulate setup failure.
        from atlas_cli.config import get_atlas_home as _get_atlas_home

        logs_dir = _get_atlas_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.atlas/logs/update.log`` only, never the terminal.

    During ``atlas update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``atlas update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from atlas_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``atlas update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from atlas_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from atlas_cli.config import recommended_update_command, running_from_source_tree
        if branch_explicit and branch != "main":
            print(f"⚠ --branch is ignored for local source installs (would have checked '{branch}').")
        if running_from_source_tree(PROJECT_ROOT):
            print("✓ Local source install detected.")
            print(f"  Refresh with: {recommended_update_command()}")
        else:
            print("✗ This pip install cannot be checked safely against the public package index.")
            print(f"  Reinstall from source instead: {recommended_update_command()}")
            sys.exit(1)
        return
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``atlas update`` without requiring a reinstall.  The
    ...
    ``atlas`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/atlas.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v atlas'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/atlas, code at /usr/local/lib/atlas-agent).
    fhs_link = Path("/usr/local/bin/atlas")
    ...
    path_comment = (
        "# Atlas Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ATLAS_HOME before running the update.
    ...
    ATLAS_HOME directories.  The ``--backup`` flag on ``atlas update``
    ...
    # ``atlas_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # ATLAS_HOME can add minutes to every update. Users who want the
    ...
    try:
        from atlas_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_atlas_home so the user sees ~/.atlas/...
    try:
        from atlas_constants import get_atlas_home, display_atlas_home

        home = get_atlas_home()
        try:
            display_path = f"{display_atlas_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  atlas import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``atlas update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``atlas update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run atlas from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.atlas-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".atlas-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The atlas.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m atlas_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``atlas
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Atlas processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Atlas Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Atlas desktop app / other Atlas terminals, then re-run:"
    )
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    atlas.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from atlas_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Atlas...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m atlas_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: atlas gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``atlas gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from atlas_cli import gateway_windows
        from atlas_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``atlas update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Atlas Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Atlas Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `atlas update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Refresh Atlas when running without a git checkout."""
    from atlas_cli import __version__
    from atlas_cli.config import running_from_source_tree, source_reinstall_command

    print(f"→ Current version: {__version__}")
    print("→ Refreshing local Atlas install...")

    if not running_from_source_tree(PROJECT_ROOT):
        print("✗ This install is not a git checkout or local source tree.")
        print("  Refusing to upgrade the public 'atlas-agent' package name because")
        print("  it may refer to an unrelated project.")
        print(f"  Reinstall from source instead: {source_reinstall_command(PROJECT_ROOT)}")
        sys.exit(1)
    ...
    print("✓ Update complete! Restart atlas to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Atlas Agent...")
    print()

    # On Windows, abort early if another atlas.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_atlas_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `atlas serve` backend) will keep .pyd
    ...
    # the atlas.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://atlas.local/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Atlas windows/gateways and re-run: atlas update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $ATLAS_HOME, this is cheap
        ...
        try:
            from atlas_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in atlas_cli/config.py bricked
        # every user who ran ``atlas update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``atlas
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``atlas update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_atlas_home added to atlas_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``atlas`` launch finishes the
        ...
        # build.  ``atlas desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``atlas desktop`` shouldn't be forced into a full
        # Electron build by ``atlas update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from atlas_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "atlas_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `atlas desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from atlas_constants import display_atlas_home as _dhh
        ...
        # current — so copy it straight over ~/.atlas/cache/model_catalog.json
        ...
        # modules in this process.  Reload atlas_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_atlas_home() added since the last release.
        try:
            import importlib
            import atlas_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ATLAS_HOME so
        # it is not affected by sync_skills()'s module-level ATLAS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ATLAS_HOME env var points at the default or a named profile.
        try:
            from atlas_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from atlas_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'atlas config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: atlas config migrate")
            else:
                print()
                print("Skipped. Run 'atlas config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from atlas_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `atlas curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``atlas update`` gives users a
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from atlas_cli.tools_config import install_cua_driver
        ...
        # When running as ``atlas update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_atlas_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... atlas-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... atlas-gateway*)
            ...
            # Discover all atlas-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. atlas-gateway.service or atlas-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `atlas update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: atlas gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: atlas gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: atlas -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Atlas gateway unit files are still installed.
        # When both atlas.service (from a pre-rename install) and the
        # current atlas-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `atlas update` surfaces the issue until the user migrates.
        try:
            from atlas_cli.gateway import (
                has_legacy_atlas_units,
                _find_legacy_atlas_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_atlas_units():
                print()
                print("⚠ Legacy Atlas gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_atlas_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (atlas.service) fight the current")
                print("  atlas-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    atlas gateway migrate-legacy")
                print()
        ...
        print("  atlas model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``atlas -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `atlas profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_atlas_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → atlas -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.atlas)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  atlas profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     atlas -p {name} chat")
                else:
        ...
        from atlas_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from atlas_constants import get_atlas_home as _hh
        ...
        from atlas_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `atlas profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → atlas -p {name}  ({wrapper})")
        print()
        ...
        from atlas_cli.profiles import profile_exists, validate_alias_name
        ...
        from atlas_cli.profiles import rename_profile
        ...
        from atlas_cli.profiles import export_profile
        ...
        from atlas_cli.profiles import import_profile
        ...
        import tempfile
        from atlas_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="atlas_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  atlas -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      atlas -p {plan.manifest.name} chat")
        ...
        from atlas_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `atlas profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"atlas -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from atlas_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("atlas_requires"):
            print(f"Requires:     Atlas {data['atlas_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from atlas_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.atlas_requires:
        print(f"  Requires: Atlas {mf.atlas_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``atlas dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``atlas dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No atlas dashboard processes running.")
        return 0

    print(f"{len(pids)} atlas dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``atlas dashboard register`` for OAuth.
    ...
    try:
        from atlas_cli.web_server import should_require_auth
    ...
    try:
        from atlas_cli.dashboard_auth import list_providers
        if list_providers():
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No atlas dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `atlas update`.
    ...
    # launches the dashboard (`worker dashboard` → ATLAS_HOME points into
    ...
    if (
        ...
        and os.environ.get("ATLAS_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "atlas_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # ATLAS_HOME.  We must resolve the root explicitly instead of just
        # dropping ATLAS_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ATLAS_HOME=/opt/data`), so an unset
        # ATLAS_HOME falls back to $HOME/.atlas = /opt/data/.atlas — an
        ...
        # update-button guard also misfires).  get_default_atlas_root()
        # returns the root for both layouts: ~/.atlas for a standard install
        ...
        try:
            from atlas_constants import get_default_atlas_root
            env["ATLAS_HOME"] = str(get_default_atlas_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ATLAS_HOME) rather than block the reroute.
    ...
    # the same logs directory as every other Atlas surface.
    try:
        from atlas_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "ATLAS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ATLAS_WEB_DIST"])
            if "ATLAS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "atlas_cli" / "web_dist"
        )
    ...
    from atlas_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with Atlas Gateway."""
    print(
        "Hosted dashboard OAuth registration has been removed from this Atlas build. "
        "Use username/password dashboard auth instead."
    )


def cmd_gateway_enroll(args):
    """Enroll a self-hosted gateway with a relay connector."""
    from atlas_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from atlas_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from atlas_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Atlas log files."""
    from atlas_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Atlas command console."""
    from atlas_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from atlas_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``atlas -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``atlas_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``atlas -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``atlas`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ATLAS_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from atlas_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from atlas_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ATLAS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ATLAS_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ATLAS_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `atlas --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_atlas_home()}/memories/\n"
            )
            return
...
def cmd_acp(args):
    """Launch Atlas Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from atlas_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from atlas_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from atlas_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from atlas_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from atlas_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from atlas_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from atlas_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from atlas_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from atlas_cli.claw import claw_command
...
def main():
    ...
    # Cosmetic: make the process show up as 'atlas' instead of 'python3.11'
    ...
    # Sweep stale ``atlas.exe.old.*`` quarantine files left by previous
    # ``atlas update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_atlas_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``atlas update``
    ...
    # ``atlas skills install update``) merely defers recovery one launch;
    # under-matching (missing ``atlas -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in atlas_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from atlas_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from atlas_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://atlas.local/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `atlas model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.atlas/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://atlas.local/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from atlas_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from atlas_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in atlas_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in atlas_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in atlas_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in atlas_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `atlas whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in atlas_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from atlas_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in atlas_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in atlas_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in atlas_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in atlas_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in atlas_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in atlas_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # portal command — Atlas Gateway status + Tool Gateway routing
    # =========================================================================
    from atlas_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from atlas_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from atlas_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in atlas_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in atlas_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in atlas_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in atlas_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in atlas_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in atlas_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.atlas/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo atlas uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from atlas_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in atlas_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in atlas_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in atlas_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in atlas_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in atlas_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from atlas_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in atlas_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``atlas --help``, ``atlas version``, ``atlas logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from atlas_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from atlas_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Atlas shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from atlas_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from atlas_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in atlas_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in atlas_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `atlas computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `atlas tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `atlas computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Atlas app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from atlas_cli.tools_config import _cua_driver_cmd
            # Honor ATLAS_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from atlas_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: atlas computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: atlas computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: atlas computer-use install --upgrade")
                return
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: atlas computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: atlas computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in atlas_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch atlas --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in atlas_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in atlas_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in atlas_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in atlas_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in atlas_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in atlas_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in atlas_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in atlas_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Atlas-Setup.exe success screen tells users to
    # run `atlas desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in atlas_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in atlas_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in atlas_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``atlas -c Pokemon Agent Dev`` → ``atlas -c 'Pokemon Agent Dev'``
    ...
    from atlas_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'atlas -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (atlas hooks list, cron
