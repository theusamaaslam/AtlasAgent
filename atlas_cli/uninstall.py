...
- Keep data: Remove code but keep ~/.atlas/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Atlas PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing atlas-agent or atlas PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Atlas Agent" comment and following line
                if '# Atlas Agent' in line or '# atlas-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('atlas' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing atlas
                if 'atlas' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the atlas wrapper script if it exists."""
    wrapper_paths = [
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains atlas_cli reference)
                content = wrapper.read_text()
                if 'atlas_cli' in content or 'atlas-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(atlas_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``atlas`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Atlas home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (atlas_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``atlas gateway run`` processes
    ...
    try:
        from atlas_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from atlas_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ATLAS_HOME`` and ``ATLAS_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\atlas\git\cmd``, ``%LOCALAPPDATA%\atlas\git\bin``,
#      ``%LOCALAPPDATA%\atlas\git\usr\bin``, ``%LOCALAPPDATA%\atlas\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\atlas\git\`` and Node to
#      ``%LOCALAPPDATA%\atlas\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``atlas dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\atlas\gateway-service\`` and sometimes
...
def _atlas_path_markers(atlas_home: Path) -> list[str]:
    """Path-entry substrings that identify Atlas-owned User-PATH entries."""
    root = str(atlas_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare atlas-agent install dir.
    markers = [root + "\\atlas-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ATLAS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "atlas".  We don't want to catch
    # unrelated entries like "catlas-foo" or "ephermeral", so we look for
    # backslash-atlas as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(atlas_home: Path) -> list[str]:
    """Strip Atlas-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _atlas_path_markers(atlas_home)
            kept: list[str] = []
...
def remove_atlas_env_vars_windows() -> list[str]:
    """Delete ATLAS_HOME and ATLAS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ATLAS_HOME", "ATLAS_GIT_BASH_PATH"):
                try:
...
def _is_default_atlas_home(atlas_home: Path) -> bool:
    """Return True when ``atlas_home`` points at the default (non-profile) root."""
    try:
        from atlas_constants import get_default_atlas_root
        return atlas_home.resolve() == get_default_atlas_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ATLAS_HOME directory.

    We shell out to ``atlas -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ATLAS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m atlas_cli.main` so we don't depend on a `atlas`
    #    wrapper that may be half-removed mid-uninstall.
    atlas_invocation = [_sys.executable, "-m", "atlas_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                atlas_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ATLAS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``atlas uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ATLAS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from atlas_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(atlas_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Atlas Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Atlas Chat GUI installation was found.")
        print(f"  Checked: {atlas_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Atlas agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(atlas_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Atlas agent at {atlas_home / 'atlas-agent'}")
        print(f"  • Your config, sessions, and secrets under {atlas_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Atlas agent is still installed. Run 'atlas' to use the CLI,")
    print("or 'atlas uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.atlas/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.atlas/ for future reinstall
    """
    project_root = get_project_root()
    atlas_home = get_atlas_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ATLAS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_atlas_home(atlas_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.atlas data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Atlas Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {atlas_home / 'config.yaml'}")
    print(f"  Secrets: {atlas_home / '.env'}")
    print(f"  Data:    {atlas_home / 'cron/'}, {atlas_home / 'sessions/'}, {atlas_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ATLAS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Atlas data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Atlas code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``atlas`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ATLAS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in atlas_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\atlas\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(atlas_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Atlas-owned PATH entries in User environment")

        log_info("Removing ATLAS_HOME / ATLAS_GIT_BASH_PATH User env vars...")
        removed_env = remove_atlas_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Atlas-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing atlas command...")
    ...
    #     (only when they still point into this Atlas home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Atlas-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(atlas_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Atlas-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(atlas_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ATLAS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from atlas_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(atlas_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.atlas/, just remove the atlas-agent subdir
            if atlas_home in project_root.parents or project_root.parent == atlas_home:
    ...
    #     under ATLAS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(atlas_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(atlas_home)
    ...
    # 5. Optionally remove ~/.atlas/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ATLAS_HOME dirs live under
        ...
        try:
            if atlas_home.exists():
                shutil.rmtree(atlas_home)
                log_success(f"Removed {atlas_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {atlas_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {atlas_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/UsamaAslam/atlas-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/UsamaAslam/atlas-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Atlas Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m atlas_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import atlas_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``atlas_constants`` + ``atlas_cli.colors``
    (and lazily ``atlas_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m atlas_cli.uninstall")
    parser.add_argument(
        "--mode",
