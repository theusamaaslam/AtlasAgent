...
Config files are stored in ~/.atlas/ for easy access:
- ~/.atlas/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.atlas/.env         - API keys and secrets

This module provides:
- atlas config          - Show current configuration
- atlas config edit     - Open config in editor
- atlas config set      - Set a specific value
- atlas config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``atlas config set`` (which rewrites
    ...
    ``config.yaml`` in place: atlas never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.atlas/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``atlas logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  atlas config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Atlas makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Atlas itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``atlas update``, the TUI build.
...
#   on every plugin install / ``atlas update``.
...
# * ``ATLAS_HOME`` / ``ATLAS_PROFILE`` / ``ATLAS_CONFIG`` /
#   ``ATLAS_ENV`` — Atlas runtime location flags. Writing these into
...
#
# IMPORTANT: ``ATLAS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ATLAS_LANGFUSE_PUBLIC_KEY,
# ATLAS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Atlas runtime location — never via dashboard env writer.
    # NOT a ATLAS_* blanket: integration credentials (ATLAS_GEMINI_*,
    # ATLAS_LANGFUSE_*, ATLAS_SPOTIFY_*, ...) ARE allowed.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Atlas runtime location "
            "(ATLAS_HOME, ATLAS_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ATLAS_TOOL_PROGRESS", "ATLAS_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `atlas plugins enable
    # observability/langfuse` or `atlas tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ATLAS_LANGFUSE_ENV",
    "ATLAS_LANGFUSE_RELEASE",
    "ATLAS_LANGFUSE_SAMPLE_RATE",
    "ATLAS_LANGFUSE_MAX_CHARS",
    "ATLAS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from atlas_cli.default_soul import (
    DEFAULT_SOUL_MD,
    is_legacy_template_soul,
    replace_legacy_creator_branding,
)
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ATLAS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_atlas_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Atlas is running in package-manager-managed mode.

    Two signals: the ATLAS_MANAGED env var (set by the systemd service),
    or a .managed marker file in ATLAS_HOME (set by the NixOS activation
...
_SOURCE_REINSTALL_MSG = "python -m pip install -e <path-to-atlas-source>"


def running_from_source_tree(project_root: Optional[Path] = None) -> bool:
    """Return true when the running code looks like a local Atlas source tree."""
    root = _install_method_project_root(project_root)
    return (root / "pyproject.toml").is_file() and (root / "atlas_cli").is_dir()


def source_reinstall_command(project_root: Optional[Path] = None) -> str:
    """Return a safe reinstall command for the running source checkout."""
    root = _install_method_project_root(project_root)
    if running_from_source_tree(root):
        return f"{sys.executable} -m pip install -e {root}"
    return _SOURCE_REINSTALL_MSG
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Atlas was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ATLAS_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ATLAS_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.atlas``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ATLAS_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.atlas:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ATLAS_HOME``, a
    ...
    ``docker`` and ``atlas update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the repository-built Atlas image bakes a ``docker``
        stamp into ``/opt/atlas`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ATLAS_HOME.
    try:
    ...
    #    host install whose shared $ATLAS_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``atlas update``.
    try:
        method = (
            (get_atlas_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``atlas_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ATLAS_HOME``, so that two installs sharing one data directory
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Atlas lives in a ``uv tool`` layout.

    ``uv tool install atlas-agent`` places the install at
    ``.../uv/tools/atlas-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``atlas-agent`` happens to be uv-tool-installed on the machine while
    the *active* Atlas is a regular pip/venv install, causing
    ``atlas update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/atlas-agent/" in norm + "/"
...
# Long-form text for ``atlas update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# atlas_cli/main.py; lives here so the wording stays consistent and we
# don't grow two slightly-different copies.
#
# Why this matters:
#   - The built image excludes ``.git`` (see .dockerignore), so the
...
#     — that script installs a *new* host-side Atlas, it doesn't update
#     the running container.
#   - Atlas does not currently publish a public registry image. The supported
#     update path is rebuilding the local image from a fresh source checkout.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``atlas update`` doesn't apply inside the Docker container.

The container has no working tree to pull into. Atlas currently ships its
Dockerfile for local builds rather than a public registry image. Update the
host checkout, rebuild the image, and recreate the services:
...
Verify the new version after restart:
  docker run --rm atlas-agent:local --version

Notes:
  • Your config and session history live under ``$ATLAS_HOME`` (``/opt/data``
    in the container, typically bind-mounted from the host) and persist
    across image rebuilds.
  • Run these commands from the AtlasAgent repository containing the
    ``Dockerfile`` and ``docker-compose.yml``."""


def format_docker_update_message() -> str:
    """Return the user-facing message for ``atlas update`` inside Docker.
...
def format_managed_message(action: str = "modify this Atlas installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ATLAS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Atlas installation is managed by NixOS "
            f"(ATLAS_MANAGED={env_hint}).\n"
            "Edit services.atlas-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Atlas installation is managed by Homebrew "
            f"(ATLAS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade atlas-agent"
        )

    return (
        f"Cannot {action}: this Atlas installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Atlas."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ATLAS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, atlas_bin
    or None if container mode is not active, we're already inside the
    container, or ATLAS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ATLAS_DEV") == "1":
        return None
    ...
    container_mode_file = get_atlas_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "atlas-agent")
    exec_user = info.get("exec_user", "atlas")
    atlas_bin = info.get("atlas_bin", "/data/current-package/bin/atlas")
...
# Re-export from atlas_constants — canonical definition lives there.
from atlas_constants import get_atlas_home  # noqa: F811,E402
...
def _resolve_atlas_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ATLAS_UID / ATLAS_GID env vars set by Docker deployments.

    Docker containers running Atlas commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ATLAS_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ATLAS_UID", "").strip()
    gid_str = os.environ.get("ATLAS_GID", "").strip()
    try:
...
def _chown_to_atlas_uid(path) -> None:
    """Chown ``path`` to ``ATLAS_UID:ATLAS_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_atlas_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the atlas group can
    share state with the gateway service.

    The mode can be overridden via the ATLAS_HOME_MODE environment variable
    (e.g. ATLAS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ATLAS_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ATLAS_UID``/``ATLAS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ATLAS_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_atlas_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Atlas runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ATLAS_CONTAINER") or os.environ.get("ATLAS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ATLAS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ATLAS_HOME, upgrading legacy empty templates.
    ...
    if soul_path.exists():
        try:
            ...
            return
        rebranded = replace_legacy_creator_branding(existing)
        if rebranded != existing:
            soul_path.write_text(rebranded, encoding="utf-8")
            _secure_file(soul_path)
            existing = rebranded
        if not is_legacy_template_soul(existing):
            return
        # Legacy empty/default template -> upgrade to the real default in place.
    soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
    _secure_file(soul_path)


def ensure_atlas_home():
    """Ensure ~/.atlas directory structure exists with secure permissions.
    ...
    """
    home = get_atlas_home()
    # Named profiles must be created explicitly (e.g. ``atlas profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ATLAS_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_atlas_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_atlas_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ATLAS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Atlas-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Atlas
        ...
        # identity slot (SOUL.md). Empty by default. The ATLAS_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Atlas adds a coding operating brief
    ...
    "terminal": {
        ...
        #             ATLAS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ATLAS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Atlas auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Atlas sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Atlas
        ...
        # bundled Atlas image, which drops to the `atlas` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Atlas sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Atlas should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``atlas chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.atlas/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: atlas sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .atlas.md, or .cursorrules before Atlas applies
    ...
    # single read_file page exceeds these limits, Atlas truncates the
    ...
    # See tools/kanban_tools.py and atlas_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``atlas dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``atlas kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``atlas kanban decompose`` and the kanban
        ...
        # ``atlas profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `atlas model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `atlas` (and `atlas chat`) launches by default:
        ...
        # REPL and `--tui` (or ATLAS_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `atlas --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `atlas -c` muscle memory.  Default off so existing
        # users aren't surprised.  ATLAS_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `atlas --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/atlas`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `atlas pets`. Disabled until
        ...
        "pet": {
            ...
            # get_atlas_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Atlas Gateway plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``ATLAS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ATLAS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ATLAS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ATLAS_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ATLAS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/atlas``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Atlas appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.atlas/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.atlas/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.atlas/cache/delegation/
    ...
    # response. If not, Atlas feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Atlas auto-pauses the goal and
    ...
    "moa": {
        ...
        # <atlas_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.atlas/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ATLAS_SKILL_DIR} and ${ATLAS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `atlas curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `atlas curator run --consolidate` overrides
        ...
        # normally restored on every `atlas update`, so pruning them only
        ...
        # skipped), snapshot ~/.atlas/skills/ into
        # ~/.atlas/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `atlas curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for atlas-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Atlas Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ATLAS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Atlas' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.atlas/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ATLAS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `atlas doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``atlas_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Atlas to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ATLAS_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Atlas Gateway token — there is no token key here.
        "chronos": {
        ...
        # Also overridable via ATLAS_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `atlas -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `atlas` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `atlas kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     atlas-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Atlas tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.atlas/logs/.
    ...
    # curated model lists for OpenRouter and Atlas Gateway from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a atlas-agent release.
    ...
    "gateway": {
        ...
        # internal ATLAS_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the ATLAS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.atlas.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.atlas/.env,
        ...
        # behavior: files must live under the Atlas cache, under
        ...
        # user. Bridged to ATLAS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Atlas
        # cache (~/.atlas/cache/{documents,images,audio,video,screenshots})
        ...
        # to ATLAS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ATLAS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ATLAS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.atlas/state.db.
    ...
    "sessions": {
        ...
        # default of ``atlas sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.atlas/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``atlas backup``-style zip of ATLAS_HOME before every
        # ``atlas update``.  Backups land in ``<ATLAS_HOME>/backups/`` and
        # can be restored with ``atlas import <path>``.  Off by default:
        # zipping a large ATLAS_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``atlas update
        ...
        "backup_keep": 5,
        # What `atlas update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ATLAS_HOME>/lsp/bin/`` on first use.
    ...
    # `atlas tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.atlas/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.atlas/.env (or your shell) and never in config.yaml.
            ...
            # ~/.atlas/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Atlas disables it for our users unless they
        # explicitly opt in here. When false (default), Atlas sets
    ...
    # Atlas Desktop (Electron app) launch options. These only affect
    # `atlas desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ATLAS_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
# selection step (Atlas Gateway / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "Atlas Gateway base URL override",
        "prompt": "Atlas Gateway base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Atlas mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'atlas model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Atlas auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Atlas, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @atlas:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ATLAS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #atlas)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: atlas-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'atlas-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Atlas API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Atlas API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Atlas API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ATLAS_TOOL_PROGRESS and ATLAS_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.atlas.config`` entries, then checks
    ...
    try:
        ...
        # should never break `atlas update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Atlas versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``atlas_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'atlas setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Atlas won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'atlas doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ATLAS_HOME", "~/.atlas")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "atlas update / atlas -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ATLAS_TOOL_PROGRESS")
            old_mode = get_env_value("ATLAS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ATLAS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ATLAS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ATLAS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ATLAS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ATLAS_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_atlas_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `atlas plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `atlas curator status` has no
    ...
    #   3. Creates `~/.atlas/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_atlas_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_atlas_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `atlas config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `atlas config set`"
                    )
    ...
    # behavior set it to true explicitly via `atlas config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from atlas_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: atlas config set <key> <value>")
    ...
    # `atlas update`, but only the version bump is persisted.
    ...
    # metadata.atlas.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``atlas config set``
    ...
    backends) — while display paths (``atlas status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _atlas_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.atlas/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.atlas/config.yaml.
    ...
    (which change ``ATLAS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/atlas/config.yaml invalidates the
...
#   openai-codex (OAuth — atlas auth) — OpenAI Codex
#   nous         (OAuth — atlas auth) — Atlas Gateway
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.atlas/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Atlas writes itself."""
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`atlas tools`, `atlas setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `atlas tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Atlas env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.atlas/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.atlas/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.atlas/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.atlas/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Atlas — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Atlas vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.atlas/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.atlas/.env`` over ``os.environ``.

    Used for Atlas-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Atlas Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from atlas_cli.auth import get_anthropic_key
    ...
    # Warn on stale ATLAS_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ATLAS_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ATLAS_MAX_ITERATIONS={_env_ghost} "
                f"(run 'atlas doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  atlas config edit     # Edit config file", Colors.DIM))
    print(color("  atlas config set <key> <value>", Colors.DIM))
    print(color("  atlas setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `atlas config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `atlas config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: atlas config set <key> <value>")
            print()
            print("Examples:")
            print("  atlas config set model anthropic/claude-sonnet-4")
            print("  atlas config set terminal.backend docker")
            print("  atlas config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'atlas config migrate' to add them")
        ...
        print("  atlas config           Show current configuration")
        print("  atlas config edit      Open config in editor")
        print("  atlas config set <key> <value>   Set a config value")
        print("  atlas config check     Check for missing/outdated config")
        print("  atlas config migrate   Update config with new options")
        print("  atlas config path      Show config file path")
        print("  atlas config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``atlas config`` UI so users
