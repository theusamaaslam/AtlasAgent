...
the tool registry or launching Atlas (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``atlas-cli`` to the non-existent ``atlas``. ``resolve_toolset('atlas')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``atlas-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``atlas`` instead of ``atlas-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `atlas tools` to reconfigure."
        )
    return warnings
