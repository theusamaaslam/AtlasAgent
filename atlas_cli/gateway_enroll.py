"""``atlas gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Atlas Gateway access token from the existing login
     (``~/.atlas/auth.json``) — the same path ``atlas dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.atlas/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Atlas Gateway "
                "token could not be verified — try `atlas auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `atlas gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.atlas/.env (0600 dir) and never logs.
    to_write = {
