...
Used by ``atlas profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``atlas_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Atlas Agent kanban board.
...
Rules:
  ...
  - Never write "Atlas Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_atlas_home())
        else:
