...
Usage:
    atlas pairing list              # Show all pending + approved users
    atlas pairing approve <platform> <code>  # Approve a pairing code
    atlas pairing revoke <platform> <user_id> # Revoke user access
    atlas pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle atlas pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: atlas pairing {list|approve|revoke|clear-pending}")
        print("Run 'atlas pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.atlas/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'atlas pairing list' to see pending codes.\n")
