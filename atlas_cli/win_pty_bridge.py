"""Windows ConPTY bridge for the `atlas dashboard` chat tab.

Drop-in counterpart to ``atlas_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``atlas_cli.web_server`` consumes: ``spawn``, ``read``,
