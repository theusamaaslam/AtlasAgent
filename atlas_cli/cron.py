...
# commands at execution time when ``_ATLAS_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: atlas gateway install", Colors.DIM))
    print(color("                    sudo atlas gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  atlas cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'atlas cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: atlas gateway restart")
        ...
        print("    atlas gateway install    # Install as a user service")
        print("    sudo atlas gateway install --system  # Linux servers: boot-time system service")
        print("    atlas gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: atlas cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
