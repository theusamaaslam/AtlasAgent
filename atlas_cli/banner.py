...
Pure display functions with no AtlasCLI state dependency.
"""
...
from atlas_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

ATLAS_AGENT_LOGO = """[bold #FFD700]  AAAAA  TTTTT  L       AAAAA   SSSS[/]
[bold #FFD700] A     A   T    L      A     A S[/]
[#FFBF00] AAAAAAA   T    L      AAAAAAA  SSS[/]
[#FFBF00] A     A   T    L      A     A     S[/]
[#CD7F32] A     A   T    LLLLL  A     A SSSS[/]"""

ATLAS_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
# Whitelabel defaults. These assignments intentionally sit after the legacy
# constants so old art is replaced before rendering.
ATLAS_AGENT_LOGO = """[bold #8F95DC] █████╗ ████████╗██╗      █████╗ ███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #8F95DC]██╔══██╗╚══██╔══╝██║     ██╔══██╗██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#BFC4ED]███████║   ██║   ██║     ███████║███████╗      ███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║   [/]
[#BFC4ED]██╔══██║   ██║   ██║     ██╔══██║╚════██║      ██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║   [/]
[#FFFFFF]██║  ██║   ██║   ███████╗██║  ██║███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║   [/]
[#FFFFFF]╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝      ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝   [/]
[#8F95DC]                         Created by Usama Aslam[/]"""

ATLAS_CADUCEUS = """[#8F95DC]          .-''''-.          [/]
[#BFC4ED]       .-'  .--.  '-.       [/]
[#FFFFFF]      /   .'    '.   \\      [/]
[#8F95DC]     |   /   /\\   \\   |     [/]
[#BFC4ED]     |   \\   \\/   /   |     [/]
...
# (e.g. nix-built atlas — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/UsamaAslam/atlas-agent.git"
...
def _fetch_pypi_latest(package: str = "atlas-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Atlas update is available.

    Two paths: if ``ATLAS_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = atlas_home / ".update_check"
    embedded_rev = os.environ.get("ATLAS_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ATLAS_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `atlas update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/atlas/update/check`
    ...
    try:
        from atlas_cli.config import detect_install_method, running_from_source_tree
        method = detect_install_method()
        if method == "docker" or (method == "pip" and running_from_source_tree()):
            return None
    except Exception:
        pass
    ...
    if embedded_rev:
        ...
        # $ATLAS_HOME/atlas-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = atlas_home / "atlas-agent"
        if not (repo_dir / ".git").exists():
            behind = None
        else:
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Atlas git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ATLAS_HOME/atlas-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``atlas_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from atlas_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/UsamaAslam/atlas-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Atlas checkout. Cached per-process. Release URL always points at the
    canonical UsamaAslam/atlas-agent repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ATLAS_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ATLAS_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Usama Aslam[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Usama Aslam[/]")

    if os.getenv("ATLAS_YOLO_MODE"):
    ...
    try:
        from atlas_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from atlas_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built atlas; we know an update
    ...
    # Pip-install warning — `pip install atlas-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ATLAS_AGENT_LOGO
        console.print(_logo)
        console.print()
