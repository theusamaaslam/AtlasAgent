"""
Interactive setup wizard for Atlas Agent.
...
from atlas_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://atlas.local/docs"

_HOSTED_GATEWAY_FEATURES_ENABLED = False
_HOSTED_GATEWAY_REMOVED_MESSAGE = (
    "The hosted Nous gateway has been removed from this Atlas build. "
    "Use `atlas model` to configure OpenRouter, OpenAI, Anthropic, xAI, "
    "local models, or a custom endpoint."
)


def _print_hosted_gateway_removed() -> None:
    print_warning(_HOSTED_GATEWAY_REMOVED_MESSAGE)
...
# display_atlas_home imported lazily at call sites (stale-module safety during atlas update)
...
from atlas_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from atlas_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Atlas Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Atlas using environment variables or config commands:")
    print_info("  atlas config set model.provider custom")
    print_info("  atlas config set model.base_url http://localhost:8080/v1")
    print_info("  atlas config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'atlas setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``ATLAS_NONINTERACTIVE=1`` (see ``atlas_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ATLAS_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ATLAS_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from atlas_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'atlas setup')")


def _print_setup_summary(config: dict, atlas_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'atlas setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, or Tavily)
    if _HOSTED_GATEWAY_FEATURES_ENABLED and subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Atlas Gateway subscription)", True, None))
    ...
    if _HOSTED_GATEWAY_FEATURES_ENABLED and subscription_features.browser.managed_by_nous:
    ...
    # provider (OpenAI, etc.)
    if _HOSTED_GATEWAY_FEATURES_ENABLED and subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (Atlas Gateway subscription)", True, None))
    ...
    # Video generation — opt-in via `atlas tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if _HOSTED_GATEWAY_FEATURES_ENABLED and subscription_features.video_gen.managed_by_nous:
        tool_status.append(("Video Generation (FAL via Atlas Gateway subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from atlas_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if _HOSTED_GATEWAY_FEATURES_ENABLED and subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via Atlas Gateway subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'atlas setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'atlas setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if _HOSTED_GATEWAY_FEATURES_ENABLED and subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (Atlas Gateway subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'atlas setup terminal'"))
    elif _HOSTED_GATEWAY_FEATURES_ENABLED and managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via Atlas Gateway subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via atlas auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'atlas setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {atlas_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('atlas setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('atlas setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('atlas setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('atlas setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('atlas setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('atlas config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('atlas config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('atlas config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('atlas', Colors.GREEN)}              Start chatting")
    print(f"   {color('atlas gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('atlas doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `atlas tools` and `atlas setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``atlas model``)
    ...
    provider added to ``atlas model`` is automatically available here.
    ...
    # Delegate to the shared atlas model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from atlas_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: atlas model")
    ...
    # on demand via `atlas auth add`, `atlas setup` vision, and
    # `atlas setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``atlas model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from atlas_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.atlas/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Atlas auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via atlas setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'atlas setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Atlas runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `atlas setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `atlas setup terminal`.
        ...
        if managed_modal_available:
            modal_choices = [
                "Use my Atlas Gateway subscription",
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ATLAS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ATLAS_MAX_ITERATIONS")
    ...
    print_info("  Run `atlas setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ATLAS_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.atlas/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from atlas_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_atlas_home(Path(get_atlas_home()))
    except Exception:
        pass
...
def _profile_name_from_atlas_home(atlas_home) -> str | None:
    """Return the active profile name when ATLAS_HOME is a profile dir."""
    if atlas_home.parent.name == "profiles":
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Atlas delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Atlas to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from atlas_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://atlas.local/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://atlas.local/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  atlas config edit")
    print_info("   Open config in your editor:  atlas config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from atlas_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Atlas from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'atlas setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     atlas config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_atlas_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: atlas gateway install --system")
                print_info("  Or run in foreground:  atlas gateway")
        else:
            from atlas_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   atlas gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   atlas gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `atlas setup tools` and `atlas tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``atlas_cli.auth`` — lists every supported
    ...
    """
    try:
        from atlas_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from atlas_cli.config
    ...
    if section_key == "model":
        ...
        from atlas_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_atlas migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_atlas", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Atlas.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Atlas to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Atlas at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Atlas at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Atlas at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Atlas at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Atlas equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Atlas config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Atlas.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Atlas's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(atlas_home: Path) -> bool:
    ...
    print_info("Atlas can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: atlas claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=atlas_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: atlas claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Atlas configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Atlas config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Atlas (use atlas claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Atlas Gateway setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``atlas setup --portal`` and ``atlas portal``. This is the
    ...
    Atlas session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``atlas setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``atlas model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``atlas portal`` always offers a model picker,
    and there is a single source of truth for the Nous onboarding steps.
    """
    _print_hosted_gateway_removed()
    return
    ...
    print(color("│     ⚕ Atlas Setup — Atlas Gateway (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Atlas Gateway sub.")
    print()
    ...
    # setup calls, so `atlas portal` == quick setup's Nous step.
    try:
        from atlas_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `atlas portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `atlas portal`: %s", exc)
        print()
        print_error(f"  Atlas Gateway setup encountered an error: {exc}")
        print_info("  You can retry later with `atlas portal`.")
        return
    ...
    print_info("  Run `atlas portal info` to inspect routing.")
    print_info("  Run `atlas` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      atlas setup           — full or quick (auto-detected)
      atlas setup model     — just model/provider
      atlas setup tts       — just text-to-speech
      atlas setup terminal  — just terminal backend
      atlas setup gateway   — just messaging platforms
      atlas setup tools     — just tool configuration
      atlas setup agent     — just agent settings
    """
    from atlas_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal used to start hosted gateway setup. It is retained only so old
    # scripts get a clear message instead of an argparse failure.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Atlas Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Atlas Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Atlas Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, atlas_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Atlas configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'atlas setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(atlas_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Atlas?",
            [
                "Full setup - configure provider, tools & options yourself",
        ...
        if setup_mode == 1:
            _run_blank_slate_setup(config, atlas_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {atlas_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'atlas config edit'")
    ...
    # Tune later with `atlas setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, atlas_home)


def _run_first_time_quick_setup(config: dict, atlas_home, is_existing: bool):
    """Streamlined first-time setup via Atlas Gateway: OAuth, model, terminal & messaging.

    Routes straight to the Atlas Gateway provider — runs the device-code OAuth
    ...
    settings, tools); the user can customize later via ``atlas setup <section>``
    or switch providers with ``atlas model``.
    """
    _print_hosted_gateway_removed()
    return
    ...
    # Step 1: Atlas Gateway — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nous model picker). Provider is set to "nous" by the login/model save.
    print()
    print_header("Atlas Gateway")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.nousresearch.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Atlas Gateway setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"Atlas Gateway setup encountered an error: {exc}")
        print_info("You can try again later with: atlas model")
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'atlas setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    atlas setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  atlas setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``atlas tools`` (which
    ...
    try:
        ...
        from atlas_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``atlas setup agent`` /
    ``atlas config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, atlas_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `atlas update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        atlas tools")
        print_info("  Seed skills:         atlas skills opt-in --sync")
        print_info("  Add MCP servers:     atlas mcp add")
        print_info("  Enable plugins:      atlas plugins")
        print_info("  Tune agent settings: atlas setup agent")
        print()
        _print_setup_summary(config, atlas_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, atlas_home)


def _blank_slate_walkthrough(config: dict, atlas_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`atlas update` runs from re-injecting them. Opt back in any")
            print_info("time with `atlas skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `atlas tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `atlas plugins list` / `atlas plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `atlas plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `atlas mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `atlas mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   atlas tools")
    print_info("  Seed skills:         atlas skills opt-in --sync")
    print_info("  Add MCP servers:     atlas mcp add")
    print_info("  Tune agent settings: atlas setup agent")
    print()
...
def _run_quick_setup(config: dict, atlas_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'atlas setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Atlas to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'atlas setup gateway'.")
