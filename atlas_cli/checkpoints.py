"""`atlas checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.atlas/checkpoints/``.  Actions:

    atlas checkpoints               # same as `status`
    atlas checkpoints status        # total size, project count, breakdown
    atlas checkpoints list          # per-project checkpoint counts + workdir
    atlas checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    atlas checkpoints clear [-f]    # nuke the entire base (asks first)
    atlas checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    atlas checkpoints
    atlas checkpoints prune --retention-days 3 --max-size-mb 200
    atlas checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: atlas checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``atlas checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `atlas checkpoints` → status
