...
Profile-aware location: ``$ATLAS_HOME/logs/dashboard-auth.log``.
...
from ``atlas_constants`` or other atlas_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ATLAS_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``atlas_constants.get_atlas_home`` semantics: env var wins,
    else ``~/.atlas``. A local copy avoids an import cycle with the
    middleware which lives below ``atlas_cli``.
    """
    home = os.environ.get("ATLAS_HOME") or str(Path.home() / ".atlas")
