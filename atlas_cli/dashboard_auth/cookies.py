...
Three cookies in play:
  - atlas_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - atlas_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Atlas Gateway issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / atlas
  ...
  - atlas_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/atlas) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/atlas`` covers
...
setter wrote ``__Secure-atlas_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "atlas_session_at"
SESSION_RT_COOKIE = "atlas_session_rt"
PKCE_COOKIE = "atlas_session_pkce"
...
SSO_ATTEMPT_COOKIE = "atlas_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /atlas`` we want ``Path=/atlas`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Atlas Gateway
    issues a 24h rotating refresh token (atlas #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/atlas``)
