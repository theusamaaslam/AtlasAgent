...
from atlas_cli.dashboard_auth.base import (
    InvalidCodeError,
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``ATLAS_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /atlas`` (Mission Control deploys) — we
    ...
    from atlas_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``atlas_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from atlas_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    if getattr(p, "supports_password", False):
        # Password providers do not have an OAuth first leg. Keep old
        # /auth/login?provider=basic links from crashing by sending the browser
        # back to the server-rendered password form.
    ...
    pkce = ls.cookie_payload.get("atlas_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from atlas_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
