...
from atlas_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
...
from atlas_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /atlas``, the
    ``login_url`` is prefixed (``/atlas/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from atlas_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["atlas.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``atlas_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # tries to reach Atlas Gateway's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from atlas_cli.dashboard_auth.cookies import clear_session_cookies
        from atlas_cli.dashboard_auth.prefix import prefix_from_request
