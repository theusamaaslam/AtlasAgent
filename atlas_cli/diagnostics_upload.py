"""Client for uploading ``atlas debug share`` bundles to Nous-internal S3.

This is the opt-in (``--nous``) destination for ``atlas debug share``.
...
only viewable by Atlas support (and allowlisted Discord mods) through a
...
NAS_BASE = os.environ.get(
    "ATLAS_DIAGNOSTICS_BASE_URL", "https://portal.nousresearch.com"
)
...
_USER_AGENT = "atlas-agent/debug-share"
