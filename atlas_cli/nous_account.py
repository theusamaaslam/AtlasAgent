"""Normalized Atlas Gateway account entitlement helpers."""
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Atlas Gateway "
    ...
    if account_info is None:
        return (
            f"Atlas could not verify your Atlas Gateway entitlement, so {capability} "
            f"is unavailable. Run `atlas model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Atlas cannot verify "
                f"your Atlas Gateway paid access for {capability}. Log in with "
                f"`atlas model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Atlas Gateway to use {capability}: run `atlas model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Atlas could not verify your Atlas Gateway paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `atlas model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Atlas could not find a Atlas Gateway account or organisation for this "
            f"login, so {capability} is unavailable. Run `atlas model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `atlas model` to refresh Atlas."
        return message

    return (
        f"Your Atlas Gateway account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Atlas Gateway credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Atlas Gateway plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Atlas Gateway account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Atlas Gateway account has no usable paid credits{credit_detail}, so "
...
def get_nous_portal_account_info(
    *,
    ...
    """Return normalized Atlas Gateway account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        from atlas_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from atlas_cli.auth import _decode_jwt_claims
    except Exception:
        return None
