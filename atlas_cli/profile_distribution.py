"""Profile distributions — shareable, packaged Atlas profiles via git.

A distribution is a Atlas profile published as a git repository (or
...
* ``atlas profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``atlas skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``atlas profile``, not a parallel tree):

    atlas profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "atlas-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            atlas_requires=str(data.get("atlas_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.atlas_requires:
            out["atlas_requires"] = self.atlas_requires
        if self.author:
...
def check_atlas_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Atlas {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Atlas distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``atlas profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Atlas profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Atlas distribution."
        )

    # Version check up-front so we fail fast
    check_atlas_requires(manifest.atlas_requires, atlas_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.atlas).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="atlas_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `atlas profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`atlas profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="atlas_dist_update_") as tmp:
        plan = plan_install(
