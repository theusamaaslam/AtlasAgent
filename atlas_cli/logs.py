"""``atlas logs`` — view and filter Atlas log files.
...
Usage examples::
    ...
    atlas logs                    # last 50 lines of agent.log
    atlas logs -f                 # follow agent.log in real time
    atlas logs errors             # last 50 lines of errors.log
    atlas logs gateway -n 100    # last 100 lines of gateway.log
    atlas logs gui -f            # follow gui.log (dashboard/pty/ws)
    atlas logs desktop -f        # follow desktop.log (Electron app boot/backend)
    atlas logs --level WARNING    # only WARNING+ lines
    atlas logs --session abc123   # filter by session ID substring
    atlas logs --component tools  # only tool-related lines
    atlas logs --since 1h         # lines from the last hour
    atlas logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_atlas_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Atlas runs — try 'atlas chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_atlas_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_atlas_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_atlas_home()}/logs/")
        return

    print(f"Log files in {display_atlas_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'atlas chat' to generate logs)")
