...
Pure logic: walks a Atlas config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `atlas doctor` and a future `atlas migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Atlas config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Atlas config and return retirement issues.
