"""
Dump command for atlas CLI.

Outputs a compact, plain-text summary of the user's Atlas setup
...
from atlas_cli.config import get_atlas_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.atlas/.env.
    ...
    exports. ``atlas debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.atlas_build_sha`` by the Dockerfile's
    ``ATLAS_GIT_SHA`` build-arg (see ``atlas_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``atlas dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from atlas_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(atlas_home: Path) -> int:
...
def _cron_summary(atlas_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = atlas_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"atlas_home:      {display_atlas_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.atlas/.env: a managed
        ...
        # A credential added via `atlas auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `atlas auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["atlas-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(atlas_home)}")
    lines.append(f"  skills:             {_count_skills(atlas_home)}")
