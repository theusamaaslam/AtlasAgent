"""
Status command for atlas CLI.

Shows the status of all Atlas Agent components.
"""
...
from atlas_cli.auth import AuthError, resolve_provider
from atlas_cli.colors import Colors, color
from atlas_cli.config import get_env_path, get_env_value, get_atlas_home, load_config
from atlas_cli.models import provider_label
from atlas_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``atlas config``'s
...
def show_status(args):
    """Show status of all Atlas Agent components."""
    ...
    if _effective_provider_label() == "LM Studio":
        from atlas_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from atlas_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   atlas gateway")
    ...
    jobs_file = get_atlas_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_atlas_home() / "sessions" / "sessions.json"
