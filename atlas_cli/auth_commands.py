...
def _block_hosted_gateway_provider(provider: str) -> None:
    if provider == "nous":
        raise SystemExit(_HOSTED_GATEWAY_REMOVED)
...
def auth_add_command(args) -> None:
    ...
    _block_hosted_gateway_provider(provider)
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_atlas_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:atlas_pkce",
    ...
    if provider == "nous":
        ...
        # <atlas-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `atlas --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``atlas auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`atlas auth add {provider}` is not implemented for auth type {requested_type} yet.")


def auth_list_command(args) -> None:
    provider_filter = _normalize_provider(getattr(args, "provider", "") or "")
    _block_hosted_gateway_provider(provider_filter)
    if provider_filter:
        providers = [provider_filter]
    else:
        providers = sorted({
            *(p for p in PROVIDER_REGISTRY.keys() if p != "nous"),
            "openrouter",
            *list_custom_pool_providers(),
        })
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Atlas reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `atlas auth status spotify`.")
    _block_hosted_gateway_provider(provider)
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `atlas auth` is called bare."""
...
def _pick_provider(prompt: str = "Provider") -> str:
    ...
    known = sorted(p for p in set(list(PROVIDER_REGISTRY.keys()) + ["openrouter"]) if p != "nous")
