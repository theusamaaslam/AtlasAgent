...
Add, remove, or reorder entries here — both `atlas setup` and
`atlas` provider-selection will pick up the change automatically.
"""
...
_ATLAS_USER_AGENT = f"atlas-cli/{_ATLAS_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `atlas model`
    flow without maintaining a separate static list.
    """
    from atlas_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ATLAS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Atlas-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ATLAS_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Atlas refreshes the cache from
...
# ---------------------------------------------------------------------------
# Atlas Gateway free-model helper
# ---------------------------------------------------------------------------
# The Atlas Gateway models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Atlas Gateway account tier detection
# ---------------------------------------------------------------------------
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Atlas that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Atlas that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Atlas Gateway user is on a free (unpaid) tier.
...
# ---------------------------------------------------------------------------
# Atlas Gateway recommended models
#
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_atlas_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the Atlas Gateway's curated recommended-models payload.
    ...
    (``$ATLAS_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  atlas model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `atlas model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `atlas model` TUI
...
#
# Some vendors expose several Atlas provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `atlas model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``atlas model``, the
...
# sensible default, but Atlas Gateway is a per-token *metered aggregator* whose
...
# path. The *interactive* default (GUI onboarding / ``atlas model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in atlas_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``atlas auth add openai-codex`` without
    ``atlas model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``atlas model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    atlas-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Atlas Gateway, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from atlas_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — atlas-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated Atlas Gateway model-id list.
    ...
    """
    try:
        from atlas_cli.model_catalog import get_curated_nous_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Atlas Gateway).
    """
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Atlas Gateway pricing.
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``atlas model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from atlas_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from atlas_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Atlas' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``atlas_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Atlas should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``atlas auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.atlas/.env``.
    ...
    """
    try:
        from atlas_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Atlas release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Atlas release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try live Atlas Gateway /models endpoint
        try:
            from atlas_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Atlas release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``atlas model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $ATLAS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from atlas_constants import get_atlas_home
    return get_atlas_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ATLAS_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ATLAS_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Atlas' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ATLAS_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from atlas_constants import get_atlas_home
    return get_atlas_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `atlas moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Atlas will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Atlas cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Atlas actually ships.  (#46850)
