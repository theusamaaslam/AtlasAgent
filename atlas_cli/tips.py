...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or atlas -c.",
    ...
    "Ctrl+Z suspends Atlas to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "atlas -c resumes your most recent CLI session. atlas -c \"project name\" resumes by title.",
    "atlas -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "atlas -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "atlas chat -t web,terminal enables only specific toolsets for a focused session.",
    "atlas chat -s github-pr-workflow preloads a skill at launch.",
    "atlas chat -q \"query\" runs a single non-interactive query and exits.",
    "atlas chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "atlas chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "atlas --yolo bypasses all dangerous command approval prompts for the entire session.",
    "atlas chat --source telegram tags the session for filtering in atlas sessions list.",
    "atlas -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "atlas doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "atlas dump outputs a compact setup summary — great for bug reports.",
    "atlas config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "atlas config edit opens config.yaml in your default editor.",
    "atlas config check scans for missing or stale configuration options.",
    "atlas sessions browse opens an interactive session picker with search.",
    "atlas sessions stats shows session counts by platform and database size.",
    "atlas sessions prune --older-than 30 cleans up old sessions.",
    "atlas skills search react --source skills-sh searches the skills.sh public directory.",
    "atlas skills check scans installed hub skills for upstream updates.",
    "atlas skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "atlas skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "atlas mcp add github --command npx adds MCP servers from the command line.",
    "atlas mcp serve runs Atlas itself as an MCP server for other agents.",
    "atlas auth add lets you add multiple API keys for credential pool rotation.",
    "atlas completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "atlas logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "atlas backup creates a zip backup of your entire Atlas home directory.",
    "atlas profile create coder creates an isolated profile that becomes its own command.",
    "atlas profile create work --clone copies your current config and keys to a new profile.",
    "atlas update syncs new bundled skills to ALL profiles automatically.",
    "atlas gateway install sets up Atlas as a system service (systemd/launchd).",
    "atlas memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "atlas webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: atlas tools disables unused tools, atlas skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "atlas models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as atlas -w).",
    ...
    "ATLAS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Atlas tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'atlas profile create coder' creates the 'coder' command.",
    "atlas profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Atlas prints a resume command with session ID and stats.",
    "atlas sessions export backup.jsonl exports all sessions for backup or analysis.",
    "atlas -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "atlas skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .atlas/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: atlas cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Atlas runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "atlas gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Atlas via a local macOS server.",
    ...
    "Context files (.atlas.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Atlas loads project context from .atlas.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "atlas mcp opens an interactive picker of Atlas-approved MCPs you can install in one keystroke.",
    "atlas mcp catalog lists Atlas-approved MCP servers shipped with the repo.",
    "atlas mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — atlas tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.atlas/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "atlas chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "atlas plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "atlas acp runs Atlas as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ATLAS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "atlas auth add nous or atlas auth add openai-codex sets up OAuth-based providers.",
    ...
    "atlas status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.atlas/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Atlas your own.",
    ...
    "Before a gateway session reset, Atlas auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.atlas/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Atlas state via ATLAS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ATLAS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ATLAS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.atlas/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'atlas profile create ls' would be rejected.",
    "atlas profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "atlas claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ATLAS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via atlas tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.atlas/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.atlas/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Atlas auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Atlas config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Atlas is working.',
    ...
    '/reload reloads ~/.atlas/.env into the running session — pick up new API keys without restarting.',
    ...
    "ATLAS_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.atlas/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.atlas/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'atlas curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "atlas curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'atlas curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'atlas auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Atlas Gateway subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'ATLAS_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ATLAS_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'atlas dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'atlas dashboard embeds the full Atlas TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.atlas/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.atlas/dashboard-plugins/ — no npm build required.',
    ...
    'ATLAS_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ATLAS_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ATLAS_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ATLAS_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.atlas/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.atlas/skills/ so drop-in skills appear without restarting the session.',
    ...
    'atlas -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'atlas chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'atlas chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'atlas chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "atlas chat --source tool tags programmatic chats so they don't clutter atlas sessions list.",
    'atlas dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'atlas sessions rename <ID> "new title" renames any past session; atlas sessions delete <ID> removes one.',
    'atlas import restores a session export or profile archive produced by sessions export or profile export.',
    'atlas fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'atlas pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'atlas setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'atlas status --deep runs the full health sweep across every component; plain atlas status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ATLAS_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ATLAS_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.atlas/plugins/ — trust-gated by design.',
    "ATLAS_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ATLAS_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ATLAS_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ATLAS_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ATLAS_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ATLAS_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ATLAS_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ATLAS_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ATLAS_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ATLAS_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ATLAS_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Atlas block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from atlas sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.atlas/dashboard-plugins/.',
]
