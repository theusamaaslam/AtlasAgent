...
The Atlas docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Atlas Gateway). This module fetches, validates, and caches that manifest,
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.atlas/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    "https://raw.githubusercontent.com/theusamaaslam/AtlasAgent/main/website/static/api/model-catalog.json"
)
...
def _cache_path() -> Path:
    ...
    return get_atlas_home() / "cache" / "model_catalog.json"
...
def get_curated_nous_models() -> list[str] | None:
    """Return Atlas Gateway's curated list of model ids from the manifest.
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``atlas update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.atlas/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``atlas model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
