"""Helpers for Atlas Gateway subscription managed-tool capabilities."""
...
# coverage category (atlas_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `atlas tools` picker scope its entitlement gate to the selected backend, so a
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("ATLAS_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `atlas model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("ATLAS_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("ATLAS_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from atlas_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    source_label = "free tool pool" if pool_only else "Atlas Gateway subscription"
    ...
    if pool_only:
        ...
        title = (
            "Your Atlas Gateway subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline Atlas Gateway login for the Tool Gateway picker (`atlas tools`)
# ---------------------------------------------------------------------------


def ensure_nous_portal_access(
    *,
    capability: str = "the Atlas Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Atlas Tool Gateway, logging in if
    needed.

    Used by ``atlas tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Atlas Gateway)").  Unlike ``atlas model``'s Nous
    login, this:
    ...
    It only performs the Atlas Gateway device-code OAuth (when the user isn't
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the Atlas Gateway device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start Atlas Gateway login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Atlas Gateway login.")
    try:
        proceed = input("  Log in to Atlas Gateway now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Atlas Gateway login.")
        return False
    ...
    try:
        ...
        _sync_nous_pool_from_auth_store()
        print("  Atlas Gateway login successful.")
        return True
        ...
        print(f"  Atlas Gateway login failed: {exc}")
        return False
