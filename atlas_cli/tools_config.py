"""
Unified tool configuration for Atlas Agent.

`atlas tools` and `atlas setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.atlas/config.yaml under
...
from atlas_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from atlas_cli.nous_account import format_nous_portal_entitlement_message
...
from atlas_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ATLAS_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `atlas tools` → Video Generation, which walks
...
# `atlas tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from atlas_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `atlas tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``atlas tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from atlas_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``atlas tools`` checklist actually offers
    for ``platform``.
    ...
    so ``atlas tools`` never claims to add or remove a toolset the user was
...
from atlas_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "video_gen": {
        ...
        # FAL video generation billed via the Atlas Gateway.  Plugin-backed
    ...
    "x_search": {
        ...
        "setup_note": (
            "Atlas routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        #     billed via Atlas Gateway subscription (requires_nous_auth +
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set ATLAS_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ATLAS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ATLAS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ATLAS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ATLAS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ATLAS_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `atlas model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ATLAS_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``atlas update`` and
      by ``atlas computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `atlas update` calls this
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    driver_cmd = _cua_driver_cmd()
    try:
        # When not verbose (e.g. `atlas update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `atlas update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.atlas/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Atlas process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_atlas_home()}/atlas-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in atlas-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/atlas-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.atlas/config.yaml")
        ...
        # Run the full `atlas auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.atlas/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from atlas_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: atlas auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `atlas auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: atlas auth spotify")
        except Exception as exc:
        ...
        try:
            from atlas_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: atlas plugins enable observability/langfuse")
        _print_info("    Restart Atlas for tracing to take effect.")
        _print_info("    Verify: atlas plugins list")
        ...
        try:
            ...
            _print_info("    Run later: atlas auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `atlas auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: atlas auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: atlas auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``atlas tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``atlas tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: atlas tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    # "atlas-cli" (which include all _ATLAS_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[atlas-cli, spotify]`` after enabling Spotify via ``atlas
        ...
        # (e.g. "atlas-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``atlas tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `atlas tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"atlas-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("atlas-")}
    ...
    # shipped as a bundled plugin but user must opt in via `atlas tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `atlas tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by atlas tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `atlas` instead of
    # `atlas-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `atlas update`/`atlas doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `atlas tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (atlas-cli, atlas-telegram, etc.)
    ...
    # Opening `atlas tools` is the user's opt-in to reconfigure tools, so treat
...
def _visible_providers(
    ...
    *,
    ...
    that the capability exists.  Selecting one drives an inline Atlas Gateway
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        if (
            not _HOSTED_GATEWAY_FEATURES_ENABLED
            and (
...
def _hidden_nous_gateway_message(
    ...
    *,
    ...
    """Deprecated: Atlas Tool Gateway rows are no longer hidden.
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `atlas tools` gets a silent no-op
    ...
    *,
    ...
    if len(providers) == 1:
        ...
        for p in providers:
            ...
            # "included" star; everyone else gets a "via Atlas Gateway" hint so
            ...
            sub_marker = ""
            if _HOSTED_GATEWAY_FEATURES_ENABLED and p.get("managed_nous_feature"):
                if _nous_logged_in:
                    sub_marker = "  ★ Included with your Atlas Gateway subscription"
                else:
                    sub_marker = "  ★ via Atlas Gateway (login on select)"
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Atlas Gateway auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    if (
        not _HOSTED_GATEWAY_FEATURES_ENABLED
        and (managed_feature or provider.get("requires_nous_auth"))
    ):
        _print_warning("  Hosted gateway providers are disabled in this Atlas build.")
        return
    ...
    # "enable all tools" prompt (that lives in `atlas model`).
    if managed_feature:
        from atlas_cli.nous_subscription import (
        ...
        if not ensure_nous_portal_access(
            capability=f"{provider.get('name', 'the Atlas Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Atlas Gateway access is required for this backend."
            )
            return
    ...
    if provider.get("requires_nous_auth") and not managed_feature:
        ...
        if not features.nous_auth_present or not entitled:
            ...
            _print_warning(
                f"  {message or 'Nous Subscription is only available after logging into Atlas Gateway.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Atlas Gateway subscription.")
    ...
    if _show_portal_hint:
        _print_info("  Available through Atlas Gateway subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``atlas model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `atlas model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Atlas Gateway login + entitlement gate as _configure_provider:
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `atlas model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `atlas tools` and `atlas setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Atlas Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://atlas.local/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Atlas Gateway subscription defaults", Colors.GREEN))
    ...
    print()
    from atlas_constants import display_atlas_home
    print(color(f"  Tool configuration saved to {display_atlas_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'atlas' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, atlas mcp
