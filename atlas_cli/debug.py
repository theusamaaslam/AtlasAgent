"""``atlas debug`` debug tools for Atlas Agent.

Currently supports:
    atlas debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.atlas/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--nous`` to upload instead to private Atlas
                          storage (AWS S3) via a signed URL minted by the
                          diagnostics service: the bundle is private
                          (viewable only by Atlas support / allowlisted mods via
...
_REDACTION_BANNER = (
    "[atlas debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.atlas/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``atlas debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``atlas debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `atlas debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "atlas-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``atlas debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.atlas/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``atlas debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "atlas-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----AtlasDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from atlas_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_atlas_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``atlas dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``atlas dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "atlas-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``atlas debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "atlas debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `atlas debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  atlas debug delete <url>")

    print(f"\nShare these links with the Atlas team for support.")


_NOUS_PRIVACY_NOTICE = """\
⚠️  --nous: This uploads your debug bundle to private Atlas diagnostics storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Atlas version, provider, which API keys are
  ...
  • The bundle is viewable only by Atlas support (and allowlisted Discord mods)
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``atlas debug share --nous``: upload the bundle to private diagnostics storage.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the diagnostics envelope, requests a signed URL, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from atlas_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "atlas debug share --nous: applied force-mode redaction before upload"
        )
    blob = build_nous_bundle(bundle, redact=redact)

    print("Uploading to private Atlas diagnostics storage...")
    try:
        res = share_to_nous(blob)
    except Exception as exc:
        print(
            f"\nPrivate diagnostics upload failed: {exc}\n"
            "\nThe Atlas diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `atlas debug share --local` to print the report instead, "
            "or `atlas debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Atlas diagnostics (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with Atlas support — only authorized reviewers "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: atlas debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'atlas debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``atlas debug`` call.
    ...
    # best-effort — any failure is swallowed so ``atlas debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: atlas debug <command>")
        print()
        print("Commands:")
        ...
        print("  --nous       Upload to private Atlas diagnostics (private, staff-only,")
