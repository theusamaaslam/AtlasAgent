"""Atlas Gateway upstream adapter.

Reads the user's Nous OAuth state from ``~/.atlas/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the Atlas Gateway inference API."""
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Atlas Gateway. Run `atlas auth add nous` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh Atlas Gateway credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "Atlas Gateway refresh did not return a usable inference JWT. "
    ...
    # to atlas_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
