"""CLI handlers for the ``atlas proxy`` subcommand."""
...
from atlas_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "atlas proxy requires aiohttp. Install one of:\n"
        "  pip install 'atlas-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if provider == "nous":
        print(
            "Hosted gateway proxying has been removed from this Atlas build.",
            file=sys.stderr,
        )
        return 2
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"atlas auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Atlas proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Atlas proxy upstream adapters\n")
    for name in sorted(n for n in ADAPTERS if n != "nous"):
    ...
    print(
        "\nStart the proxy with: atlas proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``atlas proxy <subcommand>``."""
    ...
    print(
        "atlas proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  atlas proxy start [--provider xai] [--host 127.0.0.1] [--port 8645]\n"
