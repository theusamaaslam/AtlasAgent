"""
Backup and import commands for atlas CLI.

`atlas backup` creates a zip archive of the entire ~/.atlas/ directory
(excluding the atlas-agent repo and transient files).

`atlas import` restores from a backup zip, overlaying onto the current
ATLAS_HOME root.
"""
...
from atlas_constants import get_default_atlas_root, get_atlas_home, display_atlas_home
...
# ``atlas-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/atlas-agent/``
...
# ATLAS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "atlas-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ATLAS_HOME) —
...
# File names that ``atlas import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ATLAS_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ATLAS_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to atlas root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``atlas-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/atlas-agent/`` — must be preserved.
        if part == "atlas-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ATLAS_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Atlas home directory."""
    atlas_root = get_default_atlas_root()

    if not atlas_root.is_dir():
        print(f"Error: Atlas home directory not found at {atlas_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"atlas-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"atlas-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_atlas_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(atlas_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(atlas_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``atlas-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "atlas-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(atlas_root)
    ...
    # outside ATLAS_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_atlas_home()}."
        )
    ...
    print(f"\nRestore with: atlas import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Atlas backup.
    ...
    # Look for telltale files that a atlas home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Atlas backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.atlas/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a atlas dir name
        if prefix in {".atlas", "atlas"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_atlas_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (atlas_root / "config.yaml").exists()
        has_env = (atlas_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Atlas configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ATLAS_HOME.
            ...
            target = atlas_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(atlas_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_atlas_home()}."
            )
        ...
        profiles_dir = atlas_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # atlas_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: atlas profile list  (after installing atlas)")

        # Guidance
        print()
        if not (atlas_root / "atlas-agent").is_dir():
            print("Note: The atlas-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: atlas update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  atlas -p {pname} gateway install")

        print("Done. Your Atlas configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and atlas backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ATLAS_HOME).
...
# — `atlas update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ATLAS_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ATLAS_HOME and the entry is silently
...
def _quick_snapshot_root(atlas_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(atlas_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ATLAS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``atlas update``.
    ...
    Args:
        ...
        atlas_home: Override for the Atlas home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(atlas_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for atlas backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_atlas_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, atlas_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``atlas_root`` to ``out_path``.
...
def _pre_update_backup_dir(atlas_home: Optional[Path] = None) -> Path:
    home = atlas_home or get_atlas_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ATLAS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ATLAS_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``atlas update``) should continue even if the backup fails.
    """
    atlas_root = atlas_home or get_default_atlas_root()
    if not atlas_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(atlas_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, atlas_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `atlas claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ATLAS_HOME under ``backups/`` before a
    ``atlas claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``atlas import <archive>``.  Writes to
    ``<ATLAS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `atlas import` and the
