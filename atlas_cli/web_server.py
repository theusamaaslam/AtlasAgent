"""
Atlas Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m atlas_cli.main web          # Start on http://127.0.0.1:9119
    python -m atlas_cli.main web --port 8080
"""
...
from atlas_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from atlas_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `atlas dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ATLAS_WEB_DIST"]) if "ATLAS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``atlas gateway run`` — but the
    desktop app spawns a ``atlas dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same ATLAS_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import atlas_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire atlas_cli.gateway import into a background thread so the event
    # loop is not blocked and ATLAS_DASHBOARD_READY fires without delay.
...
app = FastAPI(title="Atlas Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from atlas_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ATLAS_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ATLAS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Atlas-Session-Token"
...
#
# This list is defined in ``atlas_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from atlas_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Atlas-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``atlas-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from atlas_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from atlas_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "dashboard.theme": {
        ...
        "options": ["default", "default-large", "nous-blue", "midnight", "ember", "mono", "cyberpunk", "rose"],
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Atlas (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``atlas model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Atlas provider slug — fine.
    ...
    1. Vendor-name → Atlas-provider mapping: when the provider string is
       not a known Atlas provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from atlas_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from atlas_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "ATLAS_DASHBOARD_FILES_ROOT"
...
# ATLAS_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to ATLAS_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Atlas credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no ATLAS_HOME resolution, so it blocks these trees wherever
...
def _default_atlas_root_is_opt_data() -> bool:
    raw = os.environ.get("ATLAS_HOME", "").strip()
    if not raw:
        return False
    try:
        from atlas_constants import get_default_atlas_root

        root = get_default_atlas_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``atlas update``.
    ...
    a container — e.g. the atlas-webui image sharing the Atlas source tree),
    the dashboard's ``atlas update`` button is the correct update path and
    ...
    """
    if _default_atlas_root_is_opt_data():
        return True
    try:
        from atlas_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Atlas root is actually /opt/data
    # (the container/hosted layout) or when ATLAS_DASHBOARD_FILES_ROOT is set.
    if _default_atlas_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``atlas:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.atlas-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``atlas_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from atlas_cli import web_git as _web_git  # noqa: E402
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_atlas_home() at call time
    ...
    try:
        ...
        # Windows install the first import of atlas_cli.gateway blocks the
        ...
        # and which providers are registered so ``atlas status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Usama Aslam" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from atlas_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_atlas": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the atlas home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_atlas_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_atlas_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Atlas Gateway auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
async def run_prompt_size():
    try:
        proc = _spawn_atlas_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_atlas_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_atlas_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from atlas_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.atlas/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_atlas_home() / "logs"
...
def _spawn_atlas_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``atlas <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``atlas_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "atlas_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ATLAS_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["atlas", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``atlas gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``atlas gateway restart`` children race each other on the
    ...
    return _spawn_atlas_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``atlas gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``ATLAS_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/atlas/update")
async def update_atlas():
    """Kick off ``atlas update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Atlas updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("atlas-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "atlas-update",
    ...
    try:
        proc = _spawn_atlas_action(["update"], "atlas-update")
    except Exception as exc:
        _log.exception("Failed to spawn atlas update")
...
@app.get("/api/atlas/update/check")
async def check_atlas_update(force: bool = False):
    """Report whether a Atlas update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/atlas/update`` actually runs ``atlas update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Atlas version string
    ...
    try:
        ...
        if force:
            try:
                (get_atlas_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.atlas/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Atlas surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Atlas supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_atlas_home() / provider.name / "config.json"
...
# in atlas_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `atlas model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `atlas model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from atlas_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from atlas_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.atlas/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (atlas_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_nous_managed_defaults):
        # auto-route any *unconfigured* tools through the Atlas Tool Gateway.
        ...
        if provider.strip().lower() == "nous":
            try:
                from atlas_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``atlas model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from atlas_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``atlas model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        from atlas_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `atlas model` provider — would be
        ...
        # Vertex — a `atlas model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `atlas model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Atlas from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Atlas to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Atlas from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Atlas to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Atlas in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Atlas through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Atlas via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Atlas through an IMAP/SMTP mailbox.",
        "docs_url": "https://atlas.local/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Atlas to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Atlas inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Atlas to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://atlas.local/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://atlas.local/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Atlas through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Atlas to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://atlas.local/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Atlas to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Atlas as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://atlas.local/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://atlas.local/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in atlas gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in atlas gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.atlas.local"
_TELEGRAM_ONBOARDING_USER_AGENT = f"AtlasDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Atlas Agent").strip() or "Atlas Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Atlas appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ATLAS_HOME.
...
# the canonical ``atlas auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.atlas/.anthropic_oauth.json`` — Atlas-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_atlas_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_atlas_oauth_credentials,
            _ATLAS_OAUTH_FILE,
        )
    except ImportError:
        read_atlas_oauth_credentials = None  # type: ignore
        _ATLAS_OAUTH_FILE = None  # type: ignore

    atlas_creds = None
    if read_atlas_oauth_credentials:
        try:
            atlas_creds = read_atlas_oauth_credentials()
        except Exception:
            atlas_creds = None
    if atlas_creds and atlas_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "atlas_pkce",
            "source_label": f"Atlas PKCE ({_ATLAS_OAUTH_FILE})",
            "token_preview": _truncate_token(atlas_creds.get("accessToken")),
            "expires_at": atlas_creds.get("expiresAt"),
            "has_refresh_token": bool(atlas_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from atlas_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Atlas even
    when they also have a separate Atlas-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Atlas never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "device_code",
        "cli_command": "atlas auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "atlas auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "atlas auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "atlas auth add xai-oauth",
        "docs_url": "https://atlas.local/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "atlas auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from atlas_cli import auth as hauth
        if provider_id == "nous":
            raw = hauth.get_nous_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "nous_portal",
                "source_label": raw.get("portal_base_url") or "Atlas Gateway",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Atlas, so the disconnect
    ...
    user sees exactly what executes, and Atlas then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Atlas — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``atlas model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `atlas model` universe (zero-edit for new plugins).
    try:
        from atlas_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"atlas auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("atlas_pkce", "claude_code", ...)
    ...
    sync with the `atlas model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Atlas-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _ATLAS_OAUTH_FILE
                if _ATLAS_OAUTH_FILE.exists():
                    _ATLAS_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from atlas_cli.auth import clear_provider_auth
        ...
        try:
            from atlas_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.atlas/.anthropic_oauth.json AND credential pool
...
# Guarded so atlas web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Atlas file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``atlas auth add anthropic``.
    """
    from agent.anthropic_adapter import _ATLAS_OAUTH_FILE
    payload = {
    ...
    _ATLAS_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _ATLAS_OAUTH_FILE.with_name(
        f"{_ATLAS_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ATLAS_OAUTH_FILE)
        try:
            _ATLAS_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ATLAS_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from atlas_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from atlas_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``atlas auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``atlas model`` re-login path in _login_xai_oauth).
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from atlas_cli.auth import _save_codex_tokens
...
async def start_oauth_login(
    ...
    if provider_id == "nous":
        raise HTTPException(
            status_code=410,
            detail="Hosted gateway login has been removed from this Atlas build.",
        )
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `atlas sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `atlas sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_atlas_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from atlas_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_atlas_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from atlas_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ATLAS_HOME)."""
    from atlas_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ATLAS_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (atlas_cli.mcp_config), so
# servers managed here show up under `atlas mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from atlas_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from atlas_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from atlas_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from atlas_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from atlas_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the ATLAS_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``atlas mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        from tools.mcp_oauth import AtlasTokenStorage, force_interactive_oauth
        ...
        # resolve ATLAS_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = AtlasTokenStorage(name)
...
async def list_mcp_catalog(profile: Optional[str] = None):
    """Browse the Atlas-approved MCP catalog (the optional-mcps/ manifests).
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds ATLAS_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (atlas_cli.webhook); the webhook
...
async def list_webhooks():
    import atlas_cli.webhook as wh
...
# spawn the real `atlas gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_atlas_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_atlas_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from atlas_cli.auth import read_credential_pool
...
# `atlas memory setup`).  The dashboard covers the common admin actions:
...
async def recall_memory(q: str = "", limit: int = 6, profile: Optional[str] = None):
    from agent.memory_facts import search_memory_recall

    safe_limit = max(1, min(int(limit or 6), 20))
    with _profile_scope(profile):
        return search_memory_recall(q, limit=safe_limit)


@app.post("/api/memory/consolidate")
async def consolidate_memory(profile: Optional[str] = None, limit: int = 200):
...
async def pending_memory_facts(limit: int = 50, profile: Optional[str] = None):
    ...
    safe_limit = max(1, min(int(limit or 50), 200))
    with _profile_scope(profile):
        return list_memory_facts(status="pending", limit=safe_limit)


@app.get("/api/memory/facts/search")
async def search_memory_facts(
    ...
    clean_status = (status or "").strip().lower() or None
    with _profile_scope(profile):
        return list_memory_facts(status=clean_status, query=q, limit=safe_limit)


@app.post("/api/memory/facts/{fact_id}/approve")
async def approve_memory_fact_api(fact_id: str, profile: Optional[str] = None):
    from agent.memory_facts import approve_memory_fact

    with _profile_scope(profile):
        return approve_memory_fact(fact_id)


@app.post("/api/memory/facts/{fact_id}/reject")
async def reject_memory_fact_api(fact_id: str, profile: Optional[str] = None):
    from agent.memory_facts import reject_memory_fact

    with _profile_scope(profile):
        return reject_memory_fact(fact_id)


@app.post("/api/memory/facts/{fact_id}/stale")
async def stale_memory_fact_api(fact_id: str, profile: Optional[str] = None):
    from agent.memory_facts import mark_memory_fact_stale

    with _profile_scope(profile):
        return mark_memory_fact_stale(fact_id)


@app.delete("/api/memory/facts/{fact_id}")
async def delete_memory_fact_api(fact_id: str, profile: Optional[str] = None):
    from agent.memory_facts import delete_memory_fact
...
async def run_doctor():
    try:
        proc = _spawn_atlas_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_atlas_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_atlas_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"atlas-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_atlas_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `atlas import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_atlas_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from atlas_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <atlas_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_atlas_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_atlas_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``atlas`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_atlas_action`` tracks one process/log per name, so a shared
...
# Human-readable labels for each hub source id (matches `atlas skills search`
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from atlas_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`atlas -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ATLAS_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_atlas_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "atlas setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ATLAS_HOME override so the write lands in the target
    ...
    token = set_atlas_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ATLAS_HOME override (same mechanism as
    ...
    """
    from atlas_constants import set_atlas_home_override, reset_atlas_home_override
    from atlas_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ATLAS_HOME override. Returns the
    number of skills newly disabled.
    """
    from atlas_constants import set_atlas_home_override, reset_atlas_home_override
    from atlas_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ATLAS_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``atlas profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ATLAS_HOME).
    """
    from atlas_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``atlas profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ATLAS_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``atlas profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from atlas_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_atlas_home()`` at call
       time — the context-local override from ``set_atlas_home_override``
    ...
    retargeted to the *current* ``get_atlas_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ATLAS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_atlas_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_atlas_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from atlas_cli.skills_config import get_disabled_skills
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``atlas tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``atlas tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `atlas tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``atlas tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.atlas/.env`` via ``save_env_value`` —
    the same store ``atlas tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``atlas tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``atlas -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ATLAS_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Atlas,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``atlas computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``atlas --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from atlas_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from atlas_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from atlas_cli.dashboard_auth.audit import AuditEvent, audit_log
        from atlas_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``atlas --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ATLAS_TUI_RESUME`` env var —
    matching what ``atlas_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ATLAS_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ATLAS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``ATLAS_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``ATLAS_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_atlas_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``atlas -p <name>`` performs. The in-process
    ``ATLAS_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from atlas_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ATLAS_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ATLAS_TUI_INLINE", "1")
    env["ATLAS_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["ATLAS_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ATLAS_TUI_RESUME"] = resume

    if sidecar_url:
        env["ATLAS_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile ATLAS_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ATLAS_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from atlas_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="atlas-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Atlas Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Atlas CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="atlas-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_atlas_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from atlas_cli.console_engine import AtlasConsoleEngine

        engine = AtlasConsoleEngine(
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Atlas Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Atlas inside WSL2 to use the dashboard's /chat "
...
# ATLAS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`atlas_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from atlas_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/atlas/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /atlas`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ATLAS_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/atlas``)
        ...
        ``__ATLAS_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ATLAS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ATLAS_BASE_PATH__="{prefix}";'
                f"window.__ATLAS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ATLAS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ATLAS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ATLAS_BASE_PATH__="{prefix}";'
                f"window.__ATLAS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/atlas`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Atlas backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Atlas Command",         "description": "Charcoal, jade, amber, and coral for the Atlas command center"},
    {"name": "default-large", "label": "Atlas Command (Large)", "description": "Atlas Command with bigger fonts and roomier spacing"},
    {"name": "nous-blue",     "label": "Atlas Blue",          "description": "Light mode with vivid blue accents on a clean canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.atlas/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.atlas/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_atlas_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.atlas/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as atlas_cli.plugins):
    1. User plugins:    ~/.atlas/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.atlas/plugins/  (only if ATLAS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from atlas_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_atlas_home() / "plugins", "user"),
    ...
    # ``atlas_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ATLAS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".atlas" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from atlas_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_atlas_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"atlas auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from atlas_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from atlas_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from atlas_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.atlas/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"atlas_dashboard_plugin_{plugin['name']}"
...
from atlas_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ATLAS_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("ATLAS_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    try:
        from atlas_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the atlas-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ATLAS_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Atlas Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `atlas dashboard register` (Atlas Gateway) or "
    ...
    # ATLAS_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"ATLAS_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Atlas Web UI → http://{host}:{actual_port}")
