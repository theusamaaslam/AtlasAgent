"""atlas hooks — inspect and manage shell-script hooks.

Usage::

    atlas hooks list
    atlas hooks test <event> [--for-tool X] [--payload-file F]
    atlas hooks revoke <command>
    atlas hooks doctor

Consent records live under ``~/.atlas/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.atlas/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``atlas hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: atlas hooks {list|test|revoke|doctor}")
        print("Run 'atlas hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.atlas/config.yaml.")
        print("See `atlas hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `atlas hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `atlas hooks test` and `atlas hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Atlas wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `atlas hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `atlas hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `atlas hooks doctor`.")
