...
load_atlas_dotenv(
    atlas_home=_atlas_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.atlas/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_atlas_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("ATLAS_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ATLAS_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ATLAS_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent AtlasCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str):
        ...
        self.proc = subprocess.Popen(
            ...
            bufsize=1,
            cwd=os.getcwd(),
            # slash_worker runs the Atlas agent → needs provider credentials.
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from atlas_cli.active_sessions import transfer_active_session
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `atlas --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("ATLAS_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `atlas --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from atlas_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its ATLAS_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_atlas_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s ATLAS_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_atlas_home``).
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `atlas` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ATLAS_HOME so config/skills/model resolve to it, and hand the
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from atlas_cli.runtime_provider import canonical_custom_identity
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _atlas_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_atlas_home_override()
        home = override if isinstance(override, str) and override else _atlas_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    atlas_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def resolve_skin() -> dict:
    try:
        from atlas_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ATLAS_MODEL", "")
        or os.environ.get("ATLAS_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over ATLAS_MODEL / ATLAS_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set ATLAS_INFERENCE_MODEL in the
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("ATLAS_TUI_PROVIDER", "").strip()
    ...
    try:
        from atlas_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ATLAS_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ATLAS_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("ATLAS_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Atlas): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `atlas --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ATLAS_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from atlas_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ATLAS_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ATLAS_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ATLAS_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _ATLAS_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off atlas-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ATLAS_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from atlas_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (ATLAS_MODEL /
    # ATLAS_INFERENCE_MODEL / ATLAS_TUI_PROVIDER / ATLAS_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # ATLAS_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ATLAS_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from atlas_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from atlas_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from atlas_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ATLAS_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ATLAS_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``AtlasCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    ``atlas_cli/cli_agent_setup_mixin.py``: when the primary provider
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses atlas_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ATLAS_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ATLAS_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ATLAS_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ATLAS_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ATLAS_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `ATLAS_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_atlas_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_atlas_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_atlas_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ATLAS_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``atlas --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``atlas gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Atlas has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from atlas_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from atlas_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from atlas_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from atlas_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from atlas_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_atlas_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("ATLAS_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Atlas Gateway / OpenRouter /
...
def _(rid, params: dict) -> dict:
    ...
    """
    from atlas_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from atlas_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from atlas_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from atlas_cli.auth import step_up_nous_billing_scope
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Atlas profile home
    # (~/.atlas/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $ATLAS_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_atlas_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ATLAS_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_atlas_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from atlas_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from atlas_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: atlas_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from atlas_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _atlas_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".atlas" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.atlas/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.atlas/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Atlas Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Atlas/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Atlas Desktop. If it serves Atlas/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from atlas_cli.model_switch import parse_model_flags
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ATLAS_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
...
def _projects_payload(conn) -> dict:
    from atlas_cli import projects_db as pdb
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under ATLAS_HOME (~/.atlas by default) — config/sessions/skills,
    ...
    atlas_home = os.path.realpath(str(get_atlas_home()))

    return real == home or real == atlas_home or real.startswith(atlas_home + os.sep)
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero atlas sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (atlas home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from atlas_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_atlas_home), "display": display_atlas_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from atlas_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Atlas provider is configured.",
                },
            )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `atlas` is interactive — use `/atlas chat -q …` or run `atlas` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`atlas setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`atlas gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`atlas sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`atlas config edit` needs $EDITOR in a real terminal"
    return None
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from atlas_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from atlas_cli.config import is_managed, save_env_value
        from atlas_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `atlas model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.atlas/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from atlas_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from atlas_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ATLAS_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ATLAS_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ATLAS_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from atlas_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["ATLAS_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from atlas_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from atlas_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from atlas_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from atlas_cli.browser_connect import launch_chrome_debug
