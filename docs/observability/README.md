# Atlas Observer Hooks

Atlas observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Atlas' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Atlas catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Atlas only builds
