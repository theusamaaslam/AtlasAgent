...
Atlas supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.atlas/config.yaml`:
...
Or set the env var: `ATLAS_KANBAN_DISPATCH_IN_GATEWAY=false`
