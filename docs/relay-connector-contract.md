...
This document is the formal interface between the **Atlas gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `atlas gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`atlas gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `atlas` CLI.
