#!/usr/bin/env bash
# GitHub environment detection helper for Atlas Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _atlas_env="${ATLAS_HOME:-$HOME/.atlas}/.env"; [ -f "$_atlas_env" ] && grep -q "^GITHUB_TOKEN=" "$_atlas_env" 2>/dev/null; then
