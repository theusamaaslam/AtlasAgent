...
metadata:
  atlas:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, atlas-video]
...
```
Atlas Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${ATLAS_HOME:-$HOME/.atlas}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Atlas config (if missing)
...
3. **Restart Atlas session** to pick up the new MCP server
