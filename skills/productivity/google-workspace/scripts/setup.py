#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Atlas Agent.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Atlas profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Atlas Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Atlas Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'atlas-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Atlas home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_atlas_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Atlas")
