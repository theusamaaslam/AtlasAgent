...
Connect Atlas to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Atlas supports two WeCom integration modes:
...
> Run `atlas gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Atlas decrypts the message, queues it for the agent
...
(Use `atlas gateway start` only after `atlas gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Atlas and the token the
...
in `~/.atlas/.env` values around `=` will also break signature checks. After
fixing, restart `atlas gateway run`.
...
Check `atlas gateway run` logs for the bound host/port. If the adapter bound to
