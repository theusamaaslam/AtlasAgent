...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Atlas Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Atlas to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.atlas/.env`:
...
All standard Atlas commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.atlas/logs/gateway.log`
...
4. Consider enabling verbose logging: `ATLAS_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Atlas automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ATLAS_LOG_LEVEL=debug atlas gateway
```
...
```bash
atlas chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
