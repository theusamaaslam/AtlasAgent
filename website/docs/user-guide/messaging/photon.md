...
Connect Atlas to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Atlas — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Atlas runs it in a
...
atlas photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Atlas Agent` project on your account.
...
Runtime credentials are written to `~/.atlas/.env`
...
(device token, dashboard project id) lives in `~/.atlas/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Atlas
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Atlas replies with a pairing code. Approve it with:

```bash
atlas pairing approve photon <CODE>
```

Use `atlas pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.atlas/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.atlas/.env`):
...
By default Atlas responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Atlas` and
`@Atlas agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Atlas will reply.
...
assigned iMessage line Atlas uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run atlas photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `atlas photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `atlas photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `atlas photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Atlas sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Atlas wake words  | JSON list / comma / newline regex patterns for group mentions |
