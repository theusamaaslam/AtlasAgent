...
sidebar_position: 12
title: "Google Chat"
description: "Set up Atlas Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Atlas Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Atlas
...
> Run `atlas gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Atlas can read (e.g.,
`~/.atlas/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `atlas-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `atlas-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a atlas restart)
...
- Principal: `atlas-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Atlas calls
...
- **App name**: whatever you want users to see ("Atlas" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/atlas-chat-events`.
...
`ADDED_TO_SPACE` event that Atlas uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Atlas

Add the Google Chat section to `~/.atlas/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/atlas-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.atlas/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Atlas extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Atlas is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Atlas detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Atlas session.
...
3. Download the JSON. Move it onto the host that runs Atlas.
4. Register the client with Atlas (run under the profile you want it scoped to):
...
atlas -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Atlas home (e.g.
`~/.atlas/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.atlas/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.atlas/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Atlas isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `atlas gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Atlas will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.atlas/google_chat_user_tokens/<sanitized_email>.json` (filesystem
