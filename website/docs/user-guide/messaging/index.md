...
description: "Chat with Atlas from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Atlas from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Atlas](/guides/use-voice-mode-with-atlas).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Atlas Gateway](/integrations/nous-portal) subscription bundles all of them.
:::
...
For group chats, hooks, and automation flows, Atlas supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Atlas keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Atlas does not hide failures just because the text resembles a silence token.
...
```bash
atlas gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
atlas gateway              # Run in foreground
atlas gateway setup        # Configure messaging platforms interactively
atlas gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo atlas gateway install --system   # Linux only: install a boot-time system service
atlas gateway start        # Start the default service
atlas gateway stop         # Stop the default service
atlas gateway status       # Check default service status
atlas gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Atlas Agent to the latest version |
...
Configure per-platform overrides in `~/.atlas/gateway.json`:
...
atlas pairing approve telegram XKGH5N7P

# Other pairing commands:
atlas pairing list          # View pending + approved users
atlas pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Atlas appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.atlas/config.yaml`:
...
Off by default. When enabled, Atlas prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.atlas/config.yaml`:
...
```bash
ATLAS_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
atlas gateway install               # Install as user service
atlas gateway start                 # Start the service
atlas gateway stop                  # Stop the service
atlas gateway status                # Check status
journalctl --user -u atlas-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo atlas gateway install --system
sudo atlas gateway start --system
sudo atlas gateway status --system
journalctl -u atlas-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Atlas installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit atlas-gateway` (or `sudo systemctl edit atlas-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `atlas update`. When `atlas update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart atlas-gateway` command (it never blocks on an interactive password prompt).
...
```bash
atlas gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `atlas update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo atlas update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/atlas-gateway`:

```
atlas ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed atlas-gateway*, /usr/bin/systemctl --no-ask-password start atlas-gateway*, /usr/bin/systemctl --no-ask-password restart atlas-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Atlas will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Atlas installations on the same machine (with different `ATLAS_HOME` directories), each gets its own systemd service name. The default `~/.atlas` uses `atlas-gateway`; other installations use `atlas-gateway-<hash>`. The `atlas gateway` commands automatically target the correct service for your current `ATLAS_HOME`.
:::

### macOS (launchd)

```bash
atlas gateway install               # Install as launchd agent
atlas gateway start                 # Start the service
atlas gateway stop                  # Stop the service
atlas gateway status                # Check status
tail -f ~/.atlas/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.atlas.gateway.plist`. It includes three environment variables:
...
- **ATLAS_HOME** — scopes the gateway to your Atlas installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `atlas gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ATLAS_HOME` directory gets its own launchd label. The default `~/.atlas` uses `ai.atlas.gateway`; other installations use `ai.atlas.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `atlas-cli` | Full access |
| Telegram | `atlas-telegram` | Full tools including terminal |
| Discord | `atlas-discord` | Full tools including terminal |
| WhatsApp | `atlas-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `atlas-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `atlas-slack` | Full tools including terminal |
| Google Chat | `atlas-google_chat` | Full tools including terminal |
| Signal | `atlas-signal` | Full tools including terminal |
| SMS | `atlas-sms` | Full tools including terminal |
| Email | `atlas-email` | Full tools including terminal |
| Home Assistant | `atlas-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `atlas-mattermost` | Full tools including terminal |
| Matrix | `atlas-matrix` | Full tools including terminal |
| DingTalk | `atlas-dingtalk` | Full tools including terminal |
| Feishu/Lark | `atlas-feishu` | Full tools including terminal |
| WeCom | `atlas-wecom` | Full tools including terminal |
| WeCom Callback | `atlas-wecom-callback` | Full tools including terminal |
| Weixin | `atlas-weixin` | Full tools including terminal |
| BlueBubbles | `atlas-bluebubbles` | Full tools including terminal |
| QQBot | `atlas-qqbot` | Full tools including terminal |
| Yuanbao | `atlas-yuanbao` | Full tools including terminal |
| Microsoft Teams | `atlas-teams` | Full tools including terminal |
| API Server | `atlas-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `atlas-webhook` | Full tools including terminal |
| Raft | `atlas-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.atlas/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
