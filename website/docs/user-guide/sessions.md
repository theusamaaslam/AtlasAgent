...
Atlas Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.atlas/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Atlas stores session history so it can resume conversations, but it does not
...
Atlas explicitly injects for that turn.
...
For example, if a user sends an image and asks Atlas to make a meme from it,
Atlas may inspect that image once with vision and run an image-processing
...
`atlas sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`atlas` or `atlas chat`) |
...
atlas -c "my project"   # → resumes "my project #3"
```
...
atlas --resume 20250305_091523_a1b2c3d4
atlas -r 20250305_091523_a1b2c3d4

# Resume by title
atlas --resume "refactoring auth"

# Or with the chat subcommand
atlas chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `atlas sessions list`.

### Conversation Recap on Resume

When you resume a session, Atlas displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Atlas session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.atlas/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `atlas -r "<title>"` from the shell) and pick up where the platform left off.
...
Atlas automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `atlas sessions list` or `atlas sessions browse`.
...
```bash
atlas sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Atlas creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`atlas -c "my project"`), it automatically picks the most recent session in the lineage.
...
Atlas provides a full set of session management commands via `atlas sessions`:
...
# Filter by platform
atlas sessions list --source telegram
...
atlas sessions export backup.jsonl

# Export sessions from a specific platform
atlas sessions export telegram-history.jsonl --source telegram

# Export a single session
atlas sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
atlas sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
atlas sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
atlas sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
atlas sessions prune

# Custom age threshold
atlas sessions prune --older-than 30

# Only prune sessions from a specific platform
atlas sessions prune --source telegram --older-than 60

# Skip confirmation
atlas sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`atlas insights`](/reference/cli-commands#atlas-insights).
...
When Atlas cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Atlas uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Atlas in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.atlas/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.atlas/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.atlas/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.atlas/sessions/sessions.json` is the **gateway routing index** — it maps
...
`atlas sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.atlas/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `atlas sessions list`, the cause is
`state.db` not receiving them — run `atlas sessions repair` and watch for a
...
`*.jsonl` files in `~/.atlas/sessions/`. They are no longer written or
read by Atlas. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Atlas process in the same `ATLAS_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.atlas/config.yaml`:
...
atlas sessions delete <session_id>
