---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Atlas Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Atlas Agent"
---
...
Migrate a user's OpenClaw customization footprint into Atlas Agent. Imports Atlas-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
# OpenClaw -> Atlas Migration

Use this skill when a user wants to move their OpenClaw setup into Atlas Agent with minimal manual cleanup.
...
```bash
atlas claw migrate              # Full interactive migration
...
atlas claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `atlas setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_atlas.py` to:

- import `SOUL.md` into the Atlas home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Atlas memory entries
- merge OpenClaw command approval patterns into Atlas `command_allowlist`
- migrate Atlas-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Atlas working-directory configuration
- copy OpenClaw skills into `~/.atlas/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Atlas workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.atlas/tts/`
- archive non-secret docs that do not have a direct Atlas destination
...
- `scripts/openclaw_to_atlas.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.atlas/skills/migration/openclaw-migration/scripts/openclaw_to_atlas.py`

Do not guess a shorter path like `~/.atlas/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.atlas/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Atlas-compatible secrets, currently:
...
Atlas CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Atlas. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Atlas workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Atlas skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.atlas/skills/migration/openclaw-migration/scripts/openclaw_to_atlas.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.atlas/skills/migration/openclaw-migration/scripts/openclaw_to_atlas.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.atlas/skills/migration/openclaw-migration/scripts/openclaw_to_atlas.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.atlas/skills/migration/openclaw-migration/scripts/openclaw_to_atlas.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.atlas/skills/migration/openclaw-migration/scripts/openclaw_to_atlas.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.atlas/skills/migration/openclaw-migration/scripts/openclaw_to_atlas.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Atlas unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Atlas targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Atlas destination. Unsupported auth blobs must still be reported as skipped.
...
- Atlas persona state imported
- Atlas memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.atlas/skills/openclaw-imports/`
