...
import upstream classes into Atlas itself.
...
```bash
mkdir -p ~/.atlas/cache/darwinian-evolver && cd ~/.atlas/cache/darwinian-evolver
...
```bash
cd ~/.atlas/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.atlas/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.atlas/skills/research/darwinian-evolver
DE_DIR=~/.atlas/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Atlas core.
   Custom driver scripts under `~/.atlas/skills/...` are user-side and fine.
