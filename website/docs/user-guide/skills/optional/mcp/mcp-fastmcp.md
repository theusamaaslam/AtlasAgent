...
- smoke-test a server with the FastMCP CLI before wiring it into Atlas or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Atlas. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.atlas/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.atlas/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Atlas integration, either:

- configure the server in `~/.atlas/config.yaml` using the `native-mcp` skill, or
...
### Atlas cannot see the deployed server

The server-building part may be correct while the Atlas config is not. Load the `native-mcp` skill and configure the server in `~/.atlas/config.yaml`, then restart Atlas.
