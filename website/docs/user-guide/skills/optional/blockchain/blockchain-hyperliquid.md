...
| | |
|---|---|
| Source | Optional — install with `atlas skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Atlas Agent |
| License | MIT |
...
The script reads `${ATLAS_HOME:-~/.atlas}/.env` for two optional defaults:
...
Helper script: `~/.atlas/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.atlas/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ATLAS_HOME:-~/.atlas}/.env`.
...
```bash
python3 ~/.atlas/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.atlas/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
