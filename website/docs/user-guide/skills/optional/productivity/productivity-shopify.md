...
| | |
|---|---|
| Source | Optional — install with `atlas skills install official/productivity/shopify` |
...
4. Save to `${ATLAS_HOME:-~/.atlas}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Atlas","productType":"Apparel","tags":["test"]}}'
```
