...
- Skill helper paths:
  - `${ATLAS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ATLAS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ATLAS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client atlas
```
...
```bash
PUBLISH="${ATLAS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client atlas
```
...
```bash
DRIVE="${ATLAS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `atlas`)    |
