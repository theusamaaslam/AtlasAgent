...
| | |
|---|---|
| Source | Optional — install with `atlas skills install official/productivity/siyuan` |
...
3. Store it in `${ATLAS_HOME:-~/.atlas}/.env`:
   ```
...
```yaml
# In ~/.atlas/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
