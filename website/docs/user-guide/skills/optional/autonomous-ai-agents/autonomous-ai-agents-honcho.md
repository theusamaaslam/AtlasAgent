...
description: "Configure and use Honcho memory with Atlas -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session sum..."
---
...
Configure and use Honcho memory with Atlas -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `atlas skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Atlas

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Atlas profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/atlas#running-honcho-locally-with-atlas

### Verify

```bash
atlas honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Atlas profile's AI peer
...
Honcho models conversations as interactions between **peers**. Atlas creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Atlas instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Atlas run |
| `global` | Single session across all directories |

Manual override: `atlas honcho map my-project-name`
...
Each Atlas profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block atlas.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `atlas.coder` host block in `honcho.json`
...
```bash
atlas honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Atlas when Honcho memory is active.
...
Config file: `$ATLAS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `atlas honcho setup`. Ensure `memory.provider: honcho` is in `~/.atlas/config.yaml`.

### Memory not persisting across sessions
Check `atlas honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `atlas profile create <name> --clone`. For existing profiles: `atlas honcho sync`.
...
| Command | Description |
|---------|-------------|
| `atlas honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `atlas honcho status` | Show resolved config, connection test, peer info for active profile |
| `atlas honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `atlas honcho disable` | Disable Honcho for the active profile |
| `atlas honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `atlas honcho peers` | Show peer identities across all profiles |
| `atlas honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `atlas honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `atlas honcho sessions` | List known directory-to-session-name mappings |
| `atlas honcho map <name>` | Map current working directory to a Honcho session name |
| `atlas honcho identity` | Seed AI peer identity or show both peer representations |
| `atlas honcho sync` | Create host blocks for all Atlas profiles that don't have one yet |
| `atlas honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Atlas + Honcho |
| `atlas memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `atlas memory status` | Show active memory provider and config |
| `atlas memory off` | Disable external memory provider |
