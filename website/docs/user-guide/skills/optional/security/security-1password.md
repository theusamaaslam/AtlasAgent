...
| | |
|---|---|
| Source | Optional — install with `atlas skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Atlas Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Atlas terminal calls (desktop app flow only)
...
### Service Account (recommended for Atlas)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ATLAS_HOME:-~/.atlas}/.env` (the skill will prompt for this on first load).
...
## Atlas Execution Pattern (desktop app flow)

Atlas terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/atlas-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/atlas-op.sock"
