...
| | |
|---|---|
| Source | Optional — install with `atlas skills install official/security/unbroker` |
...
| Related skills | [`google-workspace`](/user-guide/skills/bundled/productivity/productivity-google-workspace), [`agentmail`](/user-guide/skills/optional/email/email-agentmail), [`himalaya`](/user-guide/skills/bundled/email/email-himalaya), [`scrapling`](/user-guide/skills/optional/research/research-scrapling), [`osint-investigation`](/user-guide/skills/optional/research/research-osint-investigation) |
...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `$ATLAS_HOME/.env`** so keys
  Atlas already loads for its own tools are picked up without re-exporting - each one converts a
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `browser_*` tools. Nothing is stored. This requires Atlas to be pointed at the operator's own
      ...
      `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.atlas/chrome-debug"` (a dedicated
...
The engine stores data under `$PDD_DATA_DIR` (default `$ATLAS_HOME/unbroker`), written
...
- **Dossiers are plaintext by default** (JSON, `0600` under `ATLAS_HOME`). For at-rest encryption run
  ...
  a full-`ATLAS_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
