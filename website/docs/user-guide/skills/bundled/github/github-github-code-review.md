...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _atlas_env="${ATLAS_HOME:-$HOME/.atlas}/.env"; [ -f "$_atlas_env" ] && grep -q "^GITHUB_TOKEN=" "$_atlas_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_atlas_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Atlas Agent\",
    \"comments\": [
...
```bash
source "${ATLAS_HOME:-$HOME/.atlas}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Atlas Agent. Code looks clean — good test coverage, no security concerns."
