...
- Give it a name like "atlas-agent"
- Select scopes:
...
- Give it a title like "atlas-agent-&lt;machine-name>"
...
elif _atlas_env="${ATLAS_HOME:-$HOME/.atlas}/.env"; [ -f "$_atlas_env" ] && grep -q "^GITHUB_TOKEN=" "$_atlas_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_atlas_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
