...
This skill is separate from the Atlas Email gateway adapter. The gateway
adapter lets people email the agent and uses Atlas' built-in IMAP/SMTP
...
To reply non-interactively from Atlas, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Atlas)** — pipe the message via stdin:
