...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`atlas-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-atlas-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Atlas terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Atlas itself, `model.provider: openai-codex` uses Atlas-managed Codex
OAuth from `~/.atlas/auth.json` after `atlas auth add openai-codex`. For the
...
## Atlas Gateway Caveat

When invoking the Codex CLI from a Atlas gateway/service context (for example,
