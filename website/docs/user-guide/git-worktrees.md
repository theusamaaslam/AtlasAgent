...
title: "Git Worktrees"
description: "Run multiple Atlas agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Atlas Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Atlas so each session has a clean, isolated working directory.

## Why Use Worktrees with Atlas?

Atlas treats the **current working directory** as the project root:

- CLI: the directory where you run `atlas` or `atlas chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.atlas/config.yaml`
...
- A new branch: `feature/atlas-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Atlas there:

```bash
cd ../repo-feature

# Start Atlas in the worktree
atlas
```
...
- Works on its own branch (`feature/atlas-a` vs `feature/atlas-b`).
...
- Atlas checkpoint data under `~/.atlas/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Atlas experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/atlas-checkpoints-docs`, `feature/atlas-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Atlas from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `atlas -w` (Automatic Worktree Mode)

Atlas has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `atlas/atlas-<hash>`).
...
For parallel agents, open multiple terminals and run `atlas -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Atlas session its own clean checkout.
