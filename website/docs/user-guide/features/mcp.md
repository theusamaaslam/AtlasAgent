...
## Catalog: one-click install for Atlas-approved MCPs

Atlas ships a curated catalog of MCP servers that Usama Aslam has reviewed
...
Atlas approval. There is no community submission tier; entries are added by
merging a PR.
...
the atlas-agent repo, so Usama Aslam has reviewed each entry before it shipped —
...
The MCP server reads conversation data directly from Atlas's session store (`~/.atlas/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `atlas send` CLI.
