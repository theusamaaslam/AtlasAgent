...
description: "Customize Atlas Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Atlas Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ATLAS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Atlas is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Atlas now seeds a default `SOUL.md` automatically in:

```text
~/.atlas/SOUL.md
```

More precisely, it uses the current instance's `ATLAS_HOME`, so if you run Atlas with a custom home directory, it will use:
...
- Atlas creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Atlas loads `SOUL.md` only from `ATLAS_HOME`
- Atlas does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Atlas falls back to a built-in default identity
...
If Atlas loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ATLAS_HOME`, the personality belongs to the Atlas instance itself.

That also makes it easier to teach users:
- "Edit `~/.atlas/SOUL.md` to change Atlas' default personality."
...
- how Atlas should handle uncertainty, disagreement, or ambiguity
...
## What Atlas injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Atlas falls back to a built-in default identity ("You are Atlas Agent, an intelligent AI assistant created by Usama Aslam..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Atlas ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Atlas, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Atlas its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.atlas/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.atlas/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-atlas)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Atlas speaks
- `display.skin` and `/skin` affect how Atlas looks in the terminal
