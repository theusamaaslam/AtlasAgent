...
description: "Use Atlas Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains"
---

# ACP Editor Integration

Atlas Agent can run as an ACP server, letting ACP-compatible editors talk to Atlas over stdio and render:
...
ACP is a good fit when you want Atlas to behave like an editor-native coding agent instead of a standalone CLI or messaging bot.
...
Install Atlas normally, then add the ACP extra:
...
For Zed registry installs, Zed launches Atlas through the official ACP Registry entry. That entry uses a `uvx` distribution that runs:

```bash
uvx --from 'atlas-agent[acp]==<version>' atlas-acp
```
...
This is the standalone command. The Zed registry's terminal-auth flow (`atlas acp --setup`) also offers the browser bootstrap as a follow-up question after model selection, so most users never need to run `--setup-browser` directly.
...
3. Search for **Atlas Agent**.
4. Install it and start a new Atlas external-agent thread.

Prerequisites:

- Configure Atlas provider credentials first with `atlas model`, or set them in `~/.atlas/.env` / `~/.atlas/config.yaml`.
- Install `uv` so the registry launcher can run `uvx --from 'atlas-agent[acp]==<version>' atlas-acp`.
...
```text
/path/to/atlas-agent/acp_registry
```

## Registry manifest

The source copy of Atlas' official ACP Registry metadata lives at:
...
The upstream registry PR copies those files into the top-level `atlas-agent/` directory in `agentclientprotocol/registry`.

The registry entry uses a `uvx` distribution that points directly at the `atlas-agent` PyPI release:
...
Provider resolution uses Atlas' normal runtime resolver, so ACP inherits the currently configured provider and credentials. Atlas also advertises a terminal auth method (`--setup`) for first-run registry clients; this opens Atlas' interactive model/provider setup.
...
- In Zed, open the ACP Registry with `zed: acp registry` and search for **Atlas Agent**.
- For manual/local development, verify the custom `agent_servers` command points to `atlas acp`.
...
or by editing `~/.atlas/.env`. Registry clients can also trigger Atlas' terminal auth flow, which runs the same interactive provider/model setup.

### Zed registry launcher cannot find uv

Install `uv` from the official uv installation docs, then retry the Atlas Agent thread from Zed.
