...
The web dashboard is a browser-based UI for managing your Atlas Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Atlas Gateway OAuth; if you also want the dashboard to talk to a real backend, `atlas setup --portal` wires up the model and tool gateway too. See [Atlas Gateway](/integrations/nous-portal).
:::
...
```bash
# Custom port
atlas dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
atlas dashboard --host 0.0.0.0

# Start without opening browser
atlas dashboard --no-open
```
...
child with the selected profile's `ATLAS_HOME`, so the conversation runs
...
processes (manage them via `atlas -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `atlas-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.atlas/atlas-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.atlas/atlas-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `atlas dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `atlas dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `atlas --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.atlas/atlas-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
ExecStart=/path/to/venv/bin/python -m atlas_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.atlas/.env` containing:

```bash
ATLAS_DASHBOARD_BASIC_AUTH_USERNAME=admin
ATLAS_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
ATLAS_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `atlas config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Atlas instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `atlas profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.atlas/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `atlas skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `atlas mcp` reads from.
...
**Catalog:** browse the Atlas-approved MCP servers (the bundled `optional-mcps/`
...
`atlas mcp catalog` / `atlas mcp install` use.
...
Connect Atlas to any messaging platform from the browser — full parity with
`atlas setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.atlas/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Atlas versions, CPU core count + utilization, memory, disk usage of the Atlas home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Atlas version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `atlas update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Atlas Gateway** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `atlas portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `atlas curator`.
...
![System admin page — host stats and Atlas Gateway status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.atlas/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`ATLAS_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/mcp/catalog` | Browse the Atlas-approved MCP catalog |
...
| `GET /api/atlas/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Atlas Gateway auth + Tool Gateway routing (read-only) |
...
|-------|-----------|----------|
| `atlas dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `atlas dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
|---------|-----------|--------|----------------|
| `ATLAS_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `atlas dashboard register` |

Per the Atlas Agent convention (`~/.atlas/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: ATLAS_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). The Atlas Gateway
    provisions this env var (shape 'agent:{instance_id}') when it
    deploys a Atlas Agent instance — set it to your provisioned
...
**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Usama Aslam** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://atlas.example.com/auth/callback` and set `ATLAS_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `ATLAS_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `ATLAS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `ATLAS_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `ATLAS_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `ATLAS_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.atlas/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.atlas/.env <<EOF
ATLAS_DASHBOARD_BASIC_AUTH_USERNAME=admin
ATLAS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
ATLAS_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.atlas/.env
```
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no Atlas Gateway involved. It's verified against and works with any conformant OIDC server:
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/atlas/   # required
      client_id: atlas-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `ATLAS_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `ATLAS_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `ATLAS_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-atlas.json` — it defines a `atlas` realm, a **public PKCE client** (`atlas-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "atlas-dashboard",
      "name": "Atlas Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-atlas.json:/opt/keycloak/data/import/realm-atlas.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/atlas/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/atlas`). The admin console is at
...
```bash
export ATLAS_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/atlas"
export ATLAS_DASHBOARD_OIDC_CLIENT_ID="atlas-dashboard"
export ATLAS_DASHBOARD_PUBLIC_URL="http://localhost:9119"
atlas dashboard --host 0.0.0.0 --port 9119 --no-open
```

`ATLAS_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → atlas-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `ATLAS_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/atlas"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `ATLAS_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `ATLAS_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `atlas_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `atlas_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `atlas_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$ATLAS_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
from atlas_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `ATLAS_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
ATLAS_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  atlas dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.atlas/.env (secrets file, 0600).
cat >> ~/.atlas/.env <<'EOF'
ATLAS_DASHBOARD_BASIC_AUTH_USERNAME=admin
ATLAS_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
atlas dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `ATLAS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.atlas/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.atlas/.env`, so the credentials are in the environment at boot.

:::warning
The dashboard reads and writes your `.env` (API keys, secrets) and can run agent commands. The **username/password** setup shown here is for a trusted network — never expose a password-protected dashboard directly to the open internet. Put it behind a VPN. [Tailscale](https://tailscale.com/) is the clean option: bind to the machine's tailscale IP (`--host <tailscale-ip>`) and use `http://<tailscale-ip>:9119` as the Remote URL. Only devices on your tailnet can reach it. To reach a backend over the public internet, use the **OAuth (Atlas Gateway)** provider instead.
:::
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/atlas` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `ATLAS_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `ATLAS_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `ATLAS_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `ATLAS_DASHBOARD_BASIC_AUTH_USERNAME` / `ATLAS_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `ATLAS_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `atlas_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `atlas update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `atlas dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Atlas Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Atlas Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
