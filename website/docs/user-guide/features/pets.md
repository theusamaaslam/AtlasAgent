...
Atlas can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ATLAS_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Atlas renders the sprite at full fidelity when your
...
atlas pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `atlas pets list [query] [--limit N]` |
| List installed pets | `atlas pets list --installed` |
| Install a pet | `atlas pets install <slug> [--select] [--force]` |
...
| Preview/animate | `atlas pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `atlas pets off` |
| Remove an installed pet | `atlas pets remove <slug>` |
| Diagnose setup | `atlas pets doctor` |

`atlas pets show` flags:
...
Beyond installing pre-made pets from the gallery, Atlas can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `atlas pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Atlas state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Atlas Gateway**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Nous run a quality-first model chain by default.

- Resolution order prefers Atlas Gateway → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `atlas tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ATLAS_PET_IMAGE_PROVIDER` env var (e.g. `ATLAS_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Atlas is
...
Run `atlas pets doctor` — it reports:
