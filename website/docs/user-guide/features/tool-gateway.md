---
title: "Atlas Tool Gateway"
description: "One subscription, every tool. Web search, image generation, TTS, and cloud browsers — all routed through Atlas Gateway with no extra API keys."
...
The Tool Gateway is included with every paid [Atlas Gateway](https://portal.nousresearch.com) subscription. It routes Atlas' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let Atlas default to FLUX 2 Klein. |
...
All four are pay-as-you-use billed against your Atlas Gateway subscription. Use any combination — run the gateway for web and images while keeping your own ElevenLabs key for TTS, or route everything through Nous.
...
- **One key.** Your Atlas Gateway OAuth covers every tool.
...
```bash
atlas setup --portal     # Fresh install: Nous OAuth + set Nous as provider + turn on the Tool Gateway in one go
```

```bash
atlas model              # Switch your inference provider to Atlas Gateway — Atlas then offers to turn on the gateway for all tools
```

```bash
atlas tools              # Enable the gateway per-tool — pick "Nous Subscription" for any tool you want
```

`atlas setup --portal` and `atlas model` are the all-at-once paths: log in once, optionally flip every tool to the gateway. `atlas tools` is the à la carte path — turn on just the tools you want, one at a time.

**You don't have to log in first.** With `atlas tools`, the Nous-managed backends (Web search, Image, Video, TTS, Browser) are always listed, even if you've never signed into Atlas Gateway. Select one and Atlas runs the Portal login right there if you aren't already authenticated — no need to run `atlas model` beforehand. If your Nous OAuth is already active, selecting the backend enables it immediately with no extra prompt. This path only logs you in and turns on the one tool you picked — it does **not** switch your inference provider, and it does **not** prompt you to enable the gateway for every other tool.

Check what's active at any time:

```bash
atlas portal info        # Portal auth + Tool Gateway routing summary
atlas portal tools       # Gateway catalog with current routing per tool
atlas status             # Full system status (Tool Gateway is one section)
```

`atlas portal info` shows a section like:

```
◆ Atlas Tool Gateway
  Atlas Gateway     ✓ managed tools available
  Web tools       ✓ active via Atlas Gateway subscription
  Image gen       ✓ active via Atlas Gateway subscription
  TTS             ✓ active via Atlas Gateway subscription
  Browser         ○ active via Browser Use key
```

Tools marked "active via Atlas Gateway subscription" are going through the gateway. Anything else is using your own keys.
...
```bash
atlas tools          # Interactive picker for each tool category
```

Select the tool, pick **Nous Subscription** as the provider (or any direct provider you prefer). No config editing required. If you aren't logged into Atlas Gateway yet, picking **Nous Subscription** kicks off the Portal login inline — you don't need to authenticate through `atlas model` first.
...
The set evolves — `atlas tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `atlas model` and `atlas tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # Atlas now uses FIRECRAWL_API_KEY from .env
```

`atlas tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.atlas/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `atlas tools`. Atlas shows a clear error pointing at the portal.

### Can I see usage or costs per tool?

Yes — the [Atlas Gateway dashboard](https://portal.nousresearch.com) breaks usage down by tool so you can see what's driving your bill.

### Is Modal (serverless terminal) included?

Modal is available as an **optional add-on** through the Atlas Gateway subscription, not part of the default Tool Gateway bundle. Configure it via `atlas setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. When `use_gateway: true`, Atlas skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
