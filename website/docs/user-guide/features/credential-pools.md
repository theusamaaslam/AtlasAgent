...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Atlas automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Atlas Gateway](/integrations/nous-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Atlas auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:
...
Run `atlas auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `atlas auth` | Interactive pool management wizard |
| `atlas auth list` | Show all pools and credentials |
| `atlas auth list <provider>` | Show a specific provider's pool |
| `atlas auth add <provider>` | Add a credential (prompts for type and key) |
| `atlas auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `atlas auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `atlas auth remove <provider> <index>` | Remove credential by 1-based index |
| `atlas auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `atlas auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `atlas model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.
...
Atlas automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Atlas PKCE OAuth | `~/.atlas/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `atlas auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `atlas auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Atlas can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Atlas-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`atlas_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`atlas_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.atlas/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Atlas' credential store, so its token remains persistable.
