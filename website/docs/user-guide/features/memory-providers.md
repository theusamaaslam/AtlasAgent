...
Atlas Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
atlas memory setup      # interactive picker + configuration
atlas memory status     # check what's active
atlas memory off        # disable external provider
```

You can also select the active memory provider via `atlas plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.atlas/config.yaml`:
...
When a memory provider is active, Atlas automatically:
...
**Setup Wizard:**
```bash
atlas memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `atlas honcho`
If you previously used `atlas honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Atlas profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Atlas profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Atlas profile. Host key `atlas` → default; `atlas.<profile>` for others. |
...
```bash
atlas profile create coder --clone
```

`--clone` creates a `atlas.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Atlas profile, creates host blocks for any profile without one, inherits settings from the default `atlas` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `atlas memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
atlas memory setup    # select "openviking"
# Or manually:
atlas config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.atlas/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.atlas/.env
```
...
`OPENVIKING_AGENT` is Atlas' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
atlas memory setup    # select "mem0" → "Platform"
# Or manually:
atlas config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.atlas/.env
```

**Setup (OSS):**
```bash
atlas memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
atlas memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
atlas memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$ATLAS_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.atlas/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `atlas-user` | User identifier |
| `agent_id` | `atlas` | Agent identifier |
...
**Switching modes:** Re-run `atlas memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
atlas memory setup    # select "hindsight"
# Or manually:
atlas config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.atlas/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p atlas ui start`

**Config:** `$ATLAS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `atlas` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Atlas Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
atlas memory setup    # select "holographic"
# Or manually:
atlas config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.atlas-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ATLAS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
atlas memory setup    # select "retaindb"
# Or manually:
atlas config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.atlas/.env
```
...
- Knowledge tree stored at `$ATLAS_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=atlas) |
...
**Setup:**
```bash
atlas memory setup    # select "supermemory"
# Or manually:
atlas config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.atlas/.env
```

**Config:** `$ATLAS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `atlas` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `atlas-{identity}` → `atlas-coder`) to isolate memories per Atlas profile
...
| **Requires** | `pip install atlas-memori` + `atlas-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install atlas-memori
atlas-memori install
atlas config set memory.provider memori
atlas memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `atlas-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ATLAS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ATLAS_HOME/` so each profile has its own credentials
