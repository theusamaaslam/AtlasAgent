...
When Atlas Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.atlas/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.atlas/config.yaml` (switchable per session
via `/personality`).
...
If you use Atlas' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.atlas/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Atlas Agent's deliverable
