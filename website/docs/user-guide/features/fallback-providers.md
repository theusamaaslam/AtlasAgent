...
| Atlas Gateway | `nous` | `atlas setup --portal` (fresh) or `atlas auth add nous` (OAuth) |
...
**Atlas Gateway as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
```text
OpenRouter → Atlas Gateway → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → Atlas Gateway →
