...
description: "Durable SQLite-backed task board for coordinating multiple Atlas profiles"
---
...
Atlas Kanban is a durable task board, shared across all your Atlas profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.atlas/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.atlas/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `atlas kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `atlas kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/atlas-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `ATLAS_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
atlas kanban boards list

# Create a new board.
atlas kanban boards create atm10-server \
    --name "ATM10 Server" \
...
atlas kanban --board atm10-server list
atlas kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
atlas kanban boards switch atm10-server
atlas kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
atlas kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `ATLAS_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.atlas/kanban/current` — the slug persisted by `atlas kanban
   boards switch`.
4. `default`.
...
`atlas dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<atlas-home>/kanban/attachments/<task_id>/` for the default board, or
  `<atlas-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `ATLAS_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
atlas kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `atlas kanban show t_abcd`.
...
Override the config flag at runtime via `ATLAS_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `atlas gateway
...
until one comes up — `atlas kanban create` warns about this at creation
time.

Running `atlas kanban daemon` as a separate process is **deprecated**;
...
atlas kanban create "nightly ops review" \
    --assignee ops \
...
```bash
atlas kanban complete t_abc t_def t_hij --result "batch wrap"
atlas kanban archive  t_abc t_def t_hij
atlas kanban unblock  t_abc t_def
atlas kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `atlas kanban`.** When the dispatcher spawns a worker it sets `ATLAS_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `atlas kanban` CLI.
...
kanban_show()                                     # no args — uses ATLAS_KANBAN_TASK
...
### Why tools instead of shelling to `atlas kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `atlas kanban complete` *inside* the container, where `atlas` isn't installed and `~/.atlas/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.atlas/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `atlas chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `ATLAS_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/atlas_cli/test_kanban_db.py -q"],
...
2. `cd $ATLAS_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
atlas kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

atlas kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `atlas skills list` to see what's available); there's no runtime install.
...
```bash
atlas kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `atlas kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `atlas kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `atlas profile create --description "..."`, `atlas profile describe <name> --text "..."`, `atlas profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.atlas/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `atlas profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.atlas/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `atlas dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.atlas/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `atlas -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard Atlas dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
atlas kanban init                                     # create kanban.db + print daemon hint
atlas kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
atlas kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
atlas kanban show <id> [--json]
atlas kanban assign <id> <profile>                    # or 'none' to unassign
atlas kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
atlas kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
atlas kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
atlas kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
atlas kanban diagnostics [--json]                     # board health snapshot (alias: diag)
atlas kanban link <parent_id> <child_id>
atlas kanban unlink <parent_id> <child_id>
atlas kanban claim <id> [--ttl SECONDS]
atlas kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
atlas kanban complete <id>... [--result "..."]
atlas kanban block <id> "<reason>" [--ids <id>...]
atlas kanban unblock <id>...
atlas kanban archive <id>...

atlas kanban tail <id>                                # follow a single task's event stream
atlas kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
atlas kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
atlas kanban runs <id> [--json]                       # attempt history (one row per run)
atlas kanban assignees [--json]                       # profiles on disk + per-assignee task counts
atlas kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
atlas kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `atlas gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
atlas kanban stats [--json]                           # per-status + per-assignee counts
atlas kanban log <id> [--tail BYTES]                  # worker log from ~/.atlas/kanban/logs/
atlas kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
atlas kanban notify-list [<id>] [--json]
atlas kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
atlas kanban context <id>                             # what a worker sees
atlas kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
atlas kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
atlas kanban create "nightly backup audit" \
...
Every `atlas kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `atlas chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `atlas_cli.kanban.run_slash()` entry point that reuses the `atlas kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `atlas kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.atlas/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`atlas kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `atlas kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/atlas-kanban-v1-spec.pdf`.
...
```bash
atlas kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$ATLAS_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
atlas kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
atlas kanban notify-list
atlas kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
atlas kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
atlas kanban runs t_abcd
...
**Bulk close caveat.** `atlas kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`atlas kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `atlas kanban heartbeat $TASK` to signal liveness during long operations. |
...
`atlas kanban tail <id>` shows these for a single task. `atlas kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.atlas/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/atlas-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
