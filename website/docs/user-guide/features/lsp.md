...
Atlas runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Atlas
...
1. Atlas captures a baseline of current diagnostics for the file.
...
…). Atlas auto-detects the binary on PATH or in
`<ATLAS_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Atlas at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ATLAS_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`atlas lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Atlas installs both packages together when you
run `atlas lsp install typescript` or auto-install fires on first
use.

## CLI

```
atlas lsp status          # service state + per-server install status
atlas lsp list            # registry, optionally --installed-only
atlas lsp install <id>    # eagerly install one server
atlas lsp install-all     # try every server with a known recipe
atlas lsp restart         # tear down running clients
atlas lsp which <id>      # print resolved binary path
```

`atlas lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ATLAS_HOME>/lsp/bin
...
When `install_strategy: auto`, Atlas installs binaries into
`<ATLAS_HOME>/lsp/bin/`. NPM packages land in
`<ATLAS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Atlas-owned and is
...
Servers are kept alive for the life of the Atlas process. There's
...
**`atlas lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ATLAS_HOME>/lsp/bin/`. Run
`atlas lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `atlas lsp status`**
...
When `atlas lsp status` shows a `Backend warnings` section, install
...
`~/.atlas/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.atlas/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `atlas lsp restart` to clear the set;
