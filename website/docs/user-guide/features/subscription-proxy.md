...
description: "Use your Atlas Gateway subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Atlas-managed provider subscription as their
...
| Use case | "Use Atlas as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Atlas Gateway OAuth flow. Atlas stores
the refresh token in `~/.atlas/auth.json` — the same place all Atlas
...
```
Starting Atlas proxy for Atlas Gateway
...
Model:      Atlas-4-70B    # or Atlas-4.3-36B, Atlas-4-405B
```
...
```bash
atlas proxy providers
```

Currently shipped: `nous` (Atlas Gateway) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `atlas_cli/proxy/adapters/`.
...
```
Atlas proxy upstream adapters

  [nous    ] Atlas Gateway — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `atlas portal`. If you see
...
INFERENCE_TEXT_MODEL=Atlas-4-70B
```
...
```bash
atlas proxy start --host 0.0.0.0 --port 8645
```
...
`atlas_cli/proxy/adapters/<provider>.py` and registering it in
