---
sidebar_position: 1
title: "Run Atlas Agent with Atlas Gateway"
description: "Start-to-finish walkthrough: subscribe, set up, switch models, enable gateway tools, and verify routing"
---

# Run Atlas Agent with Atlas Gateway

This guide walks you through running Atlas Agent on a [Atlas Gateway](https://portal.nousresearch.com) subscription end to end — from signing up to verifying that every tool routes correctly. If you just want the overview of what the Portal is and what's in the subscription, see the [Atlas Gateway integration page](/integrations/nous-portal). This page is the task script.

## Prerequisites

- Atlas Agent installed ([Quickstart](/getting-started/quickstart))
...
3. Sets `model.provider: nous` in `~/.atlas/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Atlas is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
atlas setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
atlas auth add nous --type oauth
# Then re-run `atlas setup --portal` to wire the provider + gateway
```
...
If any line shows something other than "via Atlas Gateway" or the auth line says "not logged in", jump to [Troubleshooting](#troubleshooting) below.
...
```
Hey, search the web for "Atlas Agent release notes" and summarize the top 3 hits.
```

You should see Atlas call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`atlas setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
atlas config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Atlas-4 for agent work

Atlas-4-70B and Atlas-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Atlas Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Atlas-side opinion.
...
These rows appear in `atlas tools` even before you've logged into Atlas Gateway — if you pick "Nous Subscription" without an active session, Atlas runs the Portal login inline (without changing your inference provider or your other tools).
...
You'll see per-tool routing — `via Atlas Gateway` for the ones routed through the subscription, and the partner name (`browserbase`, `firecrawl`, etc.) for the ones using your own keys.
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Atlas will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
atlas cron create "every day at 9am" \
...
If you use [Atlas profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.atlas/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `atlas portal info` shows "not logged in" after `atlas setup --portal`
...
```bash
atlas config set model.provider nous
```
...
Re-verify with `atlas portal info`.

### Tool Gateway tools showing partner names instead of "via Atlas Gateway"
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Atlas doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/theusamaaslam/AtlasAgent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`atlas portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Atlas profiles where you're using the wrong one (check `atlas profile list`)

### Want to revoke and start clean

```bash
atlas auth logout nous       # wipes the local refresh token
...
- **[Atlas Gateway integration page](/integrations/nous-portal)** — Overview of what's in the subscription
- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway-routed tool
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Atlas tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Atlas configurations
