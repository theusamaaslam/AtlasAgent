---
sidebar_position: 8
title: "Use Voice Mode with Atlas"
description: "A practical guide to setting up and using Atlas voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Atlas
...
:::tip
[Atlas Gateway](/integrations/nous-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Atlas sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Atlas.
...
## Step 1: make sure normal Atlas works first
...
Atlas supports both local and cloud speech stacks.
...
### If you use `atlas setup`

If you choose NeuTTS in the setup wizard, Atlas checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Atlas transcribes and responds
...
- asking Atlas to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Atlas loop.

## Tuning CLI behavior

### Silence threshold

If Atlas starts/stops too aggressively, tune:
...
Atlas stays a normal chat bot, but can speak replies.
...
- you want Atlas to function like a portable research or ops assistant
...
Atlas joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Atlas detects speech boundaries
- transcripts are posted in the associated text channel
- Atlas responds in text and audio
...
1. get text Atlas working
2. install `atlas-agent[voice]`
