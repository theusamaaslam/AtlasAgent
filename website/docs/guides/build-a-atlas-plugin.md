...
title: "Build a Atlas Plugin"
description: "Step-by-step guide to building a complete Atlas plugin with tools, hooks, data files, and skills"
---

# Build a Atlas Plugin

This guide walks through building a complete Atlas plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Atlas has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `ATLAS_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.atlas/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `atlas skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.atlas/plugins/calculator
cd ~/.atlas/plugins/calculator
```
...
This tells Atlas: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Atlas may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `atlas my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Atlas continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `ATLAS_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
ATLAS_PLUGINS_DEBUG=1 atlas plugins list
```
...
The same logs are always written to `~/.atlas/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
atlas logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `atlas plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.atlas/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.atlas/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.atlas/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.atlas/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `atlas plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Atlas into installing arbitrary packages — only specs Atlas itself ships are eligible |
...
auditable in `~/.atlas/logs/agent.log`. Plugins load after built-in
...
- **The system prompt is Atlas's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `atlas <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for atlas my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: atlas my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for atlas my-plugin."""
...
After registration, users can run `atlas my-plugin status`, `atlas my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `atlas name` in a terminal |
...
- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `ATLAS_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `atlas <subcommand>` (e.g. `atlas kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "atlas kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Atlas from a hook.
...
Atlas has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.atlas/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `atlas gateway status` without SDK instantiation.
...
Atlas also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Atlas without any Python plugin. Declare them in `~/.atlas/config.yaml`:
...
Atlas connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.atlas/hooks/<name>/`:

```yaml
# ~/.atlas/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.atlas/hooks/long-task-alert/handler.py
...
atlas skills search my-workflow --source myorg/skills-repo
atlas skills install myorg/skills-repo/my-workflow
```
...
For STT, point `ATLAS_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```toml
# pyproject.toml
[project.entry-points."atlas_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install atlas-plugin-calculator
# Plugin auto-discovered on next atlas startup
```
...
```python
# Wrong — will break if Atlas passes extra context
def handler(args):
    ...
