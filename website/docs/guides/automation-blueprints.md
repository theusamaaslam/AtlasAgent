...
Copy-paste blueprints for common automation patterns. Each blueprint uses Atlas's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`atlas webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `atlas webhook subscribe`) |
...
```bash
atlas cron create "0 2 * * *" \
  "You are a project manager triaging the theusamaaslam/AtlasAgent GitHub repo.

1. Run: gh issue list --repo theusamaaslam/AtlasAgent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
atlas webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
atlas cron create "0 9 * * 1" \
  "Scan the theusamaaslam/AtlasAgent repo for documentation drift.

1. Run: gh pr list --repo theusamaaslam/AtlasAgent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (atlas_cli/commands.py, atlas_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (atlas_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
atlas cron create "0 6 * * *" \
  "Run a dependency security audit on the atlas-agent project.

1. cd ~/.atlas/atlas-agent && source .venv/bin/activate
...
```bash
atlas webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
atlas webhook subscribe alert-triage \
...
```python title="~/.atlas/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Atlas-Monitor/1.0"})
...
atlas cron create "every 30m" \
  ...
  --script ~/.atlas/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
atlas cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Atlas Agent development." \
...
```bash
atlas webhook subscribe github-issues \
  --events "issues" \
...
```bash
atlas webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
atlas webhook subscribe stripe-payments \
...
```bash
atlas cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the atlas-agent codebase.
