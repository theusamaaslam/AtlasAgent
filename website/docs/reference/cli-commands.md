...
| `atlas portal` | Atlas Gateway status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `atlas tools` | Configure enabled tools per platform. |
| `atlas computer-use` | Install or check the cua-driver backend (macOS Computer Use). |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Atlas itself. |
...
- log into OAuth-backed providers (Anthropic, Copilot, Codex, Atlas Gateway)
...
**Easiest path:** `atlas setup --portal` — OAuth into Atlas Gateway and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--portal` | One-shot Atlas Gateway setup: log in via OAuth, set Nous as the inference provider, and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md). Skips the rest of the wizard. |
...
Inspect Atlas Gateway auth, Tool Gateway routing, and reach the subscription page. Subcommand-less invocation runs `status`.
...
| Subcommand | Description |
|------------|-------------|
| *(none)* or `picker` | Interactive catalog picker — browse Atlas-approved MCPs and install/enable/disable. |
| `catalog` | List Atlas-approved MCPs (plain text, scriptable). |
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Atlas Plugin](../guides/build-a-atlas-plugin.md).
...
Register this install as a self-hosted dashboard with your Atlas Gateway account. Creates an OAuth client, writes `ATLAS_DASHBOARD_OAUTH_CLIENT_ID` into `~/.atlas/.env`, and prints how to engage the login gate. Requires being logged in (`atlas setup`).
...
| `--portal-url` | Override the Atlas Gateway base URL for registration (default: the portal you logged into). Also settable via `ATLAS_DASHBOARD_PORTAL_URL`. |
...
| `--backup` | Create a labeled pre-update snapshot of `ATLAS_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `updates.pre_update_backup: true` in `config.yaml`. |
...
| `atlas postinstall` | Internal bootstrap. Runs once after the install script provisions Atlas (or after `atlas update`) to install non-Python dependencies that pip cannot provide — Node.js runtime, headless browser, ripgrep, ffmpeg — and then trigger `atlas setup` if the profile has not been configured yet. Safe to re-run idempotently. |
| `atlas uninstall [--full] [--gui] [--yes]` | Remove Atlas, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--yes` skips prompts. |
