...
description: "Run Atlas Agent directly on an Android phone with Termux"
---

# Atlas on Android with Termux
...
Atlas Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Atlas from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `atlas` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `atlas` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/atlas" "$PREFIX/bin/atlas"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `atlas` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.atlas/.env`.
...
### `atlas doctor` says ripgrep or Node is missing
