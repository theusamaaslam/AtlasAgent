...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Atlas Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Atlas tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Atlas core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Atlas Plugin](/guides/build-a-atlas-plugin)
...
2. **`toolsets.py`** — add tool name to `_ATLAS_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `atlas_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `atlas_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `atlas chat -q "Use the weather tool for London"`
