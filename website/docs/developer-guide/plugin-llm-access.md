...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Atlas already has applies. The plugin never sees a
...
[`atlas-example-plugins`](https://github.com/NousResearch/atlas-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
atlas-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/atlas-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Atlas provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Atlas provider
        # (must be one Atlas already knows about — same names as
        # `atlas model` and config.yaml model.provider).
        ...
        # plugin sends — Atlas does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.atlas/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Atlas' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Atlas subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/theusamaaslam/AtlasAgent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/theusamaaslam/AtlasAgent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/atlas-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/atlas-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
