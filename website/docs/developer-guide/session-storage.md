...
Atlas Agent uses a SQLite database (`~/.atlas/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `atlas_state.py`


## Architecture Overview

```
~/.atlas/state.db (SQLite, WAL mode)
...
Multiple atlas processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.atlas/state.db
...
Default path: `~/.atlas/state.db`

This is derived from `atlas_constants.get_atlas_home()` which resolves to
`~/.atlas/` by default, or the value of `ATLAS_HOME` environment variable.
