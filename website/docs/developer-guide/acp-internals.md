...
The ACP adapter wraps Atlas' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
atlas acp / atlas-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.atlas/.env
  -> configure stderr logging
  -> construct AtlasACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'atlas-agent[acp]==<version>' atlas-acp`, pointed at the `atlas-agent` PyPI release.
...
- `allow_once` -> Atlas `once`
- `allow_always` -> Atlas `always`
- reject options -> Atlas `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Atlas tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["atlas-acp"])
...
Instead it reuses Atlas' runtime resolver:

- `acp_adapter/auth.py`
- `atlas_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Atlas provider/credentials. It also always advertises a terminal setup auth method (`atlas-setup`, args `--setup`) so first-run registry clients can open Atlas' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.atlas/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `atlas-acp` toolset definition
- `atlas_cli/main.py` — `atlas acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `atlas-acp` script
