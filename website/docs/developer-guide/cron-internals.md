...
description: "How Atlas stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `atlas_cli/cron.py` | CLI `atlas cron` subcommands |
...
Jobs are stored in `~/.atlas/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `atlas cron` commands are run or during active CLI sessions.
...
```python
# ~/.atlas/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.atlas/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `atlas cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `atlas cron` CLI provides direct job management:

```bash
atlas cron list                    # Show all jobs
atlas cron create                  # Interactive job creation (alias: add)
atlas cron edit <job_id>           # Edit job configuration
atlas cron pause <job_id>          # Pause a running job
atlas cron resume <job_id>         # Resume a paused job
atlas cron run <job_id>            # Trigger immediate execution
atlas cron remove <job_id>         # Delete a job
```
