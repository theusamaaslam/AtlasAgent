...
sidebar_position: 1
title: "Architecture"
description: "Atlas Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Atlas Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ atlas_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AtlasCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── atlas_state.py           # SQLite session/state database with FTS5
├── atlas_constants.py       # ATLAS_HOME, profile-aware paths
...
│
├── atlas_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `atlas` subcommands (large file)
...
│   ├── skills_config.py      # atlas skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # atlas tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # atlas gateway start/stop
│
...
```text
User input → AtlasCLI.process_input()
...
Three discovery sources: `~/.atlas/plugins/` (user), `.atlas/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `atlas plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-atlas-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Atlas as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`atlas -p <name>`) gets its own ATLAS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
