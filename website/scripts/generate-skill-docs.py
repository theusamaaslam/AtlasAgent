...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/theusamaaslam/AtlasAgent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    atlas_meta = (fm.get("metadata") or {}).get("atlas") or {}
    tags = atlas_meta.get("tags") or []
    related = atlas_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `atlas skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Atlas Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Atlas ships with a large built-in skill library copied into `~/.atlas/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Atlas also syncs bundled skills on `atlas update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.atlas/skills/` tree, it is still shipped with Atlas; restore it with `atlas skills reset <name> --restore`.",
        "",
