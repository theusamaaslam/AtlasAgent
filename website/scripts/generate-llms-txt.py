#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Atlas docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Atlas as a Python Library", None),
        ("guides/use-mcp-with-atlas", "Use MCP with Atlas", None),
        ("guides/use-voice-mode-with-atlas", "Use Voice Mode with Atlas", None),
        ("guides/use-soul-with-atlas", "Use SOUL.md with Atlas", None),
        ("guides/build-a-atlas-plugin", "Build a Atlas Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Atlas"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Atlas Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Usama Aslam. A terminal-native "
        ...
        "backends. Works with Atlas Gateway, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/UsamaAslam/"
        "atlas-agent/main/scripts/install.sh | bash`  "
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Atlas Agent — Full Documentation\n",
        (
            "This file is the entire Atlas Agent documentation concatenated for LLM "
