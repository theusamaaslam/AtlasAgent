...
def test_manager_builds_atlas_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AtlasMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ATLAS_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ATLAS_PROVIDER_CLS is not None
    assert isinstance(provider, _ATLAS_PROVIDER_CLS)
    assert provider._atlas_server_name == "srv"
