...
def test_close_terminal_tool_gated_on_desktop(monkeypatch):
    """Hidden unless ATLAS_DESKTOP is set (mirrors read_terminal gating)."""
...
class TestSpawnEnvSanitization:
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/atlas_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/atlas_bg_" not in bg_command
        assert "cat /tmp/atlas_bg_" not in bg_command
