...
class TestWriteDenyExactPaths:
    ...
    def test_atlas_env(self):
        # ``.env`` under the active ATLAS_HOME (profile-aware, not just
        # ``~/.atlas``) must be write-denied. The hermetic test conftest
        # points ATLAS_HOME at a tempdir — resolve via get_atlas_home()
        # to match the denylist.
        from atlas_constants import get_atlas_home
        path = str(get_atlas_home() / ".env")
        assert _is_write_denied(path) is True
