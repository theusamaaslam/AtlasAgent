...
def _clear_terminal_env(monkeypatch):
    ...
    # Default: no Atlas Gateway subscription — patch both the terminal_tool local
...
def test_local_terminal_requirements(monkeypatch, caplog):
    """Local backend uses Atlas' own LocalEnvironment wrapper."""
...
def test_modal_backend_managed_mode_does_not_fall_back_to_direct(monkeypatch, caplog, tmp_path):
    ...
    assert ok is False
    assert any(
        "Atlas Tool Gateway access is not currently available" in record.getMessage()
        for record in caplog.records
    )
