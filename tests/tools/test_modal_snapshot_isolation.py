...
def _restore_tool_modules():
    original_atlas_home = os.environ.get("ATLAS_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_atlas_home is None:
            os.environ.pop("ATLAS_HOME", None)
        else:
            os.environ["ATLAS_HOME"] = original_atlas_home
        _reset_modules(("tools", "atlas_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    atlas_cli = types.ModuleType("atlas_cli")
    atlas_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["atlas_cli"] = atlas_cli
    atlas_home = tmp_path / "atlas-home"
    os.environ["ATLAS_HOME"] = str(atlas_home)
    sys.modules["atlas_cli.config"] = types.SimpleNamespace(
        get_atlas_home=lambda: atlas_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="atlas-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": atlas_home / "modal_snapshots.json",
