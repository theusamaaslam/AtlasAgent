...
class TestAtlasToolsGeneration(unittest.TestCase):
    ...
    def test_generates_subset(self):
        src = generate_atlas_tools_module(["terminal", "web_search"])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_atlas_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_atlas_tools_module(["terminal"])
        self.assertIn("ATLAS_RPC_SOCKET", src)
...
"""
        ...
        # terminal won't be in atlas_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from atlas_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from atlas_tools import json_parse
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `atlas tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `atlas tools code_execution,vision`.
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        code = (
            "from atlas_tools import terminal, web_search, read_file\n"
    ...
    def test_empty_enabled_tools_uses_all(self):
        ...
        code = (
            "from atlas_tools import terminal, web_search\n"
            "print('imports ok')\n"
        )
        with patch("model_tools.handle_function_call",
                    return_value=json.dumps({"ok": True})):
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_returns_code_execution_section(self):
        from tools.code_execution_tool import _load_config
        with patch("atlas_cli.config.read_raw_config",
