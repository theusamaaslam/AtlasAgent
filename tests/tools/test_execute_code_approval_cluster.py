...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ATLAS_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ATLAS_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_atlas_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ATLAS_HOME": "/h", "ATLAS_PROFILE": "p",
        "ATLAS_CONFIG": "/c.yaml", "ATLAS_ENV": "/e",
        # other ATLAS_* → dropped (broad prefix removed)
        "ATLAS_BASE_URL": "https://x", "ATLAS_INTERACTIVE": "1",
        "ATLAS_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ATLAS_HOME", "ATLAS_PROFILE", "ATLAS_CONFIG", "ATLAS_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ATLAS_BASE_URL", "ATLAS_INTERACTIVE", "ATLAS_KANBAN_DB",
...
def test_env_scrub_logs_dropped_atlas_vars(caplog):
    """Dropping a non-allowlisted, non-secret ATLAS_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ATLAS_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ATLAS_HOME": "/h",          # allowlisted → kept, not logged
        "ATLAS_BASE_URL": "https://x",   # dropped → logged
        "ATLAS_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ATLAS_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ATLAS_HOME" in out and "PATH" in out
    assert "ATLAS_BASE_URL" not in out and "ATLAS_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ATLAS_BASE_URL" in msgs and "ATLAS_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ATLAS_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ATLAS_* vars."""
    import logging
