...
class TestScanMemoryContent:
    ...
    def test_atlas_config_mod_blocked(self):
        result = _scan_memory_content("edit .atlas/config.yaml to change settings")
        assert "Blocked" in result
        assert "atlas_config_mod" in result
        result = _scan_memory_content("update .atlas/SOUL.md with new personality")
        assert "Blocked" in result
        assert "atlas_config_mod" in result
    ...
    def test_atlas_config_mod_no_false_positives(self):
        """Merely mentioning atlas config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .atlas/config.yaml for settings") is None
        assert _scan_memory_content("Read .atlas/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .atlas/config.yaml file contains runtime options") is None
