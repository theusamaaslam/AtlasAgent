...
def _chown_atlas_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    *,
    ...
    atlas_home = target if atlas_home is None else atlas_home
    script = (
        "set -eu\n"
        f'ATLAS_HOME="{atlas_home}"\n'
        f"{_chown_atlas_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_atlas_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "atlas-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    proc = _run_helper(
        ...
        linked_home / "cron",
        log_path,
        atlas_home=linked_home,
    )

    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked ATLAS_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_atlas_home_trees(stage2_text: str) -> None:
    assert 'chown_atlas_tree "$ATLAS_HOME/$sub"' in stage2_text
    assert 'chown_atlas_tree "$ATLAS_HOME/profiles"' in stage2_text
    assert 'chown_atlas_tree "$ATLAS_HOME/cron"' in stage2_text
    assert 'chown -R atlas:atlas "$ATLAS_HOME/$sub"' not in stage2_text
    assert 'chown -R atlas:atlas "$ATLAS_HOME/profiles"' not in stage2_text
    assert 'chown -R atlas:atlas "$ATLAS_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_atlas_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$ATLAS_HOME"' in stage2_text
