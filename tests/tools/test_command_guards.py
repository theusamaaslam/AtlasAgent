...
def _clean_state():
    ...
    saved = {}
    for k in ("ATLAS_INTERACTIVE", "ATLAS_GATEWAY_SESSION", "ATLAS_EXEC_ASK", "ATLAS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ATLAS_INTERACTIVE"] = "1"
        session_key = os.getenv("ATLAS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ATLAS_INTERACTIVE or ATLAS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
