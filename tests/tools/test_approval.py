...
class TestApprovalModeParsing:
    ...
    def test_string_off_still_maps_to_off(self):
        with mock_patch("atlas_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\atlas-victim\file.txt"
        )
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\atlas-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\atlas-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\atlas-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
...
class TestTeePattern:
    ...
    def test_tee_atlas_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.atlas/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_atlas_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ATLAS_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_atlas_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$ATLAS_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestAtlasConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.atlas/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.atlas/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.atlas/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.atlas/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.atlas/config.yaml")
        assert dangerous is True
        assert "atlas config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.atlas/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_atlas_home_config(self):
    ...
    def test_sed_in_place_absolute_atlas_home_env(self):
        env_path = get_atlas_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_atlas_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ATLAS_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.atlas/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_atlas_home_config(self):
        config_path = get_atlas_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.atlas/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_atlas_home_env(self):
        env_path = get_atlas_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.atlas/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.atlas/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.atlas/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.atlas/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.atlas/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Atlas config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_atlas_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $ATLAS_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    def test_cp_to_atlas_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.atlas/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    ...
    def test_windows_atlas_home_config_folds(self, monkeypatch):
        # Atlas home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("ATLAS_HOME", r"C:\Users\tester\.atlas")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.atlas\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m atlas_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m atlas_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "atlas_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_atlas_gateway_stop_detected(self):
        cmd = "atlas gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_atlas_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `atlas` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "atlas -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_atlas_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "atlas --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_atlas_gateway_multiple_flags_detected(self):
        cmd = "atlas -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_atlas_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "atlas -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_atlas_gateway_start_not_flagged(self):
        cmd = "atlas gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_atlas_detected(self):
        """pkill targeting atlas/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_atlas_detected(self):
        cmd = "killall atlas"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Atlas security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.atlas/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    ...
    def test_pkill_atlas_still_detected(self):
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_stop_atlas_detected(self):
        cmd = "launchctl stop ai.atlas.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "atlas" in desc.lower()

    def test_launchctl_kickstart_atlas_detected(self):
        cmd = "launchctl kickstart -k system/ai.atlas.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_atlas_detected(self):
        cmd = "launchctl bootout system/ai.atlas.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_atlas_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.atlas.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Atlas label is out of scope for the
