"""
Tests for mcp_serve — Atlas MCP server.
...
def _isolate_atlas_home(tmp_path, monkeypatch):
    """Redirect ATLAS_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking atlas_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_when_only_sessions_json_changed(
        ...
        # _poll_once reads <ATLAS_HOME>/state.db for its mtime gate; the autouse
        # fixture points ATLAS_HOME at tmp_path.
        db_path = tmp_path / "state.db"
