"""Tests for atlas_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Atlas entry point
(atlas, atlas-agent, atlas-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Atlas entry point imports atlas_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported atlas_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("atlas_bootstrap", None)
    import atlas_bootstrap  # noqa: WPS433
    return atlas_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Atlas CLI
...
class TestEntryPointsImportBootstrap:
    """Every Atlas entry point must import atlas_bootstrap as its
    ...
    # Entry points that invoke Atlas as a process.  Each one must
    # import atlas_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "atlas_cli/main.py",   # atlas CLI (console_script)
        "run_agent.py",          # atlas-agent (console_script)
        "acp_adapter/entry.py",  # atlas-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import atlas_bootstrap' and that
        ...
        half-finished ``atlas update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``atlas_bootstrap`` as a top-level module) leaves atlas
        ...
        """
        # Resolve relative to the atlas-agent repo root.  Tests live
        # at tests/test_atlas_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # atlas start even when atlas_bootstrap hasn't been
        ...
        assert first_import_name == "atlas_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'atlas_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import atlas_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Atlas's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ATLAS_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ATLAS_PYTHON_SRC_ROOT"] = env
            elif "ATLAS_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ATLAS_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/atlas")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ATLAS_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ATLAS_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/atlas", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Atlas root.  The guard must relocate Atlas to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/atlas"])
        assert result[0] == "/opt/atlas"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Atlas root.
        assert result.index("/opt/atlas") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/atlas", "/opt/atlas", ""])
        assert result.count("/opt/atlas") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ATLAS_PYTHON_SRC_ROOT"] = "/env/atlas"
            hb.harden_import_path()
            assert sys.path[0] == "/env/atlas"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ATLAS_PYTHON_SRC_ROOT", None)
