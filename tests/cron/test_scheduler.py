...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``atlas tools`` platform config for ``cron``
        ...
        # The cron default is _ATLAS_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``atlas tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        # Even if the user has ``atlas tools`` configured to enable web+file
        # for cron, the per-job override wins.
        extra = [patch("atlas_cli.tools_config._get_platform_tools", return_value={"web", "file"})]
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("ATLAS_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("ATLAS_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("ATLAS_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("ATLAS_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("ATLAS_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("ATLAS_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("ATLAS_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("ATLAS_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("ATLAS_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this ATLAS_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_atlas_dotenv`` (which then re-applies external secret sources).
        """
        ...
        with patch("cron.scheduler._atlas_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("atlas_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("atlas_cli.env_loader.load_atlas_dotenv", _record_load), \
             patch("atlas_state.SessionDB", return_value=fake_db), \
             patch(
                 "atlas_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("ATLAS_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("ATLAS_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("ATLAS_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            ...
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_ATLAS_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_ATLAS_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
...
class TestRunJobModelResolution:
    ...
    def test_explicit_null_model_block_in_config_does_not_overwrite_env(self, tmp_path, monkeypatch):
        """``model: null`` in config.yaml must not overwrite a resolved ATLAS_MODEL.
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._atlas_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_atlas_home", return_value=tmp_path), \
             patch("atlas_cli.env_loader.load_atlas_dotenv"), \
             patch("atlas_cli.env_loader.reset_secret_source_cache"), \
             patch("atlas_state.SessionDB", return_value=fake_db), \
             patch(
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """ATLAS_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("ATLAS_CRON_MAX_PARALLEL", "1")
        call_times = []
