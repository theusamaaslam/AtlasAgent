"""Runtime smoke test for Docker $ATLAS_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the atlas user after container boot.
...
registered later runs its log service as the dropped atlas user and
...
def test_logs_gateways_seeded_and_atlas_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by atlas after boot."""
    ...
    assert "logs=atlas" in r.stdout, (
        f"logs/ not owned by atlas: {r.stdout}"
    )
    assert "gateways=atlas" in r.stdout, (
        f"logs/gateways/ not owned by atlas: {r.stdout}"
    )
