...
closure the PR changed, against a real temp ``ATLAS_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "atlas_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "atlas_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_atlas_home() / config.yaml`` — point it here.
