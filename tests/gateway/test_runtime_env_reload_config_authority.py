...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    atlas_home = tmp_path / ".atlas"
    atlas_home.mkdir()
    (atlas_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (atlas_home / ".env").write_text("ATLAS_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["ATLAS_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("ATLAS_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["ATLAS_MAX_ITERATIONS"] = "200"
