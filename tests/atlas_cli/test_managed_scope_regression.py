...
def atlas_home(tmp_path, monkeypatch):
    ...
    # /etc/atlas on the dev/CI box can't influence the test.
    monkeypatch.setenv("ATLAS_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(atlas_home, monkeypatch):
...
def test_env_expansion_in_user_config(atlas_home, monkeypatch):
    from atlas_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(atlas_home):
