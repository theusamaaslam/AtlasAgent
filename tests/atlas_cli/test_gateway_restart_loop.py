...
Covers:
- Defense 1: gateway stop/restart refuse when _ATLAS_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.atlas.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.atlas.gateway.plist",
        "launchctl stop ai.atlas.gateway",
        "systemctl restart atlas-gateway",
        "systemctl stop atlas-gateway.service",
        "systemctl start atlas-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill atlas gateway process",
        "pkill -f atlas.*gateway",
        "pkill -f gateway.*atlas",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "atlas cron list",
        "atlas update",
        "atlas config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `atlas gateway start` is benign — starting a gateway from inside a
        ...
        "atlas gateway start",
        "atlas gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway atlas
        # services must not be falsely blocked (the old `.*atlas` matched any
        # atlas token).
        "launchctl unload ai.atlas.update-checker.plist",
        "launchctl restart ai.atlas.daemon",
        "systemctl restart atlas-meta.service",
        "systemctl restart atlas-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_atlas_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade atlas then run atlas gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.atlas.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_atlas_gateway_once.sh). The script must live under
        # ATLAS_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path / ".atlas"))
        scripts_dir = tmp_path / ".atlas" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify atlas gateway stop/restart refuse when _ATLAS_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ATLAS_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ATLAS_GATEWAY=1.

    Issue #37453: systemctl --user restart atlas-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart atlas-gateway",
        "systemctl --user restart atlas-gateway",
        "systemctl stop atlas-gateway.service",
        "atlas gateway restart",
        "launchctl kickstart gui/501/ai.atlas.gateway",
        "pkill -f atlas.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart atlas-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-atlas systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _ATLAS_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart atlas-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart atlas-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run atlas gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("atlas gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeatlas gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ATLAS_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.atlas.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    atlas_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run atlas gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        result = json.loads(cronjob(
            ...
            prompt="please run atlas gateway restart nightly",
        ))
