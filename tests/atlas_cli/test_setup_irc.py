"""Tests for IRC gateway configuration via `atlas setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Atlas install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Atlas install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#atlas")
            monkeypatch.setenv("IRC_NICKNAME", "atlas-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `atlas setup gateway` experience with IRC present."""
