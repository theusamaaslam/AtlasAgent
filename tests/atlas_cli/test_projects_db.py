"""Tests for the per-profile Projects store (atlas_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Atlas Agent", folders=["/tmp/atlas"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "atlas-agent"
    assert proj.name == "Atlas Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/atlas"
    assert [f.path for f in proj.folders] == ["/tmp/atlas"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "atlas-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Atlas Agent")
    pdb.create_project(conn, name="Atlas Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["atlas-agent", "atlas-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ATLAS_HOME.
...
def test_db_path_under_atlas_home():
    # Resolves under ATLAS_HOME (set by the autouse isolation fixture).
