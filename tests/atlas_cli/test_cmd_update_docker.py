...
from atlas_cli.main import _cmd_update_check, cmd_update
...
@patch("atlas_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``atlas update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``atlas update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_cmd_update_check_on_source_install_avoids_public_package_index(
    _mock_pypi, _mock_method, capsys
):
    """Source installs report their refresh command without querying PyPI."""
    _cmd_update_check()

    out = capsys.readouterr().out
    assert "Local source install detected" in out
    assert "doesn't apply inside the Docker container" not in out
    _mock_pypi.assert_not_called()
