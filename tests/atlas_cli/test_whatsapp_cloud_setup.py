...
from atlas_cli.setup_whatsapp_cloud import (
...
def isolated_home(tmp_path, monkeypatch):
    """Redirect ATLAS_HOME so save_env_value writes into a temp .env."""
    home = tmp_path / "home"
    atlas = home / ".atlas"
    atlas.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ATLAS_HOME", str(atlas))
...
def _env_value(atlas_home: Path, key: str) -> str | None:
