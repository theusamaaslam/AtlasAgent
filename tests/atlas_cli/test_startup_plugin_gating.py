...
``atlas_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``atlas --help``, ``atlas version``,
``atlas logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``atlas --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from atlas_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["atlas", "--tui", "chat"], "chat"),
        (["atlas", "-w", "logs"], "logs"),
        (["atlas", "chat", "hello world"], "chat"),
        (["atlas", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["atlas", "-m", "gpt5", "chat"], "chat"),
        (["atlas", "--model", "gpt5", "chat", "hi"], "chat"),
        (["atlas", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["atlas", "-z", "hello world"], None),
        (["atlas", "-z", "hello", "chat"], "chat"),
        (["atlas", "--model=gpt5", "chat"], "chat"),     # inline form
        (["atlas", "--", "chat"], "chat"),               # -- terminator
        (["atlas", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["atlas", "some-plugin-cmd"], "some-plugin-cmd"),
        (["atlas", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["atlas"],                          # bare → chat
        ["atlas", "--help"],                # top-level help
        ["atlas", "-h"],
        ["atlas", "version"],               # known built-in
        ["atlas", "logs"],
        ["atlas", "gateway", "run"],
        ["atlas", "--tui"],
        ["atlas", "-w", "--tui"],
        ["atlas", "chat", "hi"],
        ["atlas", "help"],                  # accepted built-in-ish
        ["atlas", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["atlas", "meet", "join"],          # potential google_meet plugin
        ["atlas", "honcho", "status"],      # potential memory plugin
        ["atlas", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``atlas help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"atlas_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
