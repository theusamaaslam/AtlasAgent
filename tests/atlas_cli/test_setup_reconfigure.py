...
- Bare `atlas setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `atlas setup --quick` runs the narrower "fill in missing items" flow.
- `atlas setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ...
        ("atlas_cli.config.is_managed", {"return_value": False}),
        ("atlas_cli.setup.load_config", {"return_value": {}}),
        ("atlas_cli.setup.save_config", {}),
        ("atlas_cli.setup.get_env_value", {"return_value": None}),
        ("atlas_cli.auth.get_active_provider", {"return_value": "openrouter"}),
...
class TestExistingInstallDefault:
    """Bare `atlas setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="atlas_cli.setup.prompt_choice",
                quick="atlas_cli.setup._run_quick_setup",
                model="atlas_cli.setup.setup_model_provider",
                terminal="atlas_cli.setup.setup_terminal_backend",
                agent="atlas_cli.setup.setup_agent_settings",
                gateway="atlas_cli.setup.setup_gateway",
                tools="atlas_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`atlas setup --reconfigure` behaves the same as bare `atlas setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("atlas_cli.setup.prompt_choice", {"return_value": 0}),
                first="atlas_cli.setup._run_first_time_quick_setup",
            )
            from atlas_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "atlas_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["atlas", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["atlas", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["atlas", "setup"])
        try:
            main()
        except SystemExit:
            pass
