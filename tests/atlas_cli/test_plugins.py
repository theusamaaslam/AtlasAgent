"""Tests for the Atlas plugin system (atlas_cli.plugins)."""
...
from atlas_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<atlas_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<atlas_home>/plugins/``; we derive
    ``<atlas_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ATLAS_HOME>/config.yaml.
        # Config is always read from ATLAS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        atlas_home_str = os.environ.get("ATLAS_HOME")
        if atlas_home_str:
            atlas_home = Path(atlas_home_str)
        else:
            atlas_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.atlas/plugins/ are discovered."""
        plugins_dir = tmp_path / "atlas_test" / "plugins"
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATLAS_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".atlas" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under atlas_plugins.<name>."""
        plugins_dir = tmp_path / "atlas_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path / "atlas_test"))

        # Clean up any prior namespace module
        sys.modules.pop("atlas_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "atlas_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="atlas_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("atlas_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("atlas_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from atlas_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from atlas_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("atlas_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `atlas plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("atlas_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        atlas_home = tmp_path / "atlas_test"
        plugins_dir = atlas_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "atlas_test" / "plugins"
        # Set ATLAS_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("atlas_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("atlas_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("atlas_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("atlas_cli.plugins.PluginContext.dispatch_tool.__module__", "atlas_cli.plugins"):
...
class TestPluginDebugLogging:
    """ATLAS_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ATLAS_PLUGINS_DEBUG", raising=False)
        from atlas_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ATLAS_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ATLAS_PLUGINS_DEBUG", "1")
        from atlas_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from ATLAS_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """ATLAS_HOME at the root resolves to 'default'."""
        home = tmp_path / ".atlas"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """ATLAS_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".atlas" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ATLAS_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".atlas" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ATLAS_HOME", str(prof))
        ctx = self._ctx()
