...
When ``ATLAS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.atlas/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.atlas            (has its own auth.json fixture)
    * Profile     -> tmp_path/.atlas/profiles/coder   (active, ATLAS_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Atlas is not
# logged into Atlas Gateway"`` even though the user has a valid global Nous
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from atlas_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    atlas_home = tmp_path / "classic"
    atlas_home.mkdir()
    monkeypatch.setenv("ATLAS_HOME", str(atlas_home))

    _write(atlas_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ATLAS_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.atlas as the "real home" — which is NOT equal
    # to the ATLAS_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    atlas_home = tmp_path / "classic"
    ...
    _write(atlas_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from atlas_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ATLAS_HOME is set to a custom path that is NOT under
    # ~/.atlas/profiles/ — get_default_atlas_root() returns ATLAS_HOME
