...
def test_valid_config_produces_no_warnings():
    cfg = {"cli": ["atlas-cli"], "telegram": ["atlas-telegram"]}
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # atlas-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "atlas-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "atlas-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "atlas-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["atlas"], "discord": ["atlas"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_atlas_cli_valid_and_atlas_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `atlas-cli` is the real toolset and `atlas` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("atlas-cli") is True
    assert validate_toolset("atlas") is False
    warnings = validate_platform_toolsets({"cli": ["atlas"]}, validate_toolset)
    assert any("did you mean 'atlas-cli'?" in w for w in warnings)
