"""Tests for atlas_cli.security_advisories.
...
import atlas_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ATLAS_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".atlas"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full atlas_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "atlas_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "atlas_cli.config.save_config",
