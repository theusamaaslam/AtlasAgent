"""Tests for the `atlas memory reset` CLI command.
...
- Profile-scoped reset (uses ATLAS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ATLAS_HOME with memory files."""
    atlas_home = tmp_path / ".atlas"
    memories = atlas_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ATLAS_HOME", str(atlas_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAtlas repo is at ~/.atlas/atlas-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `atlas memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        atlas_home = tmp_path / ".atlas"
        (atlas_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ATLAS_HOME", str(atlas_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ATLAS_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_atlas_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
