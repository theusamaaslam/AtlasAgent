...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("atlas_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_atlas_dev(container_env, monkeypatch):
    """Returns None when ATLAS_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ATLAS_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_atlas_dev_zero(container_env, monkeypatch):
    """ATLAS_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ATLAS_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        atlas_home = Path(tmpdir) / ".atlas"
        atlas_home.mkdir()
        (atlas_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("atlas_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_atlas_home": lambda: atlas_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ATLAS_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=atlas-custom\n"
        "exec_user=myuser\n"
        "atlas_bin=/opt/atlas/bin/atlas\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "atlas-custom"
    assert info["exec_user"] == "myuser"
    assert info["atlas_bin"] == "/opt/atlas/bin/atlas"
