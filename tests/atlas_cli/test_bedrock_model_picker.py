...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ATLAS_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ATLAS_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ATLAS_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from atlas_cli.providers import ATLAS_OVERLAYS
        assert "bedrock" in ATLAS_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from atlas_cli.providers import ATLAS_OVERLAYS
        assert ATLAS_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from atlas_cli.providers import ATLAS_OVERLAYS
        assert ATLAS_OVERLAYS["bedrock"].auth_type == "aws_sdk"
