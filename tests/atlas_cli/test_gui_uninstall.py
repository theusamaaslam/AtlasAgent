...
def test_agent_is_installed_detects_source_and_venv(tmp_path):
    ...
    atlas_home.mkdir()
    assert gu.agent_is_installed(atlas_home) is False
    _make_agent(atlas_home)
    assert gu.agent_is_installed(atlas_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    atlas_home = tmp_path / ".atlas"
    (atlas_home / "atlas-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(atlas_home) is True
...
def test_source_built_artifacts_lists_known_paths(tmp_path):
    ...
    artifacts = gu.source_built_gui_artifacts(atlas_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(atlas_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(atlas_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    atlas_home = tmp_path / ".atlas"
    agent_root = _make_agent(atlas_home)
    _make_gui_build(atlas_home)
    _make_user_data(atlas_home)
    ...
    removed = gu.uninstall_gui(atlas_home)
    ...
    assert not (atlas_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "atlas_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (atlas_home / "config.yaml").exists()
    assert (atlas_home / ".env").exists()
    assert (atlas_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    atlas_home = tmp_path / ".atlas"
    _make_agent(atlas_home)
    userdata = tmp_path / "Atlas-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(atlas_home, remove_userdata=False)
...
def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Atlas"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Atlas"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Atlas"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Atlas"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    ATLAS_HOME with all the destructive externals stubbed out.
    """
    ...
    monkeypatch.setattr(uninstall, "get_atlas_home", lambda: atlas_home)
    ...
    from atlas_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (atlas_home / "config.yaml").exists()
    assert atlas_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole ATLAS_HOME non-interactively."""
    import atlas_cli.uninstall as uninstall

    atlas_home = tmp_path / ".atlas"
    (atlas_home / "atlas-agent" / "atlas_cli").mkdir(parents=True)
    (atlas_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not atlas_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m atlas_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_atlas_home", lambda: atlas_home)
    ...
    assert not (desktop / "release").exists()
    assert not (atlas_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "atlas_cli").exists()
    assert (atlas_home / "config.yaml").exists()
