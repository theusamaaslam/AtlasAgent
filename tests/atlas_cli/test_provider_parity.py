...
the SAME provider universe as ``atlas model`` (the CLI/TUI picker).
...
def test_every_atlas_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `atlas model` universe."""
    ...
    assert not missing, (
        "providers shown in `atlas model` but not configurable in the desktop "
