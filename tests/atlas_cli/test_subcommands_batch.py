...
# ── deprecated `atlas login` fails gracefully, not with argparse error ────
#
# `atlas login` is a removed command; its handler (`login_command` in
# `atlas_cli/auth.py`) prints a deprecation notice pointing at `atlas auth` /
# `atlas model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `atlas --help`.
