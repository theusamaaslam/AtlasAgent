...
atlas_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `atlas config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_atlas_home = get_atlas_home()`` at import time (module
    singleton), so monkeypatching ATLAS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ATLAS_HOME.)
    """
    import cli

    cli._atlas_home = home
