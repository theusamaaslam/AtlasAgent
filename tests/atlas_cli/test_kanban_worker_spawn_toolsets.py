...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    ATLAS_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_atlas_argv", lambda: ["atlas"])
    ...
    assert pid == 4242
    assert captured["env"]["ATLAS_HOME"] == str(profile)
    assert captured["env"]["ATLAS_KANBAN_TASK"] == "t_spawn_tools"
