"""Tests for atlas_cli.status model/provider display."""

from types import SimpleNamespace

from atlas_cli.nous_account import NousPaidServiceAccessInfo, NousPortalAccountInfo
...
def _patch_common_status_deps(monkeypatch, status_mod, tmp_path, *, openai_base_url=""):
    ...
    monkeypatch.setattr(status_mod, "get_atlas_home", lambda: tmp_path, raising=False)
...
def test_show_status_hides_removed_managed_gateway_features(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "Atlas Gateway", raising=False)
    monkeypatch.setattr(
        status_mod,
...
def test_show_status_reports_empty_lmstudio_listing_as_reachable(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        "atlas_cli.models.probe_lmstudio_models",
