...
binary download are exercised manually by `atlas secrets bitwarden
...
def atlas_home(tmp_path, monkeypatch):
    """Point Atlas at an isolated home directory."""
    home = tmp_path / ".atlas"
    home.mkdir()
    monkeypatch.setenv("ATLAS_HOME", str(home))
    # Some modules cache get_atlas_home; clear if needed.
    import atlas_constants
    if hasattr(atlas_constants, "_ATLAS_HOME_CACHE"):
        atlas_constants._ATLAS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(atlas_home, monkeypatch):
...
def test_install_bws_happy_path(atlas_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(atlas_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(atlas_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from atlas_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under atlas_home/cache/."""
    home = tmp_path / ".atlas"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
