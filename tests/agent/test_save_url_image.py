...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ATLAS_HOME under tmp_path."""
    monkeypatch.setenv("ATLAS_HOME", str(tmp_path / ".atlas"))
    (tmp_path / ".atlas").mkdir()

    # Force the constants/image cache helpers to re-read ATLAS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("atlas_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_atlas_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ATLAS_HOME — gateway cleanup
