...
from atlas_cli.nous_account import NousPortalAccountInfo, NousPaidServiceAccessInfo
...
def test_view_fetch_failure_is_logged_out(monkeypatch):
    ...
    monkeypatch.setattr("atlas_cli.nous_account.get_nous_portal_account_info", _boom)
...
def test_cli_show_credits_non_interactive_renders_text_not_modal(monkeypatch, capsys):
    ...
    monkeypatch.setattr(AtlasCLI, "_prompt_text_input_modal", _boom_modal, raising=False)
...
def test_cli_show_credits_logged_out(monkeypatch, capsys):
    ...
    cli._app = None
    cli._show_credits()
    assert "Not logged into Atlas Gateway" in capsys.readouterr().out
