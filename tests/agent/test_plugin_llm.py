...
""",
        ...
        from atlas_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        atlas_home = tmp_path / ".atlas"
        atlas_home.mkdir()
        (atlas_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ATLAS_HOME", str(atlas_home))
        from atlas_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from atlas_cli.plugins import PluginContext, PluginManifest, PluginManager
