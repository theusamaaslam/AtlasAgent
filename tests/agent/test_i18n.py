...
def test_normalize_lang_accepts_english_aliases():
    assert i18n._normalize_lang("English") == "en"
    assert i18n._normalize_lang("en-US") == "en"


@pytest.mark.parametrize("value", ["zh", "Chinese", "Deutsch", "jp", "tr-TR"])
def test_removed_languages_fall_back_to_english(value):
    assert i18n._normalize_lang(value) == "en"
...
def test_env_var_override(monkeypatch):
    """ATLAS_LANGUAGE wins over config."""
    i18n.reset_language_cache()
    monkeypatch.setenv("ATLAS_LANGUAGE", "en-US")
    assert i18n.get_language() == "en"


def test_env_var_normalized(monkeypatch):
    i18n.reset_language_cache()
    monkeypatch.setenv("ATLAS_LANGUAGE", "Chinese")
    assert i18n.get_language() == "en"


def test_default_when_nothing_set(monkeypatch):
    """With no env var and no config override, falls back to English."""
    monkeypatch.delenv("ATLAS_LANGUAGE", raising=False)
...
def test_t_explicit_lang():
    ...
    assert i18n.t("approval.denied", lang="zh") == i18n.t(
...
def test_t_missing_key_in_non_english_falls_back_to_english(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(i18n, "SUPPORTED_LANGUAGES", ("en", "zh"))
    i18n.reset_language_cache()
    try:
...
def test_locales_dir_env_override_used_when_dir_exists(tmp_path, monkeypatch):
    """ATLAS_BUNDLED_LOCALES wins when it points at a real directory."""
    bundled = tmp_path / "bundled-locales"
    bundled.mkdir()
    monkeypatch.setenv("ATLAS_BUNDLED_LOCALES", str(bundled))
    assert i18n._locales_dir() == bundled


def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus ATLAS_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("ATLAS_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
...
def test_locales_dir_falls_back_to_data_scheme(tmp_path, monkeypatch):
    ...
    # No env override.
    monkeypatch.delenv("ATLAS_BUNDLED_LOCALES", raising=False)
