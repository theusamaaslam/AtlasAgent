...
engine plugins (e.g. atlas-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``atlas plugins`` picker."""
    from atlas_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "atlas_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "atlas_cli.plugins.get_plugin_context_engine",
