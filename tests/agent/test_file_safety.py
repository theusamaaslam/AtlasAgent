...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_atlas_env(self):
        """Atlas' own .env inside ATLAS_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: atlas internal .env is in ~/.atlas/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.atlas/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.atlas/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Atlas cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        atlas_home = tmp_path / ".atlas"
        cache = atlas_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._atlas_home_path", return_value=atlas_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Atlas cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        atlas_home = tmp_path / ".atlas"
        hub = atlas_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_atlas_home(self, tmp_path):
        """The env basename guard does not depend on ATLAS_HOME resolution."""
        atlas_home = tmp_path / ".atlas"
        atlas_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        atlas_home = tmp_path / ".atlas"
        cache = atlas_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
