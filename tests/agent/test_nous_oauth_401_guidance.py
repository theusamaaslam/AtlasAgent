...
to a generic "Your API key was rejected... run atlas setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # (Atlas Gateway has no API key path — auth_type=oauth_device_code only).
    assert "Atlas Gateway OAuth token was rejected" in source

    # Must give a concrete re-auth command, not a generic "atlas setup".
    assert "atlas portal" in source
...
def test_free_slug_hint_for_nous_provider():
    ...
    on the next message because Atlas Gateway doesn't carry the OpenRouter
    free-tier slug.
    """
    source = inspect.getsource(conversation_loop.run_conversation)
