...
path via ``atlas chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ATLAS_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ATLAS_HOME at a temp dir and turn moa.save_traces on."""
    atlas_home = tmp_path / ".atlas"
    atlas_home.mkdir()
    monkeypatch.setenv("ATLAS_HOME", str(atlas_home))

    # save_moa_turn reads config via atlas_cli.config.load_config; stub it to
