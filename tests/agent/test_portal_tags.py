"""Tests for agent.portal_tags — Atlas Gateway request tag contract."""

from __future__ import annotations


def test_atlas_client_tag_includes_current_version():
    """The client tag must reflect atlas_cli.__version__ verbatim."""
    from atlas_cli import __version__
    from agent.portal_tags import atlas_client_tag

    assert atlas_client_tag() == f"client=atlas-client-v{__version__}"


def test_atlas_client_tag_format():
    """The client tag has the exact shape Atlas Gateway expects."""
    from agent.portal_tags import atlas_client_tag

    tag = atlas_client_tag()
    assert tag.startswith("client=atlas-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    """Every Atlas Gateway request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import atlas_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=atlas-agent" in tags
    assert atlas_client_tag() in tags
    assert len(tags) == 2
