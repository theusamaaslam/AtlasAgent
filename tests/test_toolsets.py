...
class TestToolsetConsistency:
    ...
    def test_atlas_platforms_share_core_tools(self):
        """All atlas-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on atlas-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["atlas-cli", "atlas-telegram", "atlas-discord", "atlas-whatsapp", "atlas-slack", "atlas-signal", "atlas-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_atlas_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("atlas-whatsapp")

    def test_atlas_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("atlas-api-server")
