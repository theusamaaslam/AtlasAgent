"""Shared fixtures for the atlas-agent test suite.
...
2. **Isolated ATLAS_HOME.** ATLAS_HOME points to a per-test tempdir so
   code reading ``~/.atlas/*`` via ``get_atlas_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".atlas"`` instead of the canonical
   ``get_atlas_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ATLAS_SESSION_* inheritance** — the agent's current gateway
...
# ATLAS_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ATLAS_BEHAVIORAL_VARS = frozenset({
    "ATLAS_YOLO_MODE",
    "ATLAS_INTERACTIVE",
    "ATLAS_QUIET",
    "ATLAS_TOOL_PROGRESS",
    "ATLAS_TOOL_PROGRESS_MODE",
    "ATLAS_MAX_ITERATIONS",
    "ATLAS_SESSION_PLATFORM",
    "ATLAS_SESSION_CHAT_ID",
    "ATLAS_SESSION_CHAT_NAME",
    "ATLAS_SESSION_THREAD_ID",
    "ATLAS_SESSION_SOURCE",
    "ATLAS_SESSION_KEY",
    "ATLAS_GATEWAY_SESSION",
    "ATLAS_CRON_SESSION",
    "_ATLAS_GATEWAY",
    "ATLAS_PLATFORM",
    "ATLAS_MODEL",
    "ATLAS_INFERENCE_MODEL",
    ...
    "ATLAS_BACKGROUND_NOTIFICATIONS",
    "ATLAS_EXEC_ASK",
    "ATLAS_HOME_MODE",
    "ATLAS_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.atlas/kanban.db instead of the per-test ATLAS_HOME.
    "ATLAS_KANBAN_DB",
    "ATLAS_KANBAN_BOARD",
    "ATLAS_KANBAN_HOME",
    "ATLAS_KANBAN_WORKSPACES_ROOT",
    "ATLAS_KANBAN_LOGS_ROOT",
    "ATLAS_KANBAN_TASK",
    "ATLAS_KANBAN_WORKSPACE",
    "ATLAS_KANBAN_RUN_ID",
    "ATLAS_KANBAN_CLAIM_LOCK",
    "ATLAS_KANBAN_DISPATCH_IN_GATEWAY",
    "ATLAS_TENANT",
    ...
    "ATLAS_DASHBOARD_OAUTH_CLIENT_ID",
    "ATLAS_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ATLAS_HOME to per-test tempdirs so code that
    reads ``~/.atlas/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ATLAS_* vars that could change test semantics.
    for name in _ATLAS_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ATLAS_HOME to a per-test tempdir. Code that reads
    #    ``~/.atlas/*`` via ``get_atlas_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.atlas/*`` via
    #    ``Path.home() / ".atlas"`` instead of ``get_atlas_home()``
    #    is a bug to fix at the callsite.
    fake_atlas_home = tmp_path / "atlas_test"
    fake_atlas_home.mkdir()
    (fake_atlas_home / "sessions").mkdir()
    (fake_atlas_home / "cron").mkdir()
    (fake_atlas_home / "memories").mkdir()
    (fake_atlas_home / "skills").mkdir()
    monkeypatch.setenv("ATLAS_HOME", str(fake_atlas_home))
    ...
    #    ~/.atlas/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import atlas_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_atlas_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal atlas config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``atlas-gateway`` process
...
# correlated with a ``tests/atlas_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> atlas-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart atlas-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting atlas/python patterns are also blocked.
    """
    ...
    def _matches_atlas_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ATLAS_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_atlas_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch atlas-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m atlas_cli.main".
                if (
                    "atlas" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live atlas-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting atlas/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `atlas update` (or the
        # equivalent `python -m atlas_cli.main update`).  These commands
        ...
        if "update" in low and (
            # atlas update / atlas update --gateway / setsid bash -c ... atlas update
            ("atlas" in low and "update" in low.split())
            or
            # python -m atlas_cli.main update --gateway
            ("atlas_cli" in low and "update" in low.split())
            or
            # venv/bin/atlas update  (absolute path variant used in tests)
            (".venv/bin/atlas" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`atlas update` against the real checkout, fetching "
