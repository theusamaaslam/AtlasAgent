...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    atlas_state = ModuleType("atlas_state")
    atlas_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "atlas_state", atlas_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Atlas"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Atlas"
