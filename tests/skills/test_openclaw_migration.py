...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `atlas cron` to recreate scheduled tasks" in notes_text
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Atlas remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "atlas prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Atlas was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "atlas uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Atlas said to ask Atlas about Atlas settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.atlas`` (the real Atlas home), not the broken
    ``.Atlas``.
    ...
    rewritten to ``~/.Atlas/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.atlas/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .atlas directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.atlas'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "atlas config path: ~/.atlas/"
