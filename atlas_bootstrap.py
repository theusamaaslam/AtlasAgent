"""Windows UTF-8 bootstrap for Atlas entry points.
...
should be imported at the very top of every Atlas entry point
(``atlas``, ``atlas-agent``, ``atlas-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Atlas modules.

    Atlas ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Atlas source root
    ...
    """
    root = src_root or os.environ.get("ATLAS_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ATLAS_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ATLAS_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import atlas_bootstrap``
# (or ``from atlas_bootstrap import apply_windows_utf8_bootstrap``) at
