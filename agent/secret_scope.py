...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ATLAS_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "ATLAS_HOME", "ATLAS_PROFILE", "ATLAS_GATEWAY_LOCK_DIR",
    "ATLAS_MAX_ITERATIONS", "ATLAS_MAX_TOKENS", "ATLAS_API_TIMEOUT",
    "ATLAS_REDACT_SECRETS", "ATLAS_NOUS_TIMEOUT_SECONDS",
    "_ATLAS_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ATLAS_KANBAN_DB", "ATLAS_KANBAN_WORKSPACES_ROOT", "ATLAS_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "ATLAS_KANBAN_",
    "ATLAS_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(atlas_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(atlas_home) / ".env")
