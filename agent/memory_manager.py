...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ATLAS_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Atlas expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``atlas_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_atlas_home()`` themselves.
        """
        if "atlas_home" not in kwargs:
            from atlas_constants import get_atlas_home
            kwargs["atlas_home"] = str(get_atlas_home())
        for provider in self._providers:
            try:
