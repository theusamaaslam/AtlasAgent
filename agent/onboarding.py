...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Atlas starts and finds ``~/.openclaw/``.

    Points users at ``atlas claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``atlas claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Atlas, run "
        "`atlas claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `atlas claw cleanup` (renames it to ~/.openclaw.pre-migration — "
