"""Anthropic Messages API adapter for Atlas Agent.

Translates between Atlas's internal OpenAI-style message format and
...
# Atlas effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to atlas's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to atlas's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Atlas notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Atlas historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``atlas models``) never mutates ``~/.atlas/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Atlas)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.atlas/auth.json)
    ...
    # 1. Atlas-managed OAuth/setup token env var
    ...
    # 4. Atlas credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Atlas configs.
...
# ── Atlas-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.atlas/.anthropic_oauth.json (our own file).
...
_ATLAS_OAUTH_FILE = get_atlas_home() / ".anthropic_oauth.json"
...
def run_atlas_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Atlas-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Atlas with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from atlas_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_atlas_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Atlas-managed OAuth credentials from ~/.atlas/.anthropic_oauth.json."""
    if _ATLAS_OAUTH_FILE.exists():
        try:
            data = json.loads(_ATLAS_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Atlas OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See atlas-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See atlas-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See atlas-agent#13848 (Kimi) and
    atlas-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Atlas Agent", "Claude Code")
                text = text.replace("Atlas agent", "Claude Code")
                text = text.replace("atlas-agent", "claude-code")
                text = text.replace("Usama Aslam", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Atlas-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Atlas surfaces in its CLI. We
