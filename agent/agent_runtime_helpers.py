...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Atlas-Session-Id header), and an
        ...
        if env_var_enabled("ATLAS_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from atlas_cli.config import load_config as _load_moa_cfg
    ...
    # Atlas Gateway proxies to OpenRouter behind the scenes — identical
    ...
    if (is_openrouter or is_nous_portal) and is_claude:
        return True, False
    # Atlas Gateway Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to atlas's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from atlas_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from atlas_cli.plugins import get_pre_tool_call_block_message
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: atlas' own dispatch loop intentionally keeps an
