...
Storage
-------
Bundles live in ``~/.atlas/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ATLAS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``atlas bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ATLAS_HOME.

    Honors ``ATLAS_BUNDLES_DIR`` for tests; falls back to
    ``<ATLAS_HOME>/skill-bundles``.
    """
    override = os.environ.get("ATLAS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_atlas_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `atlas bundles` CLI subcommand.
# ---------------------------------------------------------------------------
