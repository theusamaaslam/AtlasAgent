...
projection into Atlas' display, approval bridging, transcript projection into
...
"codex_app_server"`. Atlas' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Atlas secret — gateway
        ...
        spawn_env = atlas_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ATLAS_KANBAN_TASK"):
            kanban_db = spawn_env.get("ATLAS_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ATLAS_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ATLAS_HOME", os.path.expanduser("~/.atlas")),
                        "kanban",
                    ),
                )
            )
