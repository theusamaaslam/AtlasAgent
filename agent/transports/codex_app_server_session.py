...
Owns one Codex thread per Atlas session. Drives `turn/start`, consumes
...
# Atlas' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ATLAS_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Atlas/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Atlas session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ATLAS_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ATLAS_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="atlas",
            client_title="Atlas Agent",
            client_version=_get_atlas_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Atlas' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Atlas' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own atlas-tools callback we
            # auto-accept — the user already approved Atlas' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Atlas approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Atlas returns 'once', 'session', 'always', or 'deny'.
...
def _get_atlas_version() -> str:
    """Best-effort Atlas version string for codex's userAgent line."""
    try:
