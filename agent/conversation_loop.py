...
from atlas_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Atlas tool use: "
    ...
    return (
        ...
        f"context, but Atlas needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Atlas config, set `model.ollama_num_ctx: 65536` "
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `atlas fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from atlas_cli.moa_config import decode_moa_turn
    ...
    # all run inside Codex). Default Atlas path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Atlas internals.
        #
        # Atlas invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Atlas tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Atlas Gateway rate limit guard ──────────────────────
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"Atlas Gateway rate limit active — "
            ...
            try:
                ...
                try:
                    from atlas_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("ATLAS_DUMP_REQUESTS"):
                ...
                from atlas_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Atlas/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: atlas auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `atlas doctor` for credential-chain diagnostics, or")
                    ...
                    from atlas_constants import display_atlas_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Atlas-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: atlas config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: atlas config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Atlas Gateway: record rate limit & skip retries ─────
                ...
                #
                # IMPORTANT: Atlas Gateway multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Atlas).  A 429 can
                ...
                # which Atlas' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Atlas' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`atlas",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `atlas auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `atlas model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Atlas Gateway OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: atlas portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.nousresearch.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; Atlas Gateway will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Atlas Gateway won't recognize that model name. Either switch to a", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: atlas setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        atlas fallback add   (interactive picker — same as `atlas model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.atlas/config.yaml` to extend the per-call "
                            f"timeout. (Atlas's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Atlas mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from atlas_cli.plugins import get_pre_verify_continue_message, has_hook
