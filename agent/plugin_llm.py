...
Plugins built on Atlas Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Atlas
...
:class:`~atlas_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Atlas' main config
...
handles every provider, fallback chain, and per-task override Atlas
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`atlas_cli.plugins.PluginContext`
