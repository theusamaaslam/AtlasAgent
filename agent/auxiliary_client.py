...
Resolution order for text tasks (auto mode):
  ...
  3. Atlas Gateway (~/.atlas/auth.json active provider)
...
# Atlas Gateway extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Atlas Gateway.
#
...
def _nous_extra_body() -> dict:
    """Return a fresh Atlas Gateway ``extra_body`` dict.
...
# Set at resolve time — True if the auxiliary client points to Atlas Gateway
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Atlas Gateway (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    if runtime is None and not nous:
        logger.warning(
            "Auxiliary Atlas Gateway client unavailable: no gateway authentication found "
            "(run: atlas auth)."
        )
    ...
    logger.debug("Auxiliary client: Atlas Gateway")
    ...
    if runtime is not None:
        ...
        if not api_key:
            logger.warning(
                "Auxiliary Atlas Gateway client unavailable: no usable inference token found "
                "(run: atlas auth)."
            )
            _mark_provider_unhealthy("nous", ttl=60)
            return None, None
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the Atlas Gateway's recommended model after a stale-model 404.
...
def _mark_provider_unhealthy(provider: str, ttl: Optional[float] = None) -> None:
    ...
    logger.warning(
        ...
        _AUX_UNHEALTHY_DISPLAY_LABELS.get(label, label),
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Atlas Gateway not configured (run: atlas auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3. Atlas Gateway (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Atlas Gateway product tags when the auxiliary client is backed
    by Atlas Gateway. Returns empty dict otherwise.
    """
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # ── Stale-model self-heal (Atlas Gateway recommendation drift) ───
