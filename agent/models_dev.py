...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.atlas/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Atlas ↔ models.dev
# ---------------------------------------------------------------------------

# Atlas provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Atlas (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_atlas_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``atlas config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Atlas provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Atlas agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Atlas provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Atlas ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Atlas or models.dev provider ID.  Tries exact match then
