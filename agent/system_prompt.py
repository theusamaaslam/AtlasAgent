...
upstream prefix cache warm.  See ``atlas-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Atlas never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ATLAS_HOME persona while keeping
    ...
    # ATLAS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Atlas, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Atlas profile the agent is running
    # under so it doesn't conflate ~/.atlas/skills/ (default profile) with
    # ~/.atlas/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Atlas profile: default. Other profiles (if any) live "
            "under ~/.atlas/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Atlas profile: {active_profile}. This session reads "
            f"and writes ~/.atlas/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.atlas/skills/, ~/.atlas/plugins/, "
            f"~/.atlas/cron/, ~/.atlas/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Atlas never rebuilds or reinjects parts of it
