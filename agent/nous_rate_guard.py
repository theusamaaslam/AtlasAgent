"""Cross-session rate limit guard for Atlas Gateway.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Atlas Gateway is currently rate-limited
...
(3 SDK retries x 3 Atlas retries), and every one of those calls counts
...
def record_nous_rate_limit(
    *,
    ...
    """Record that Atlas Gateway is rate-limited.
...
def nous_rate_limit_remaining() -> Optional[float]:
    """Check if Atlas Gateway is currently rate-limited.
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 from Atlas Gateway is a real account rate limit.

    Atlas Gateway multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Atlas, ...) behind one endpoint.  A 429 can mean either:
