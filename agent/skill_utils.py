...
from atlas_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ATLAS_KANBAN_TASK`` / ``ATLAS_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ATLAS_KANBAN_TASK") or os.getenv("ATLAS_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Atlas Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``atlas_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ATLAS_PLATFORM`` or
            ``ATLAS_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``atlas`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.atlas/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``atlas`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ATLAS_HOME, not cwd
        if not p.is_absolute():
            p = (atlas_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.atlas/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Atlas can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    atlas = metadata.get("atlas") or {}
    if not isinstance(atlas, dict):
        atlas = {}
    return {
        "fallback_for_toolsets": atlas.get("fallback_for_toolsets", []),
        "requires_toolsets": atlas.get("requires_toolsets", []),
        "fallback_for_tools": atlas.get("fallback_for_tools", []),
        "requires_tools": atlas.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Atlas metadata, VCS, virtualenv/dependency, cache, and skill
