...
def get_atlas_home() -> Path:
    ...
    Reads ATLAS_HOME env var, falls back to the platform-native default.
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Atlas is installed
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Atlas-approved
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<ATLAS_HOME>/skills`` last-resort
    """
...
def get_atlas_dir(new_subpath: str, old_name: str) -> Path:
