...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("ATLAS_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                if toolset_name.startswith("atlas-"):
                    # Platform bundles (atlas-*) include _ATLAS_CORE_TOOLS, so
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from atlas_cli.plugins import has_hook, invoke_hook
