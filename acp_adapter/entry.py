"""CLI entry point for the atlas-agent ACP adapter.

Loads environment variables from ``~/.atlas/.env``, configures logging
...
try:
    ...
    # shadowing Atlas's own modules — ``atlas acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from ATLAS_HOME (default ``~/.atlas``)."""
    from atlas_cli.env_loader import load_atlas_dotenv

    atlas_home = get_atlas_home()
    loaded = load_atlas_dotenv(atlas_home=atlas_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", atlas_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="atlas-acp",
        description="Run Atlas Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Atlas version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Atlas provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.atlas/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import AtlasACPAgent  # noqa: F401

    print("Atlas ACP check OK")


def _run_setup() -> None:
    from atlas_cli.main import main as atlas_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "atlas", "model"]
        atlas_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``atlas postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting atlas-agent ACP adapter")
    ...
    import acp
    from .server import AtlasACPAgent
