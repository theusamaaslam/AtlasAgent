...
import { getAtlasConfig, getAtlasConfigDefaults } from '@/atlas'
...
interface AtlasConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useAtlasConfig({ activeSessionIdRef, refreshProjectBranch }: AtlasConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshAtlasConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getAtlasConfig(), getAtlasConfigDefaults().catch(() => ({}))])
  ...
  return { refreshAtlasConfig, sttEnabled, voiceMaxRecordingSeconds }
}
