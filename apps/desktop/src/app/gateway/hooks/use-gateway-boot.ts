import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@atlas/shared'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.atlasDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: AtlasGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: AtlasConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Atlas...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Atlas…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Atlas gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshAtlasConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshAtlasConfig()
