...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `atlas auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'nous' ? 'Atlas Gateway' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Atlas Gateway' })
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    fireEvent.click(await screen.findByText('Atlas Gateway'))
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `atlas auth add qwen-oauth` or that provider's CLI to remove it.",
