...
import { disconnectOAuthProvider, listOAuthProviders } from '@/atlas'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/atlas'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.atlasDesktop?.terminal)
...
//      in atlas_cli/provider_catalog.py) — the SAME provider identity
//      `atlas model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Atlas can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Atlas can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Atlas never deletes creds another tool owns behind a silent API call.
