...
import { invalidateAtlasConfig, setAtlasConfigCache, useAtlasConfigRecord } from '../hooks/use-config-record'
...
// Atlas' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Atlas default (medium), shown as Medium.
...
// backend now surfaces the full `atlas model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in atlas_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useAtlasConfigRecord()
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `atlas model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Atlas runs the flow for you.`}
          </p>
        )}
