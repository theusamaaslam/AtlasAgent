...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../atlas'
...
import { useAtlasConfig } from './session/hooks/use-atlas-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.atlasDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.atlasDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  // atlas:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.atlasDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshAtlasConfig, sttEnabled, voiceMaxRecordingSeconds } = useAtlasConfig({
    activeSessionIdRef,
  ...
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshAtlasConfig])
