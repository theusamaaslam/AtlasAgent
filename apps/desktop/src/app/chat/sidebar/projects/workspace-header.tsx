...
const branchActionLabel = (branch: AtlasGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `atlas/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<AtlasGitBranch[]>([])
  ...
  const convert = async (branch: AtlasGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
