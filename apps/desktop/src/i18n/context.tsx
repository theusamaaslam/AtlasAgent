...
import { getAtlasConfigRecord, type AtlasConfigRecord, saveAtlasConfig } from '@/atlas'
...
export interface I18nConfigClient {
  getConfig: () => Promise<AtlasConfigRecord>
  saveConfig: (config: AtlasConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.atlasDesktop?.api) {
      return Promise.resolve({})
    }

    return getAtlasConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.atlasDesktop?.api) {
...
export function getConfigDisplayLanguage(config: AtlasConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AtlasConfigRecord, locale: Locale): AtlasConfigRecord {
