...
function LanguageProbe() {
  ...
  return (
    <div>
      ...
      <button onClick={() => void setLocale('en').catch(() => undefined)} type="button">
        switch
      </button>
    </div>
  )
...
describe('I18nProvider', () => {
  ...
  it('normalizes unsupported initial locales to English', () => {
    render(
      <I18nProvider configClient={null} initialLocale="xx-YY">
        <LanguageProbe />
      </I18nProvider>
    )
  ...
  it('loads English from display.language config', async () => {
  ...
  it('falls back to English for old non-English display.language values', async () => {
    const configClient: I18nConfigClient = {
      getConfig: vi.fn().mockResolvedValue({ display: { language: 'xx-YY' } }),
      saveConfig: vi.fn()
    }

    render(
      <I18nProvider configClient={configClient} initialLocale="xx">
        <LanguageProbe />
      </I18nProvider>
    )
  ...
  it('reads latest config before saving English and preserves unrelated values', async () => {
    const saveConfig = vi.fn().mockResolvedValue({ ok: true })
    const latestConfig: AtlasConfigRecord = {
