...
import { getGlobalModelOptions } from '@/atlas'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/atlas'
...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/theusamaaslam/AtlasAgent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `atlas model`
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse secondary providers only when a featured provider is configured.
