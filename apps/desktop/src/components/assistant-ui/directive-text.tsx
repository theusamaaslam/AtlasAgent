...
const ATLAS_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
...
const ICON_PATHS: Record<AtlasRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AtlasRefType] ?? ICON_FALLBACK
...
const ATLAS_DIRECTIVE_RE = new RegExp(
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(ATLAS_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AtlasRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AtlasRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
