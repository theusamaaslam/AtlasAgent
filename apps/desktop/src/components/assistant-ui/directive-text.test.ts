...
import { formatRefValue, atlasDirectiveFormatter } from './directive-text'
...
describe('atlasDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = atlasDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = atlasDirectiveFormatter.parse('@file:src/main.tsx the entry point')
