...
import type { AtlasReviewFile, AtlasReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Atlas' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'atlas.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'atlas.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'atlas.desktop.reviewTreeMode'
const SELECTED_KEY = 'atlas.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<AtlasReviewFile[]>([])
...
export const $reviewShipInfo = atom<AtlasReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['atlasDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: AtlasReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.atlasDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.atlasDesktop?.openExternal?.(url)
    }
