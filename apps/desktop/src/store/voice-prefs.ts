...
import { getAtlasConfigRecord, saveAtlasConfig } from '@/atlas'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getAtlasConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveAtlasConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
