...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/atlas'
...
async function fetchProviderDefaultModel(
  ...
  // `atlas model` does (for Nous it honors the user's free/paid tier, so a
...
async function openSignInUrl(url: string) {
  if (window.atlasDesktop?.openExternal) {
    try {
      await window.atlasDesktop.openExternal(url)
