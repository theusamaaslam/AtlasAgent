...
import type { AtlasGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/atlas'
...
const PROJECT_SCOPE_KEY = 'atlas.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Atlas gateway is not connected')
  }
...
// repo, returning where Atlas should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<AtlasGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.atlasDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.atlasDesktop?.writeClipboard?.(path)
  }
}
