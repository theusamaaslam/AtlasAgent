...
import type { SessionInfo, UsageStats } from '@/types/atlas'
...
const COMPOSER_MODEL_KEY = 'atlas.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'atlas.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'atlas.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'atlas.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'atlas.desktop.lastSessionId'
...
function workspaceCwdKey(connection: AtlasConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.atlasDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.atlasDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<AtlasConnection | null>(null)
...
export const setConnection = (next: Updater<AtlasConnection | null>) => updateAtom($connection, next)
