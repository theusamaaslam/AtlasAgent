...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Atlas doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in atlas_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `atlas model` uses — instead of
...
export type AtlasConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `atlas model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors atlas_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Atlas, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `atlas tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Atlas); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/atlas/update/check` — the backend's own update state.
...
/** One Atlas-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Atlas Tool Gateway as a result of
