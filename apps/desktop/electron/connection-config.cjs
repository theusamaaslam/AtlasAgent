 ...
 *     `X-Atlas-Session-Token` header; WS uses `?token=`.
...
// atlas_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - atlas_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - atlas_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / atlas #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-atlas_session_at', '__Secure-atlas_session_at', 'atlas_session_at']
const RT_COOKIE_VARIANTS = ['__Host-atlas_session_rt', '__Secure-atlas_session_rt', 'atlas_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts = {}) {
  ...
  let parsed
  try {
    parsed = new URL(rawPath, 'http://atlas.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a atlas session ACCESS-token cookie
