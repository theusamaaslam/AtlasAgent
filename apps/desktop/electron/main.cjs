...
const { canImportAtlasCli, verifyAtlasCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeAtlasHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.ATLAS_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ATLAS_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with ATLAS_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[atlas] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[atlas] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('atlas:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[atlas] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[atlas] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[atlas] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ATLAS_HOME — the user-facing root for everything Atlas-related. Mirrors
// scripts/install.ps1's $AtlasHome and scripts/install.sh's $ATLAS_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\atlas (matches install.ps1)
//   macOS / Linux: ~/.atlas (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.atlas directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\atlas yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ATLAS_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ATLAS_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.atlas / %LOCALAPPDATA%\atlas.
function resolveAtlasHome() {
  if (process.env.ATLAS_HOME) return normalizeAtlasHomeRoot(process.env.ATLAS_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'atlas-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ATLAS_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\atlas and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ATLAS_HOME')
    if (fromRegistry) return normalizeAtlasHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'atlas')
    const legacy = path.join(app.getPath('home'), '.atlas')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.atlas setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.atlas')
}

const ATLAS_HOME = resolveAtlasHome()

function atlasManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_atlas_node_dirs() in
  // atlas_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(ATLAS_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithAtlasManagedNode(...entries) {
  return [...atlasManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ATLAS_ROOT — the canonical mutable Atlas install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ATLAS_ROOT = path.join(ATLAS_HOME, 'atlas-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ATLAS_ROOT, 'venv')
...
// boots, going straight to `resolveAtlasBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ATLAS_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ATLAS_ROOT, '.atlas-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Atlas profile the desktop launches its
// local backend as. When set, startAtlas() passes `atlas --profile <name>
// dashboard …`, which deterministically pins ATLAS_HOME (see
// _apply_profile_override in atlas_cli/main.py) and bypasses the sticky
// ~/.atlas/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors atlas_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// atlasDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ATLAS_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by atlas_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ATLAS_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (atlas_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ATLAS_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ATLAS_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'atlas:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.atlas) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.atlas')
}
// Seed the native About panel with the live Atlas version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `atlas update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveAtlasVersion(),
  copyright: 'Copyright © 2026 Usama Aslam'
})
...
const MEDIA_PROTOCOL = 'atlas-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(atlas-media://…)`.
...
// (the desktop's launch profile) stays managed by atlasProcess +
// connectionPromise + startAtlas(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ATLAS_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ATLAS_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startAtlas() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning atlas serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Atlas backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  atlasLog.push(...lines)
  if (atlasLog.length > 300) {
    atlasLog.splice(0, atlasLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-atlas-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('atlas:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the atlas:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('atlas:bootstrap:event', ev)
}
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(ATLAS_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Atlas will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(ATLAS_HOME)
  }
  if (marker) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `atlas` — must not shadow `atlas.cmd`/`atlas.exe`.
...
function unwrapWindowsVenvAtlasCommand(command, backendArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^atlas(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing Atlas Python at ${python}`,
    command: python,
    args: ['-m', 'atlas_cli.main', ...backendArgs],
    bootstrap: false,
...
// spawns `atlas serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `atlas` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'atlas_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ATLAS_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isAtlasSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'atlas_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ATLAS_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Atlas deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Atlas' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\atlas\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'atlas', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'atlas', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal ATLAS_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\atlas\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'atlas', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'atlas', 'git', 'bin', 'git.exe'))
  }
...
function recentAtlasLog() {
  return atlasLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's atlas root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ATLAS_ROOT.
// ATLAS_DESKTOP_ATLAS_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ATLAS_DESKTOP_ATLAS_ROOT && path.resolve(process.env.ATLAS_DESKTOP_ATLAS_ROOT),
    !IS_PACKAGED && isAtlasSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAtlasSourceRoot(ACTIVE_ATLAS_ROOT) ? ACTIVE_ATLAS_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ATLAS_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('atlas:updates:progress', merged)
  }
}
...
// ATLAS_HOME/atlas-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_atlas_home). That binary owns
// ALL repo mutation — running `atlas update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'atlas-setup.exe' : 'atlas-setup'
  const candidate = path.join(ATLAS_HOME, name)
...
// Path to the venv shim whose lock decides whether `atlas update` can write
// fresh entry points. On Windows this is the file the running backend
// `atlas.exe` holds open; on POSIX it's never mandatory-locked.
function venvAtlasShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'atlas.exe')
    : path.join(updateRoot, 'venv', 'bin', 'atlas')
}
...
// only signals the direct child, so on Windows a backend `atlas.exe` that
// spawned its own grandchildren (a `atlas` REPL, a pty terminal session, the
...
// `atlasProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `atlas.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a atlas
// REPL, a pty terminal, the gateway) keep `atlas.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (atlasProcess && Number.isInteger(atlasProcess.pid)) pids.push(atlasProcess.pid)
  ...
  if (atlasProcess && !atlasProcess.killed) {
    try {
      atlasProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvAtlasShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (atlasProcess && Number.isInteger(atlasProcess.pid)) stragglers.push(atlasProcess.pid)
    for (const entry of backendPool.values()) {
...
// `atlas update`). Instead we spawn the staged Atlas-Setup binary with
// --update and quit, so it can run `atlas update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri atlas-setup. Unlike Windows
      ...
      // whole update itself: `atlas update` (backend) + `atlas desktop
    ...
    if (!updater) {
      ...
      // `atlas desktop`, never the Tauri installer that self-copies
      // atlas-setup.exe into ATLAS_HOME). They DO have a working `atlas`
      ...
      // checkout they're on — bare `atlas update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'atlas update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `atlas update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `atlas update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, atlasRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Atlas — this window will close and the updater will open. Don’t reopen Atlas yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // atlas.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running atlas, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Atlas install open ' +
        '(a second Atlas window or a terminal running atlas?). Close it and retry.'
    ...
    // `atlas update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: ATLAS_HOME,
      env: {
        ...process.env,
        ATLAS_HOME,
        PATH: pathWithAtlasManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAtlas = path.join(venvBin, IS_WINDOWS ? 'atlas.exe' : 'atlas')
  ...
  // not just the `atlas.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvAtlas) || fileExists(path.join(updateRoot, '.atlas-bootstrap-complete'))
...
// Resolve the atlas CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `atlas` on PATH.
function resolveAtlasCliBinary(updateRoot) {
  const venvAtlas = path.join(updateRoot, 'venv', 'bin', 'atlas')
  if (fileExists(venvAtlas)) return venvAtlas
  return findOnPath('atlas') || null
}
...
// macOS/Linux in-app update: backend (`atlas update`) + OS-aware GUI rebuild
// (`atlas desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const atlas = resolveAtlasCliBinary(updateRoot)
  if (!atlas) {
    emitUpdateProgress({ stage: 'manual', message: 'atlas update', percent: null })
    return { ok: true, manual: true, command: 'atlas update', atlasRoot: updateRoot }
  }

  // Put the Atlas-managed Node and the venv on PATH so `atlas desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\atlas\node, not node\bin.
  const env = {
    ATLAS_HOME,
    PATH: pathWithAtlasManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `atlas update` reaps stale `atlas serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ATLAS_DESKTOP_CHILD_PID and excludes
  ...
  if (atlasProcess && Number.isInteger(atlasProcess.pid)) {
    desktopChildPids.push(atlasProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.ATLAS_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Atlas (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(atlas, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'atlas update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'atlas update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(atlas, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Atlas to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `atlas desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Atlas…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `atlas-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Atlas to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Atlas desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Atlas to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Atlas.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Atlas.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Atlas to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.atlas-update-new"; then
    rm -rf "$DST.atlas-update-old" 2>/dev/null || true
    mv "$DST" "$DST.atlas-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.atlas-update-new" "$DST"
    rm -rf "$DST.atlas-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Atlas to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ATLAS_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isAtlasSourceRoot(ACTIVE_ATLAS_ROOT) &&
    fileExists(venvPython) &&
    canImportAtlasCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ATLAS_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `atlas
...
function resolveWebDist() {
  const override = process.env.ATLAS_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ATLAS_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: atlas desktop --force-build`
  )
  return candidates[0]
}
...
function resolveAtlasCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Atlas.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAtlasCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_ATLAS_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Atlas at ${ACTIVE_ATLAS_ROOT}`,
    command,
    args: ['-m', 'atlas_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      atlasHome: ATLAS_HOME,
      pythonPathEntries: [ACTIVE_ATLAS_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ATLAS_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveAtlasBackend(backendArgs) {
  // 1. Explicit override -- ATLAS_DESKTOP_ATLAS_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ATLAS_DESKTOP_ATLAS_ROOT && path.resolve(process.env.ATLAS_DESKTOP_ATLAS_ROOT)
  if (overrideRoot && isAtlasSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Atlas source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `atlas` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAtlasSourceRoot.)
  if (!IS_PACKAGED && isAtlasSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Atlas source at ${SOURCE_REPO_ROOT}`, backendArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_ATLAS_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\atlas\atlas-agent (Windows) or ~/.atlas/atlas-agent.
  ...
  //    to spawning atlas. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `atlas update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `atlas` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ATLAS_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ATLAS_DESKTOP_IGNORE_EXISTING !== '1') {
    let atlasCommand = null
    const atlasOverride = process.env.ATLAS_DESKTOP_ATLAS

    if (atlasOverride) {
      const resolvedOverride = findOnPath(atlasOverride)
      if (resolvedOverride) {
        atlasCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(atlasOverride, { isWsl: IS_WSL })) {
        atlasCommand = atlasOverride
      } else {
        rememberLog(`Ignoring Windows Atlas override under WSL: ${atlasOverride}`)
      }
    } else {
      atlasCommand = findOnPath('atlas')
    }

    if (atlasCommand) {
      if (looksLikeDesktopAppBinary(atlasCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Atlas CLI: ${atlasCommand}`)
        atlasCommand = null
      }
    }

    if (atlasCommand) {
      const unwrapped = unwrapWindowsVenvAtlasCommand(atlasCommand, backendArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `atlas` shim
      ...
      const shellForProbe = isCommandScript(atlasCommand)
      if (verifyAtlasCli(atlasCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvAtlasCommand(atlasCommand, backendArgs) || {
            label: `existing Atlas CLI at ${atlasCommand}`,
      ...
      rememberLog(
        `Ignoring existing Atlas CLI at ${atlasCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed atlas_cli module via system Python.
  ...
  if (python) {
    ...
    // having atlas_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\atlas\atlas-agent\venv.
    if (canImportAtlasCli(python)) {
      return {
        kind: 'python',
        label: `installed atlas_cli module via ${python}`,
        command: python,
        args: ['-m', 'atlas_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: atlas_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAtlasBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Atlas Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAtlasBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Atlas install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Atlas recovery was handed off to Atlas Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      atlasHome: ATLAS_HOME,
      logRoot: path.join(ATLAS_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Atlas install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Atlas bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ATLAS_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startAtlas() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // atlas:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveAtlasBackend(backend.args))
  }
  ...
  if (!isAtlasSourceRoot(ACTIVE_ATLAS_ROOT)) {
    throw new Error(
      `Atlas install at ${ACTIVE_ATLAS_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Atlas' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\atlas\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `atlas` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Atlas on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAtlasSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Atlas venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Atlas at ${ACTIVE_ATLAS_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Atlas runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Atlas backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Atlas backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Atlas backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Atlas-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('atlas:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAtlasCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('atlas:preview-file-changed', payload)
}
...
async function waitForAtlas(baseUrl, token) {
  ...
  throw new Error(`Atlas backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('atlas:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('atlas:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Atlas...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('atlas:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('atlas:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('atlas:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'atlas:desktop:zoomLevel'
...
//
// Hosted Atlas gateways gate the dashboard behind an OAuth provider (e.g.
// Usama Aslam) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``atlas_session_at``),
...
//   * Atlas Gateway now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / atlas #37247).
//     Both are set as HttpOnly cookies (``atlas_session_at`` ~15 min,
//     ``atlas_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:atlas-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Atlas gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Atlas backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root ATLAS_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ATLAS_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.ATLAS_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by ATLAS_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (atlas #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Atlas gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Atlas gateway is selected, but no session token is saved. ' +
...
//   2. env override (ATLAS_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ATLAS_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ATLAS_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ATLAS_DESKTOP_REMOTE_URL is set but ATLAS_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Atlas backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.ATLAS_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Atlas
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Usama Aslam" instead of a generic
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAtlas())
  ...
  // connect to Atlas gateway". Mirror the renderer's connect here so the test
...
function resetAtlasConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(atlasProcess)
...
// startAtlas() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetAtlasConnection() nulls atlasProcess.
  const dying = atlasProcess && !atlasProcess.killed ? atlasProcess : null
  resetAtlasConnection()
...
// profile to startAtlas() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startAtlas() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAtlas(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: atlasLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited ATLAS_HOME env (see _apply_profile_override
  // step 3 in atlas_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveAtlasBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const atlasCwd = resolveAtlasCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Atlas backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: atlasCwd,
        ATLAS_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ATLAS_DESKTOP: '1',
        ATLAS_WEB_DIST: webDist,
        ...(readyFile ? { ATLAS_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Atlas backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Atlas backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Atlas backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForAtlas(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Atlas backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_atlas_home() recreates the deleted profile directory.
...
async function startAtlas() {
  ...
  // process, every subsequent startAtlas() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Atlas backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Atlas backend at ${remote.baseUrl}`, 24)
      await waitForAtlas(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Atlas backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.atlas/active_profile file) and
    // resolves ATLAS_HOME the same way `atlas -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Atlas runtime', 28)
    const backend = await ensureRuntime(resolveAtlasBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const atlasCwd = resolveAtlasCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Atlas backend via ${backend.label}`, 84)
    rememberLog(`Starting Atlas backend via ${backend.label}`)
    ...
    atlasProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ATLAS_HOME for the child so Python's get_atlas_home()
          // resolves to the SAME location our resolveAtlasHome() picked. Without
          // this pin, Python falls back to ~/.atlas on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\atlas, which differs from C:\Users\<u>\.atlas.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ATLAS_HOME via setx; the desktop
    ...
    atlasProcess.stdout.on('data', rememberLog)
    atlasProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    atlasProcess.once('error', error => {
      rememberLog(`Atlas backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Atlas backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    atlasProcess.once('exit', (code, signal) => {
      rememberLog(`Atlas backend exited (${signal || code})`)
      atlasProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Atlas backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Atlas backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentAtlasLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Atlas backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(atlasProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Atlas backend to become ready', 90)
    await Promise.race([waitForAtlas(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null atlasProcess when the child dies.
      childAlive: () => atlasProcess !== null && atlasProcess.exitCode === null && !atlasProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Atlas backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Atlas is
...
// pushes pet state over IPC (atlas:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via atlas:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Atlas app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Atlas icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see atlas:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Atlas in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('atlas:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startAtlas().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('atlas:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Atlas…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('atlas:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetAtlasConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Atlas backend failed liveness probe; dropping stale connection.')
    resetAtlasConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('atlas:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('atlas:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('atlas:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('atlas:window:openNewSession', async () => {
...
ipcMain.handle('atlas:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('atlas:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('atlas:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('atlas:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('atlas:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('atlas:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('atlas:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('atlas:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('atlas:pet-overlay:control', payload)
})
ipcMain.handle('atlas:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startAtlas() call restarts the
...
ipcMain.handle('atlas:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startAtlas() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('atlas:bootstrap:cancel', async () => {
...
ipcMain.handle('atlas:boot-progress:get', async () => bootProgressState)
ipcMain.handle('atlas:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('atlas:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('atlas:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('atlas:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('atlas:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('atlas:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('atlas:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('atlas:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('atlas:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('atlas:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ATLAS_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('atlas:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('atlas:requestMicrophoneAccess', async () => {
...
ipcMain.handle('atlas:api', async (_event, request) => {
  ...
  // backend calls ensure_atlas_home() which recreates the profile directory,
...
ipcMain.handle('atlas:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('atlas:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('atlas:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('atlas:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('atlas:readFileText', async (_event, filePath) => {
...
ipcMain.handle('atlas:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('atlas:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('atlas:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('atlas:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('atlas:saveClipboardImage', async () => {
...
ipcMain.handle('atlas:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('atlas:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('atlas:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('atlas:titlebar-theme', (_event, payload) => {
...
ipcMain.on('atlas:native-theme', (_event, mode) => {
...
ipcMain.on('atlas:translucency', (_event, payload) => {
...
ipcMain.handle('atlas:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('atlas:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveAtlasCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('atlas:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAtlasCwd()
}))

ipcMain.handle('atlas:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('atlas:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('atlas:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('atlas:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('atlas:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('atlas:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: atlasLog.slice(-200) }))
...
function terminalShellCommand() {
  // ATLAS_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ATLAS_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Atlas' TUI into
  ...
  env.TERM_PROGRAM = 'Atlas'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a atlas/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ATLAS_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ATLAS_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `atlas:terminal:${id}:${suffix}`
}
...
ipcMain.handle('atlas:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('atlas:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('atlas:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('atlas:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('atlas:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('atlas:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('atlas:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('atlas:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('atlas:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('atlas:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('atlas:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('atlas:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('atlas:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('atlas:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('atlas:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('atlas:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('atlas:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('atlas:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('atlas:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('atlas:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('atlas:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('atlas:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('atlas:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('atlas:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('atlas:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('atlas:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Atlas.')
  }
...
ipcMain.handle('atlas:terminal:write', (_event, id, data) => {
...
ipcMain.handle('atlas:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('atlas:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('atlas:updates:check', async () =>
...
ipcMain.handle('atlas:updates:apply', async (_event, payload) =>
...
ipcMain.handle('atlas:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('atlas:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Atlas version (the one `release.py` bumps in
// atlas_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Atlas version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAtlasVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'atlas_cli', '__init__.py')
...
// Re-resolve the live Atlas version and push it into the native About panel
// just before showing it, so an in-place `atlas update` is reflected without
...
ipcMain.handle('atlas:version', async () => ({
  appVersion: resolveAtlasVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  atlasRoot: resolveUpdateRoot()
}))
...
// the actual removal via `atlas uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (atlas_cli/uninstall.py + atlas_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ATLAS_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isAtlasSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'atlas_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ATLAS_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Atlas agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import atlas_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ATLAS_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `atlas-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `atlas-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('atlas:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('atlas:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('atlas:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('atlas:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// atlas:// deep links (e.g. atlas://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ATLAS_PROTOCOL = 'atlas'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ATLAS_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // atlas://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('atlas:deep-link', payload)
...
ipcMain.handle('atlas:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${ATLAS_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ATLAS_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ATLAS_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `atlas://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching atlas:// URL is in our own argv.
