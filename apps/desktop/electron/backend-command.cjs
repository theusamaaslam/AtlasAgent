...
// Backend subcommand routing for the desktop-managed Atlas process.
//
// The desktop app launches its own headless backend via `atlas serve` — it
...
// app hasn't updated yet, or an older `atlas` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Atlas profile to pin via `--profile`.
 */
 ...
 * `-m atlas_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `atlas_cli/subcommands/dashboard.py` source registers
