 ...
 * Background
 * ----------
 * After `atlas update` + `atlas desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ATLAS_DESKTOP_* is preserved
// plus ATLAS_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ATLAS_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ATLAS_DESKTOP_']
