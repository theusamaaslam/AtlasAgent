 ...
 *
 * Drives apps/desktop's first-launch install of Atlas Agent by spawning
...
function bootstrapCacheDir(atlasHome) {
  return path.join(atlasHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.atlas/atlas-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(atlasHome) {
  if (!atlasHome) return null
  const candidate = path.join(atlasHome, 'atlas-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(atlasHome, commit) {
  return path.join(bootstrapCacheDir(atlasHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(atlasHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(atlasHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, atlasHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ATLAS_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ATLAS_HOME: atlasHome || process.env.ATLAS_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, atlasHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, atlasHome }) {
  const args = ['--dir', activeRoot, '--atlas-home', atlasHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, atlasHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, atlasHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, atlasHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, atlasHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(atlasHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, atlasHome, emit })
