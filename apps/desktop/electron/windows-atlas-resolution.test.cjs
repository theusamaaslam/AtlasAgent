...
// Regression guards for Windows `atlas` resolution in main.cjs.
//
...
//      Git-Bash `atlas` shim shadowed the real atlas.cmd/atlas.exe; the
...
//      --repair by checking ONLY venv\Scripts\atlas.exe (the console-script
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow atlas.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\atlas.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvAtlas\) \?/,
    'recovery regressed to gating only on the atlas.exe shim, which forces destructive --repair'
  )
})
