...
// even though a fresh shell — and the Atlas CLI — sees it immediately. The
// desktop's ATLAS_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\atlas. Reading
...
//       ATLAS_HOME    REG_SZ    F:\Atlas\data
//
