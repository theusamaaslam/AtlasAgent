...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'atlas-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/atlas-runner-test',
    sourceRepoRoot: null,
    atlasHome: '/tmp/atlas-runner-test',
    logRoot: '/tmp/atlas-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'atlas-agent', 'scripts')
