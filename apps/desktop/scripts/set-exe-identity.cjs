#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Atlas icon + version metadata onto the
// built Atlas.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Atlas.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `atlas desktop`, the installer's
...
// path (which rebuilds via `atlas desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Atlas.exe>
//
...
// Stamp the Atlas icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'Usama Aslam',
      LegalCopyright: 'Copyright (c) 2026 Usama Aslam'
    }
  })

  console.log('[set-exe-identity] done — Atlas icon + identity stamped')
}
