...
// no longer bundles the Atlas Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Atlas.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Atlas'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Atlas.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'atlas'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ATLAS_HOME for non-sandboxed runs -- matches main.cjs's
// resolveAtlasHome(). On Windows it's %LOCALAPPDATA%\atlas; elsewhere
// it's ~/.atlas. The fresh-install sandbox launchFresh() sets its own
// ATLAS_HOME and never touches this.
const DEFAULT_ATLAS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'atlas')
  }
  return path.join(os.homedir(), '.atlas')
})()
const VENV_ROOT = path.join(DEFAULT_ATLAS_HOME, 'atlas-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'atlas-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ATLAS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Atlas-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Atlas-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Atlas-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Atlas-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ATLAS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ATLAS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const atlasHome = path.join(sandbox, 'atlas-home')
  ...
  env.ATLAS_DESKTOP_CWD = cwd
  env.ATLAS_DESKTOP_IGNORE_EXISTING = '1'
  env.ATLAS_DESKTOP_TEST_MODE = 'fresh-install'
  env.ATLAS_DESKTOP_USER_DATA_DIR = userDataDir
  env.ATLAS_HOME = atlasHome
  delete env.ATLAS_DESKTOP_ATLAS
  delete env.ATLAS_DESKTOP_ATLAS_ROOT
  ...
  console.log(`  ATLAS_HOME: ${atlasHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(atlasHome, 'atlas-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Atlas Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of atlas_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'atlas-agent', 'atlas_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Atlas
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ATLAS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ATLAS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
