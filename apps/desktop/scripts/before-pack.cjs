 ...
 * product name (`Atlas`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Atlas'
 *
 * This is a hard failure with no obvious cause for the user — `atlas desktop`
 ...
 * (the mac-unpacked Atlas.app bundle) and Windows (win-unpacked), so we
