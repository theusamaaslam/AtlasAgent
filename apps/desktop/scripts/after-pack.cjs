 ...
 *
 * Stamps the Atlas icon + identity onto the packed Windows Atlas.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `atlas desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Atlas')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Atlas'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Atlas.exe keeps the stock Electron icon`)
  }
}
