...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $ATLAS_SETUP_DEV_REPO_ROOT
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("ATLAS_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let base_url = std::env::var("ATLAS_INSTALL_SCRIPT_BASE_URL")
        .unwrap_or_else(|_| DEFAULT_INSTALL_SCRIPT_BASE_URL.to_string())
        .trim_end_matches('/')
        .to_string();
    let url = format!(
        "{}/{}/scripts/{}",
