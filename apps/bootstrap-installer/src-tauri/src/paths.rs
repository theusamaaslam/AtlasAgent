...
//!
//! Mirrors `atlas_constants.get_atlas_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\atlas
//!   macOS:   ~/.atlas
//!   Linux:   ~/.atlas  (override via $ATLAS_HOME)
//!
//! NOTE (macOS): Python's get_atlas_home(), scripts/install.sh, and the
//! Electron desktop's resolveAtlasHome() ALL use ~/.atlas on macOS — there
...
/// Returns the canonical Atlas home directory, respecting $ATLAS_HOME if set.
pub fn atlas_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ATLAS_HOME") {
    ...
    {
        // %LOCALAPPDATA%\atlas — matches scripts/install.ps1's $AtlasHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("atlas");
        }
    }

    // macOS + Linux + fallback: ~/.atlas (matches Python get_atlas_home(),
    // install.sh, and the Electron desktop's resolveAtlasHome()).
...
pub fn bootstrap_cache_dir() -> PathBuf {
    atlas_home().join("bootstrap-cache")
}
...
/// ATLAS_HOME so it survives repo checkout deletion (unlike anything under
/// atlas-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\atlas\atlas-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_atlas`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ATLAS_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ATLAS_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    atlas_home().join(".atlas-update-in-progress")
}
...
pub fn copy_self_to_atlas_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ATLAS_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ATLAS_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".atlas-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ATLAS_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[atlas-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ATLAS_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_atlas_home() -> String {
    atlas_home().to_string_lossy().into_owned()
}
