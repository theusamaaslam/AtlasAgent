...
export const $atlasHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.atlas/logs/bootstrap-installer.log')
    $atlasHome.set('~/.atlas')
  ...
  try {
    const [logPath, atlasHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_atlas_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $atlasHome.set(atlasHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Atlas" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Atlas-Setup.exe --update);
...
export async function launchAtlasDesktop(): Promise<void> {
  ...
  await invoke('launch_atlas_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Atlas repository' },
