...
_ATLAS_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on ATLAS_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (ATLAS_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `atlas tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ATLAS_KANBAN_TASK env "
    ...
    # Coding posture (base Atlas — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in atlas_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Atlas toolsets (CLI + messaging platforms)
    #
    ...
    # the `atlas send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "atlas-cli": {
        ...
        "tools": _ATLAS_CORE_TOOLS,
        "includes": []
    },

    "atlas-cron": {
        # Mirrors atlas-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `atlas tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as atlas-cli; gated by `atlas tools`",
        "tools": _ATLAS_CORE_TOOLS,
        "includes": []
    },
    ...
    "atlas-email": {
        "description": "Email bot toolset - interact with Atlas via email (IMAP/SMTP)",
        "tools": _ATLAS_CORE_TOOLS,
        "includes": []
    },
    ...
    "atlas-sms": {
        "description": "SMS bot toolset - interact with Atlas via SMS (Twilio)",
        "tools": _ATLAS_CORE_TOOLS,
        "includes": []
    },
    ...
    "atlas-webhook": {
        ...
        "tools": _ATLAS_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "atlas-gateway": {
        ...
        "tools": [],
        "includes": ["atlas-telegram", "atlas-discord", "atlas-whatsapp", "atlas-slack", "atlas-signal", "atlas-bluebubbles", "atlas-homeassistant", "atlas-email", "atlas-sms", "atlas-mattermost", "atlas-matrix", "atlas-dingtalk", "atlas-feishu", "atlas-wecom", "atlas-wecom-callback", "atlas-weixin", "atlas-qqbot", "atlas-webhook", "atlas-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``atlas-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ATLAS_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``atlas-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (atlas-<name>).
        # Gives them _ATLAS_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("atlas-"):
            platform_name = name[len("atlas-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ATLAS_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
