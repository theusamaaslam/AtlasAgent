...
React + Ink terminal UI for Atlas. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ATLAS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `atlas setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.atlas/.atlas_history` or under `ATLAS_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `atlas setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/atlas-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external atlas setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.atlas/tui-parent.log
    ...
    types/
      atlas-ink.d.ts            type declarations for @atlas/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AtlasCLI subprocess for slash commands
```