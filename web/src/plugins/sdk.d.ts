/**
 * Atlas Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ATLAS_PLUGIN_SDK__`` and
 * ``window.__ATLAS_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ATLAS_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@atlas/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``AtlasPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@atlas/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @atlas/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__ATLAS_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ATLAS_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ATLAS_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface AtlasPluginSDK {
...
declare global {
  interface Window {
    __ATLAS_PLUGIN_SDK__?: AtlasPluginSDK;
    __ATLAS_PLUGINS__?: PluginRegistry;
  }
}
