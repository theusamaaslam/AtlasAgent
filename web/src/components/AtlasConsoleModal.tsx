...
interface AtlasConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function AtlasConsoleModal({ open, onClose }: AtlasConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("atlas> ");
  const inputPromptRef = useRef("atlas> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "atlas> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Atlas Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="atlas-console-title"
    >
      <div
        className={cn(
          themedBody,
