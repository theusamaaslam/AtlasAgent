import { buildAtlasWebSocketUrl } from "@atlas/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/atlas/). The Python backend
// injects ``window.__ATLAS_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ATLAS_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ATLAS_BASE_PATH = readBasePath();
const BASE = ATLAS_BASE_PATH;
...
declare global {
  interface Window {
    __ATLAS_SESSION_TOKEN__?: string;
    __ATLAS_BASE_PATH__?: string;
    ...
    __ATLAS_AUTH_REQUIRED__?: boolean;
  }
}
...
export async function fetchJSON<T>(
  ...
  const token = window.__ATLAS_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``atlas update``, ``atlas gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ATLAS_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ATLAS_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("atlas.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("atlas.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__ATLAS_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("atlas.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ATLAS_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Atlas dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``atlas_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ATLAS_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Atlas-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``atlas_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ATLAS_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateAtlas: () =>
    fetchJSON<ActionResponse>("/api/atlas/update", { method: "POST" }),
  checkAtlasUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/atlas/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * Atlas Gateway contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** atlas-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``atlas update``. */
  can_update_atlas?: boolean;
  config_path: string;
