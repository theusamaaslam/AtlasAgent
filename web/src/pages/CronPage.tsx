...
import { Segmented } from "@atlas/ui/ui/components/segmented";
...
const cronModalOverlayClass =
  "fixed inset-0 z-[220] flex items-start justify-center overflow-y-auto bg-[#10101a]/55 p-4 backdrop-blur-sm sm:items-center sm:p-6";

const cronModalShellClass =
  "relative my-4 flex max-h-[calc(100dvh-2rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-[#dfe2f1] bg-white text-[#1f1b2d] shadow-[0_28px_90px_rgba(31,27,45,0.25)] sm:my-6";

const cronModalHeaderClass =
  "shrink-0 border-b border-[#dfe2f1] px-5 py-4 sm:px-6 sm:py-5";

const cronModalBodyClass =
  "grid min-h-0 flex-1 gap-4 overflow-y-auto px-5 py-5 sm:px-6";
...
export default function CronPage() {
  ...
  // attach-skill selector (parity with `atlas cron edit --add-skill`).
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {createModalOpen && (
        <div
          ref={createModalRef}
          className={cronModalOverlayClass}
          onClick={(e) => e.target === e.currentTarget && setCreateModalOpen(false)}
          role="dialog"
          aria-modal="true"
          aria-labelledby="create-cron-title"
        >
          <div className={cn(themedBody, cronModalShellClass)}>
            <Button
              ghost
              size="icon"
              onClick={() => setCreateModalOpen(false)}
              className="absolute right-3 top-3 z-10 text-[#74718a] hover:bg-[#f4f5fb] hover:text-[#1f1b2d]"
              aria-label="Close"
            >
              <X />
            </Button>

            <header className={cronModalHeaderClass}>
              <h2
                id="create-cron-title"
                className="text-xl font-semibold tracking-normal text-[#1f1b2d]"
              >
                {t.cron.newJob}
              </h2>
            </header>

            <div className={cronModalBodyClass}>
              <div className="grid gap-2">
              ...
              <div className="sticky bottom-0 -mx-5 -mb-5 flex justify-end border-t border-[#dfe2f1] bg-white/95 px-5 py-4 backdrop-blur sm:-mx-6 sm:px-6">
                <Button
                  className="uppercase"
                  size="sm"
        ...
        >
          <div className={cn(themedBody, cronModalShellClass)}>
            ...
            <div className={cronModalBodyClass}>
              ...
              <div className="sticky bottom-0 -mx-5 -mb-5 flex items-center justify-between border-t border-[#dfe2f1] bg-white/95 px-5 py-4 backdrop-blur sm:-mx-6 sm:px-6">
