...
export default function SystemPage() {
  ...
  // spawned `atlas import` runs non-interactively (stdin is /dev/null), so
  ...
  const loadAll = useCallback(() => {
    Promise.allSettled([
      ...
      api.checkAtlasUpdate(false),
    ])
      .then(([s, st, m, p, c, h, upd]) => {
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_atlas === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkAtlasUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_atlas],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_atlas === false) {
      showToast(
        "Atlas updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateAtlas();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "atlas-update");
  ...
  const canUpdateAtlas = status?.can_update_atlas !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateAtlas && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Atlas?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'atlas update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'atlas update' (${updateInfo?.update_command ?? "atlas update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Atlas</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.atlas_version}</span>
                  {canUpdateAtlas &&
                    updateInfo &&
          ...
          <CardContent className="flex flex-col gap-4 py-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="ml-auto">
                New credentials:{" "}
                <span className="font-mono">atlas memory setup</span>
              </span>
            </div>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$ATLAS_HOME/backups/atlas-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Atlas backup?"
              description={`This will overwrite your current Atlas configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Atlas team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
