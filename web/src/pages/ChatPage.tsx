/**
 * ChatPage — embeds `atlas --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (atlas_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ATLAS_SESSION_TOKEN__ &&
    !window.__ATLAS_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `atlas dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ATLAS_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Atlas TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[atlas-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ATLAS_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets ATLAS_HOME pointed at the
    ...
    // forwarding them into Atlas. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="atlas-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
