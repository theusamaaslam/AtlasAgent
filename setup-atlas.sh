...
# 5. Symlinks the 'atlas' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Atlas Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/atlas-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink atlas into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up atlas command..."

ATLAS_BIN="$SCRIPT_DIR/venv/bin/atlas"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ATLAS_BIN" "$COMMAND_LINK_DIR/atlas"
echo -e "${GREEN}✓${NC} Symlinked atlas → $COMMAND_LINK_DISPLAY_DIR/atlas"
...
# ============================================================================
# Seed bundled skills into ~/.atlas/skills/
# ============================================================================

ATLAS_SKILLS_DIR="${ATLAS_HOME:-$HOME/.atlas}/skills"
mkdir -p "$ATLAS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.atlas/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ATLAS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  atlas status        # Check configuration"
if is_termux; then
    echo "  atlas gateway       # Run gateway in foreground"
else
    echo "  atlas gateway install # Install gateway service (messaging + cron)"
fi
echo "  atlas cron list     # View scheduled jobs"
echo "  atlas doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m atlas_cli.main setup
fi
