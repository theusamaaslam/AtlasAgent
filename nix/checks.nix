...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      atlas-agent = self'.packages.default;
      atlasVenv = atlas-agent.atlasVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "atlas-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${atlasVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "atlas-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "atlas-build-package" { } ''
          echo "PASS: package built at ${atlas-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "atlas-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "atlas-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${atlas-agent}/bin/atlas || (echo "FAIL: atlas binary missing"; exit 1)
          test -x ${atlas-agent}/bin/atlas-agent || (echo "FAIL: atlas-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${atlas-agent}/bin/atlas version 2>&1 | grep -qi "atlas" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "atlas-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in atlas atlas-agent atlas-acp; do
            test -x ${atlas-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "atlas-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking atlas --help ==="
          ${atlas-agent}/bin/atlas --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${atlas-agent}/bin/atlas --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "atlas-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${atlas-agent}/share/atlas-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${atlas-agent}/share/atlas-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ATLAS_BUNDLED_SKILLS" ${atlas-agent}/bin/atlas || \
            (echo "FAIL: ATLAS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ATLAS_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "atlas-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${atlas-agent}/share/atlas-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${atlas-agent}/share/atlas-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ATLAS_BUNDLED_PLUGINS" ${atlas-agent}/bin/atlas || \
            (echo "FAIL: ATLAS_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ATLAS_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "atlas-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${atlas-agent}/share/atlas-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${atlas-agent}/share/atlas-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${atlas-agent}/share/atlas-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ATLAS_BUNDLED_LOCALES" ${atlas-agent}/bin/atlas || \
            (echo "FAIL: ATLAS_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ATLAS_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ATLAS_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ATLAS_BUNDLED_LOCALES=${atlas-agent}/share/atlas-agent/locales \
            ${atlasVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ATLAS_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${atlasVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${atlasVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ATLAS_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "atlas-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${atlas-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${atlas-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ATLAS_TUI_DIR" ${atlas-agent}/bin/atlas || \
            (echo "FAIL: ATLAS_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ATLAS_TUI_DIR set in wrapper"
        ...
        # Verify ATLAS_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        atlas-node = pkgs.runCommand "atlas-node-version" { } ''
          set -e
          echo "=== Checking ATLAS_NODE in wrapper ==="
          grep -q "ATLAS_NODE" ${atlas-agent}/bin/atlas || \
            (echo "FAIL: ATLAS_NODE not set in wrapper"; exit 1)
          echo "PASS: ATLAS_NODE present in wrapper"

          ATLAS_NODE=$(sed -n "s/^export ATLAS_NODE='\(.*\)'/\1/p" ${atlas-agent}/bin/atlas)
          test -x "$ATLAS_NODE" || (echo "FAIL: ATLAS_NODE=$ATLAS_NODE not executable"; exit 1)
          echo "PASS: ATLAS_NODE executable at $ATLAS_NODE"

          NODE_MAJOR=$("$ATLAS_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ATLAS_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ATLAS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "atlas-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ATLAS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ATLAS_MANAGED guards ==="
          check_blocked "config set" ${atlas-agent}/bin/atlas config set model foo
          check_blocked "config edit" ${atlas-agent}/bin/atlas config edit
        ...
        extra-python-packages = let
          ...
          atlasWithExtra = atlas-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "atlas-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${atlasWithExtra}/bin/atlas || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${atlasWithExtra}/bin/atlas || \
          ...
          if grep -q "PYTHONPATH" ${atlas-agent}/bin/atlas; then
        ...
        extra-dependency-groups = let
          atlasWithGroups = atlas-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "atlas-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${atlasWithGroups}"
          echo "venv: ${atlasWithGroups.atlasVenv}"
        ...
        messaging-variant = pkgs.runCommand "atlas-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.atlasVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "atlas-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$atlas_home/config.yaml"
            ${atlasVenv}/bin/python3 -c '
import json, sys
