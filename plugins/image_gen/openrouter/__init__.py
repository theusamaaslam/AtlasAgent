"""OpenRouter-compatible image generation backend (OpenRouter + Atlas Gateway).

Both OpenRouter and the Atlas Gateway inference endpoint speak the same
...
Atlas Gateway proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~atlas_cli.runtime_provider.resolve_runtime_provider`,
...
# image_gen.model from ``atlas tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Atlas Gateway). The two differ only
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``atlas tools``) → the quality-first default chain.
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `atlas tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            ...
            setup_schema={
                "name": "Atlas Gateway (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Atlas Gateway (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Atlas Gateway image gen providers."""
