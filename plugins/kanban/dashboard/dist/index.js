/**
 * Atlas Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.atlas/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ATLAS_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ATLAS_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "atlas-kanban-dot-triage",
    todo: "atlas-kanban-dot-todo",
    ready: "atlas-kanban-dot-ready",
    running: "atlas-kanban-dot-running",
    blocked: "atlas-kanban-dot-blocked",
    done: "atlas-kanban-dot-done",
    archived: "atlas-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-atlas-task";
  ...
  const DOCS_URL = "https://atlas-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://atlas-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "atlas.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="atlas-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])atlas-kanban-md-code\1/i.test(attrs)) {
      return ' class="atlas-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "atlas-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "atlas-kanban-md",
  ...
  // standard `drop` event and our `atlas-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("atlas-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("atlas-kanban-column--drop");
          if (target) target.classList.add("atlas-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("atlas-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("atlas-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("atlas-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ATLAS_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "atlas-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "atlas-kanban-attention",
        "atlas-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "atlas-kanban-attention-bar" },
        h("span", { className: "atlas-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "atlas-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "atlas-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "atlas-kanban-attention-row",
                  "atlas-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "atlas-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "atlas-kanban-attention-row-id" }, task.id),
                h("span", { className: "atlas-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "atlas-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "atlas-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "atlas-kanban-diag-action-btn",
      action.suggested ? "atlas-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " atlas-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".atlas-kanban-drawer-comment-row input, .atlas-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "atlas-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("atlas-kanban-diag", sevClass) },
      h("div", { className: "atlas-kanban-diag-header" },
        h("span", { className: "atlas-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "atlas-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "atlas-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "atlas-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "atlas-kanban-diag-data-row" },
                  h("span", { className: "atlas-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "atlas-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "atlas-kanban-diag-data-row" },
                h("span", { className: "atlas-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "atlas-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "atlas-kanban-diag-reassign-row" },
            h("span", { className: "atlas-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "atlas-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "atlas-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "atlas-kanban-diag-msg",
              msg.ok ? "atlas-kanban-diag-msg--ok" : "atlas-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "atlas-kanban-section" },
      h("div", { className: "atlas-kanban-section-head-row" },
        h("span", { className: "atlas-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "atlas-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "atlas-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "atlas-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "atlas-kanban-docs-link",
      title: "Open Atlas Kanban docs in a new tab",
      "aria-label": "Atlas Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `atlas kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "atlas-kanban-msg-ok" : "atlas-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "atlas-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "atlas-kanban-boardswitcher" },
      h("div", { className: "atlas-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "atlas-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "atlas-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "atlas-kanban-dialog-title" },
        ...
        h("div", { className: "atlas-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Atlas profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "atlas-kanban-bulk" },
      h("span", { className: "atlas-kanban-bulk-count" },
      ...
      h("div", { className: "atlas-kanban-bulk-priority",
      ...
      h("div", { className: "atlas-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Atlas profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "atlas-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("atlas-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("atlas-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "atlas-kanban-trash",
        dragOver ? "atlas-kanban-trash--drop" : "",
        props.draggingTaskId ? "atlas-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "atlas-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "atlas-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".atlas-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "atlas-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("atlas-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("atlas-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "atlas-kanban-column",
        dragOver ? "atlas-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "atlas-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "atlas-kanban-col-check",
        ...
        h("span", { className: cn("atlas-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "atlas-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "atlas-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "atlas-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "atlas-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "atlas-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "atlas-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "atlas-kanban-lane" },
                  h("div", { className: "atlas-kanban-lane-head" },
                    h("span", { className: "atlas-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "atlas-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "atlas-kanban-card--stale-red";
    if (age >= tier.amber) return "atlas-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".atlas-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "atlas-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "atlas-kanban-card",
        props.selected ? "atlas-kanban-card--selected" : "",
        props.failed ? "atlas-kanban-card--failed" : "",
        props.draggingSource ? "atlas-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "atlas-kanban-card-content" },
          h("div", { className: "atlas-kanban-card-row" },
            h("label", {
              className: "atlas-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "atlas-kanban-card-check",
            ...
            h("span", { className: "atlas-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or atlas kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "atlas-kanban-warning-badge",
                    "atlas-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "atlas-kanban-needs-assignee",
          ...
          h("div", { className: "atlas-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "atlas-kanban-card-row atlas-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "atlas-kanban-assignee",
                            title: `Assigned to Atlas profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "atlas-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "atlas-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "atlas-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "atlas-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Atlas profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Atlas profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "atlas-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "atlas-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "atlas-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "atlas-kanban-drawer-close",
        ...
        data ? h("div", { className: "atlas-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "atlas-kanban-section" },
      h("div", { className: "atlas-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "atlas-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "atlas-kanban-drawer-body" },
      h("div", { className: "atlas-kanban-drawer-title" },
        h("span", { className: cn("atlas-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "atlas-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "atlas-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "atlas-kanban-section" },
        h("div", { className: "atlas-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "atlas-kanban-section" },
        h("div", { className: "atlas-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "atlas-kanban-comment" },
                h("div", { className: "atlas-kanban-comment-head" },
                  h("span", { className: "atlas-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "atlas-kanban-comment-ago" },
      ...
      h("div", { className: "atlas-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "atlas-kanban-event",
              isDiag ? "atlas-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "atlas-kanban-event-header" },
                  h("span", { className: "atlas-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "atlas-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "atlas-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "atlas-kanban-event-header-plain" },
                  h("span", { className: "atlas-kanban-event-kind" }, e.kind),
                  h("span", { className: "atlas-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "atlas-kanban-event-phantom-row" },
                  h("span", { className: "atlas-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "atlas-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "atlas-kanban-event-payload" },
    ...
    return h("div", { className: "atlas-kanban-section" },
      h("div", { className: "atlas-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "atlas-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `atlas-kanban-run--${r.outcome || r.status || "ended"}`
          : "atlas-kanban-run--active";
        return h("div", { key: r.id, className: cn("atlas-kanban-run", outcomeClass) },
          h("div", { className: "atlas-kanban-run-head" },
            h("span", { className: "atlas-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "atlas-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "atlas-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "atlas-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "atlas-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "atlas-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "atlas-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "atlas-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "atlas-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "atlas-kanban-pre atlas-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "atlas-kanban-meta-row" },
      h("span", { className: "atlas-kanban-meta-label" }, props.label),
      h("span", { className: "atlas-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "atlas-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "atlas-kanban-meta-row" },
        h("span", { className: "atlas-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "atlas-kanban-meta-value atlas-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "atlas-kanban-meta-row" },
        h("span", { className: "atlas-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "atlas-kanban-meta-value atlas-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "atlas-kanban-section" },
      h("div", { className: "atlas-kanban-section-head-row" },
        h("span", { className: "atlas-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "atlas-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "atlas-kanban-section" },
      h("div", { className: "atlas-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "atlas-kanban-deps-row" },
        h("span", { className: "atlas-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "atlas-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "atlas-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "atlas-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "atlas-kanban-dep-chip-x",
      ...
      h("div", { className: "atlas-kanban-deps-row" },
        h("span", { className: "atlas-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "atlas-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "atlas-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "atlas-kanban-section" },
      ...
      h("div", { className: "atlas-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "atlas-kanban-home-sub atlas-kanban-home-sub--on"
              : "atlas-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ATLAS_PLUGINS__ && typeof window.__ATLAS_PLUGINS__.register === "function") {
    window.__ATLAS_PLUGINS__.register("kanban", KanbanPage);
  }
})();
