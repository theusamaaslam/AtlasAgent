...
Small Node helper that bridges Atlas Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Atlas is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Atlas-Sidecar-Token`)
...
The Atlas plugin's `atlas photon setup` command runs `npm install`
here automatically.
