// Atlas Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Atlas): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Atlas -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Atlas-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `atlas photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Atlas can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Atlas'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Atlas allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Atlas, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-atlas-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Atlas supplied them so a known-good
