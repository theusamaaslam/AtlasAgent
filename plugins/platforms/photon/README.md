...
This plugin connects Atlas Agent to iMessage (and other Spectrum
...
TypeScript-only, Atlas runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Atlas-Sidecar-Token`.
...
There is no separate `login` command; like every other Atlas channel,
...
Run `atlas photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.atlas/.env` (the same place every other
...
Management metadata lives in `~/.atlas/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `atlas photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `atlas photon status`, a DM and a group roundtrip,
