...
Credential storage mirrors every other Atlas channel:

    * runtime SDK creds  -> ``~/.atlas/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.atlas/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a atlas dependency
...
# registers Atlas as its own client_id.
...
# Default name of the project Atlas provisions for the operator.
DEFAULT_PROJECT_NAME = "Atlas Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of atlas-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.atlas/auth.json`` honouring the active Atlas profile."""
    try:
        from atlas_constants import get_atlas_home
        return Path(get_atlas_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.atlas")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.atlas/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.atlas/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.atlas/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: atlas_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `atlas photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `atlas photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `atlas photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `atlas photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `atlas photon setup`)"
