...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `atlas slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_atlas_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Atlas does not currently need to route them into the agent loop.
            ...
            # /model work directly without /atlas prefix). A single regex
            ...
            # manifest (see `atlas slack manifest`). In Socket Mode, Slack
            ...
            from atlas_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/atlas$")

            @self._app.command(_slash_pattern)
            async def handle_atlas_command(ack, command):
            ...
            for _action_id in (
                "atlas_approve_once",
                "atlas_approve_session",
                "atlas_approve_always",
                "atlas_deny",
            ):
            ...
            for _action_id in (
                "atlas_confirm_once",
                "atlas_confirm_always",
                "atlas_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Atlas handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Atlas session — matching the per-thread behavior channels already
                ...
                },
                {
                    ...
                    "elements": [
                        ...
                        },
                        {
                            ...
                            "action_id": "atlas_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "atlas_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                ...
                },
                {
                    ...
                    "elements": [
                        ...
                        },
                        {
                            ...
                            "action_id": "atlas_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "atlas_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "atlas_confirm_once": "once",
            "atlas_confirm_always": "always",
            "atlas_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "atlas_approve_once": "once",
            "atlas_approve_session": "session",
            "atlas_approve_always": "always",
            "atlas_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/atlas <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/atlas
        ...
        if slash_name in {"atlas", ""}:
            # Legacy /atlas <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from atlas_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/atlas   ``).
        ...
        # "Running /cmd…" ack shown by handle_atlas_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/atlas <question>" must produce public replies so
...
# ``atlas_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``atlas_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ATLAS_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from atlas_cli.slack_cli import _build_full_manifest
            from atlas_constants import get_atlas_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Atlas",
                bot_description="Your Atlas agent on Slack",
            )
            target = Path(get_atlas_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `atlas slack manifest --write` anytime to refresh after "
                "Atlas adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `atlas update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://atlas-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``atlas_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'atlas-agent[slack]'",
        # Interactive setup wizard — replaces atlas_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in atlas_cli/gateway.py.
