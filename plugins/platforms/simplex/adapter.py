"""SimpleX Chat platform adapter (Atlas plugin).
...
This adapter ships as a Atlas platform plugin under
``plugins/platforms/simplex/``. The Atlas plugin loader scans the
...
Optional environment variables:
    ...
    ATLAS_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``atlas setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Atlas — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ATLAS_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``atlas setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.atlas/.env`` via
    ``atlas_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "atlas_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.atlas/.env"
        )
        return
