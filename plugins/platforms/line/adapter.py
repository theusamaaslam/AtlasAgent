"""
LINE Messaging API platform adapter for Atlas Agent.
...
to Atlas Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ATLAS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            atlas_home = Path(get_atlas_home()).resolve()
        except Exception:
            atlas_home = Path.home().joinpath(".atlas").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``atlas status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``atlas status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``atlas setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.atlas/.env`` via ``atlas_cli.config``.
    """
    print()
    ...
    try:
        from atlas_cli.config import get_env_var, set_env_var
    except ImportError:
        print("atlas_cli.config not available; set LINE_* vars manually in ~/.atlas/.env")
        return
