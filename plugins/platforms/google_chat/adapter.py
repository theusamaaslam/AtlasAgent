...
# ``model_tools`` import time, so ``atlas status``, ``atlas chat``, etc.
...
# ``atlas_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Atlas is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a atlas MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ATLAS_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _atlas_home = _get_atlas_home()
        except (ModuleNotFoundError, ImportError):
            _atlas_home = _Path.home() / ".atlas"
        self._thread_count_store = _ThreadCountStore(
            _atlas_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Atlas is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ATLAS_HOME", str(_Path.home() / ".atlas"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ATLAS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a atlas MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Atlas is thinking…' marker message.
        ...
        "Atlas is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Atlas is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Atlas is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Atlas is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Atlas is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``atlas setup``.

    The setup wizard at ``atlas_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.atlas/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. atlas-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.atlas/.env")
    print_info("Restart the gateway: atlas gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``atlas cron`` running as a
    separate process from ``atlas gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Atlas plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'atlas-agent[google_chat]'",
        ...
        # the "Atlas is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Atlas is thinking…' marker message "
