...
- Per-user tokens (keyed by sender email):
    ``${ATLAS_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ATLAS_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ATLAS_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ATLAS_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${ATLAS_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ATLAS_HOME helper so the token follows the user's
# profile (e.g. tests can override via ATLAS_HOME=/tmp/...).
try:
    from atlas_constants import display_atlas_home, get_atlas_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where atlas_constants isn't importable
...
def _atlas_home() -> Path:
    """Resolve ATLAS_HOME at call time (NOT module import).

    Tests and ``ATLAS_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.atlas/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _atlas_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _atlas_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _atlas_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _atlas_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _atlas_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install atlas-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'atlas-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ATLAS_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_atlas_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_atlas_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Atlas (native attachment delivery)"
    )
