"""
Microsoft Teams platform adapter for Atlas Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Atlas"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        atlas_action = data.get("atlas_action", "")
        session_key = data.get("session_key", "")

        if not atlas_action or not session_key:
        ...
        choice = choice_map.get(atlas_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="atlas_approve",
                    data={**btn_data_base, "atlas_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="atlas_approve",
                    data={**btn_data_base, "atlas_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="atlas_approve",
                    data={**btn_data_base, "atlas_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="atlas_approve",
                    data={**btn_data_base, "atlas_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Atlas\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.atlas/.env")
