"""
Email platform adapter for the Atlas gateway.

Allows users to interact with Atlas by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "atlas-agent" "version" "{_atlas_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `atlas config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `atlas gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Atlas Agent")
        ...
        msg_id = f"<atlas-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# atlas_cli/gateway.py, and the _send_email dispatch in
