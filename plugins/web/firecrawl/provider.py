...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``atlas tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_nous_tools_enabled():
        return ""
    return (
        ", or use the Atlas Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            " With your Atlas Gateway subscription you can also use the Tool Gateway. "
            "run `atlas tools` and select Nous Subscription as the web provider."
        )
    else:
