...
``{title, url, description, position}`` rows every other Atlas web
provider produces.
...
prefers Atlas-managed xAI Grok OAuth (via ``atlas auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.atlas/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``atlas tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `atlas auth` to sign in with "
