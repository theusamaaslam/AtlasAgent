...
def _save_mem0_json(atlas_home: str, data: dict) -> None:
...
def _setup_platform(atlas_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    provider.save_config(provider_config, atlas_home)

    if env_writes:
        _write_env(Path(atlas_home) / ".env", env_writes)
...
def _setup_oss(atlas_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(atlas_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "atlas-user")
    ...
    if env_writes:
        _write_env(Path(atlas_home) / ".env", env_writes)
    _save_mem0_json(atlas_home, {"mode": "oss", "user_id": user_id, "agent_id": "atlas", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, atlas_home: str) -> str:
...
_PGVECTOR_CONTAINER = "atlas-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "atlas"
...
def _setup_oss_interactive(atlas_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], atlas_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], atlas_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'atlas-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "atlas-user")

    agent_id = input("  Agent ID [atlas]: ").strip()
    agent_id = agent_id or "atlas"
    ...
    if env_writes:
        _write_env(Path(atlas_home) / ".env", env_writes)
    _save_mem0_json(atlas_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(atlas_home: str, config: dict) -> None:
    """Entry point called by atlas memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(atlas_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(atlas_home, config, flags)
        return
