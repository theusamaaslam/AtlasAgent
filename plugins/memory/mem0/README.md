...
```bash
atlas memory setup    # select "mem0"
```
...
Behavioral settings live in `$ATLAS_HOME/mem0.json` (set them via `atlas memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.atlas/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `atlas-user` | User identifier on Mem0 |
...
```bash
atlas memory setup mem0 --mode oss \
...
```bash
atlas memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ATLAS_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.atlas/mem0_qdrant"}}
  }
}
```
...
```bash
atlas memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ATLAS_HOME/mem0.json`).
