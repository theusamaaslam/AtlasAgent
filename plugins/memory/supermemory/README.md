...
- Supermemory API key from [app.supermemory.ai/integrations?connect=atlas](http://app.supermemory.ai/integrations?connect=atlas)
...
Config file: `$ATLAS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `atlas` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `atlas-{identity}` → `atlas-coder`). |
...
All Supermemory API calls send `x-sm-source: atlas`, and document writes stamp
`metadata.sm_source: atlas`. This is a **functional routing key, not telemetry**:
it groups Atlas-written memories into a dedicated "Atlas" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Atlas profile:

```json
{
  "container_tag": "atlas-{identity}"
}
```

For a profile named `coder`, this resolves to `atlas-coder`. The default profile resolves to `atlas-default`. Without `{identity}`, all profiles share the same container.
