...
Optional Atlas observability plugin that maps Atlas observer hooks to
...
not replace Atlas Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Atlas emit NeMo Relay lifecycle events for
the work Atlas already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Atlas Agent can:

- Preserve Atlas execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
atlas plugins enable observability/nemo_relay
```

The `ATLAS_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ATLAS_HOME` that the
agent run will use:

```bash
env ATLAS_HOME=/tmp/atlas-nemo-relay-test \
  atlas plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ATLAS_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ATLAS_HOME` is the Atlas profile/config home used by both
`atlas plugins enable ...` and the later `atlas chat ...` run. If unset,
Atlas uses the user's default home, usually `~/.atlas`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ATLAS_HOME=/tmp/atlas-nemo-relay-test
atlas plugins enable observability/nemo_relay
atlas chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `atlas` command you run is built from the
...
uv run atlas plugins enable observability/nemo_relay
uv run atlas chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/atlas_agent-*.whl
...
The plugin can configure exporters directly from `ATLAS_NEMO_RELAY_*`
...
```bash
export ATLAS_NEMO_RELAY_ATOF_ENABLED=1
export ATLAS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ATLAS_NEMO_RELAY_ATIF_ENABLED=1
export ATLAS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ATLAS_NEMO_RELAY_ATOF_FILENAME`
- `ATLAS_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ATLAS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ATLAS_NEMO_RELAY_ATIF_AGENT_NAME`
- `ATLAS_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ATLAS_NEMO_RELAY_ATIF_MODEL_NAME`
- `ATLAS_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export ATLAS_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ATLAS_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ATLAS_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ATLAS_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Atlas does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Atlas keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Atlas routes LLM and tool
...
For the full generic Atlas middleware contract, see
...
export ATLAS_HOME=/tmp/atlas-nemo-relay-docs/atlas-home
mkdir -p "$ATLAS_HOME"

cat > "$ATLAS_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Atlas session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ATLAS_NEMO_RELAY_ATOF_ENABLED=1
export ATLAS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/atlas-nemo-relay-docs/subagent/atof
export ATLAS_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ATLAS_NEMO_RELAY_ATOF_MODE=overwrite
export ATLAS_NEMO_RELAY_ATIF_ENABLED=1
export ATLAS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/atlas-nemo-relay-docs/subagent/atif
export ATLAS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ATLAS_NEMO_RELAY_ATIF_AGENT_NAME='Atlas Agent E2E'
export ATLAS_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ATLAS_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"atlas.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Atlas Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Atlas dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/atlas-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/atlas-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/atlas-nemo-relay-docs/workdir/beta.txt
cd /tmp/atlas-nemo-relay-docs/workdir

export ATLAS_NEMO_RELAY_ATOF_ENABLED=1
export ATLAS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/atlas-nemo-relay-docs/parallel/atof
export ATLAS_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ATLAS_NEMO_RELAY_ATOF_MODE=overwrite
export ATLAS_NEMO_RELAY_ATIF_ENABLED=1
export ATLAS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/atlas-nemo-relay-docs/parallel/atif
export ATLAS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ATLAS_NEMO_RELAY_ATIF_AGENT_NAME='Atlas Agent E2E'
export ATLAS_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Atlas sessions map to `agent` scopes.
- Atlas API request hooks map to `llm` scope start/end events.
- Atlas tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Atlas execution middleware to hand
...
```bash
export ATLAS_NEMO_RELAY_PLUGINS_TOML=/tmp/atlas-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Atlas routes execution
...
Atlas provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Atlas provider adapter next_call(...)

Atlas tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Atlas tool dispatcher next_call(...)
```
...
middleware for a local Atlas run. This path requires a NeMo Relay runtime that
...
```bash
export ATLAS_HOME=/tmp/atlas-middleware-test/atlas-home
mkdir -p "$ATLAS_HOME" /tmp/atlas-middleware-test/nemo-relay
...
cat > /tmp/atlas-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/atlas-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/atlas-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Atlas Middleware E2E"
agent_version = "local"
...
export ATLAS_NEMO_RELAY_PLUGINS_TOML=/tmp/atlas-middleware-test/nemo-relay/plugins.toml
