...
This document describes Atlas Agent's trust model, names the one
...
or open a private security advisory on GitHub. Do not open public issues for
...
- Environment details (`atlas version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Atlas Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Atlas Agent's documentation
...
Atlas Agent supports two OS-level isolation postures. They address
...
Atlas Agent supports this in two ways:

- **Atlas Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Atlas Agent's in-process heuristics
...
Atlas Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Atlas Agent
itself. Bugs in Atlas Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Atlas Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Atlas Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Atlas Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  atlas-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Atlas Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Atlas Agent. Bugs in Atlas
...
- Atlas Agent includes supply-chain guards for MCP server
